...
function store_message($name, $email, $info, $message,$groupid,$referrer,$phone='') {
...
if( count($errors) > 0 ) {
	setup_leavemessage($visitor_name,$email,$message,$groupid,$groupname,$info,$referrer,can_show_captcha(),$phone);
	setup_logo();
...
if($inbox_mail) {
	$link = connect();
	if(strpos($_SERVER['HTTP_REFERER'], 'support.vmc.in/client.php')){
		webim_mail($inbox_mail, $email, $subject, $body, $link);
		$leadAPI = "http://mcube.vmc.in/api/prospects_api?username=".urlencode('leads@vmc.in')."&password=vmc123&group=".urlencode('Webenquires')."&assignto=auto&source=".urlencode('Support')."&name=".urlencode($visitor_name)."&number=".$phone."&email=".urlencode($email)."&remark=".urlencode($message);
		$ch = curl_init();
