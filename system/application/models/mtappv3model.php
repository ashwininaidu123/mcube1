    ...
    function insert_calldetail()
    {
        ...
        if ($result->num_rows() > 0) {
            ...
            if ($res1->num_rows() > 0) {
                ...
                if ($res->num_rows() > 0) {
			     $sqlcbn = "SELECT * FROM " . $userDetails['bid'] . "_mt_callbarring  WHERE number = '".$callto."'";
        ...
        if ($result->num_rows() > 0) {
            ...
            if ($res1->num_rows() > 0) {
                ...
                if ($userDetails['deviceid'] == $deviceid) {
					...
					$query1 = "UPDATE ".$userDetails['bid']."_mt_emplist SET last_activity='".$dt."',version='".$version."' WHERE eid='".$userDetails['eid']."' AND bid='".$userDetails['bid']."'";
				    }else{
					$query1 = "UPDATE ".$userDetails['bid']."_mt_emplist SET last_activity='".$dt."' WHERE eid='".$userDetails['eid']."' AND bid='".$userDetails['bid']."'";
					}
					$this->db->query($query1);
