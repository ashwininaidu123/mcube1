...
Class Systemmodel extends Model
{
	...
	function addcustomfield(){
		...
		$valueupdate = ($this->input->post('valueupdate')) ? '1' : '0';
		...
		$this->db->set('valueupdate',$valueupdate);
		if(isset($_POST['custType']) && $_POST['custType'] == 1){
			$this->db->set('custType', $this->input->post('custType'));
			$this->db->set('custTypeval', $this->input->post('custTypeval'));
		}
	...
	function delete_customfields($id,$modid){
		...
		//if(($bid == 47 || $bid == 1  || $bid == 257)){
			...
			$getcusdetails = $this->getcusdetails($id,$bid);
			$module_name = $this->get_modules($modid);
			if($this->lang->line('table_'.$modid) != ''){
				...
				if($result){
		            ...
		            $this->auditlog->auditlog_info('Custom Label',$getcusdetails[0]['fieldname']. " Custom Field deleted for Module ".$module_name);
					return $modid;
				}
			}
		//~ }else{
			//~ $sql=$this->db->query("delete from ".$bid."_custom_label where fieldid=$id and modid=$modid and fieldtype='c'");
			//~ $sql1=$this->db->query("delete from ".$bid."_customfields where fieldid=$id and modid=$modid");
			//~ $sql1=$this->db->query("delete from ".$bid."_customfieldsvalue where fieldid=$id and modid=$modid");
			//~ $sql1=$this->db->query("delete from ".$bid."_role_access where fieldid=$id and fieldtype='c' and modid=$modid");
			//~ //$sql2=$this->db->query("delete from ".$this->session->userdata('bid')."_label_listing where label_name='$id'");
			//~ return $modid;
		//~ }
		return $modid;
	}
	function get_custom_info($id,$modid){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
			//echo "select * from ".$this->session->userdata('bid')."_custom_label where fieldid=$id and modid=$modid";exit;
			//$res=$this->db->query("select * from ".$bid."_custom_label where fieldid=$id and modid=$modid");
		$res=$this->db->query("SELECT f.fieldname,f.fieldtype,f.options,c.customlabel,f.custType,f.custTypeVal FROM ".$this->session->userdata('bid')."_customfields f LEFT JOIN ".$this->session->userdata('bid')."_custom_label c on f.fieldid=c.fieldid and f.modid=c.modid
	...
	function update_custom_label($id,$modid){
		...
		$q = '';
		if($types[0]['custType'] == 1){
			$val = $this->input->post('custTypeVal');
			$sql=$this->db->query("UPDATE ".$bid."_customfields SET custTypeVal='".$val."' WHERE fieldid=$id");	
		}
	...
	function get_all_modules($modid=''){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$q=($modid!='') ? " AND m.modid='".$modid."'" : '';
		//~ $sql=$this->db->query("select * from module where status=1 $q");
		$sql=$this->db->query("SELECT m.* FROM module as m  JOIN  business_feature bf on bf.feature_id = m.feature_id  WHERE m.status=1 AND bf.bid = ".$bid." UNION SELECT m.* FROM module as m  WHERE m.defualt_mod = 1 AND m.status = 1 $q ORDER BY modid ASC ");
		 $res=$sql->result_array();
		 return $res;
	}
	...
	function get_emp_roles(){
		...
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$r=array();
		//echo "select * from ".$this->session->userdata('bid')."_user_role";
	...
	//
	function getcusdetails($fieldid,$bid){
		$res=$this->db->query("SELECT * FROM ".$bid."_customfields WHERE fieldid='".$fieldid."'")->result_array();											
		return $res;
	}
