...
class Masteradmin_model extends Model {
	...
	function get_business(){
		$sql=$this->db->query("select bid,businessname from business where status=1 ORDER BY businessname ASC");
	    $res['0'] = 'Select';
		if($sql->num_rows()>0){
        ...
        function get_business_mconnect(){
		$sql=$this->db->query("SELECT b.bid,b.businessname,f.feature_id FROM business b "
                        . "LEFT JOIN business_feature f ON f.bid = b.bid"
                        . " WHERE b.status=1 AND f.feature_id = 17"
	...
	function get_businessname(){
		...
		if($sql->num_rows()>0){
 			$ress=$sql->result_array();	
			foreach($ress as $rec)
				$res[$rec['businessname']] = $rec['businessname'];
	    }				
	    return $res;
	}
	...
	function managePriList($ofset='0',$limit='20'){
		...
		$res=array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS pac.packagename,p.svdate,p.climit,p.rental,p.used,b.bid,p.package_id,p.number,p.pri,p.landingnumber,b.businessname as businessname,if(b.act=0,'Regular','Demo') as type,p.ownership,p.flimit,p.status,p.type as service,p.number_status  
										FROM prinumbers p
	...
	function getBusinesslist($ofset='0',$limit='20'){
		...
		$res=array();
    	$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS p.companyname,b.*,bd.accname,bd.accemail,bd.accphone FROM business b
								     	LEFT JOIN businessacc_details bd ON bd.bid = b.bid 
	...
	function Number_email($number,$clientname){
		$message_body=" ".$number." is Removed From the Business - ".$clientname."<br/><br/><br/>
						Regards<br/>
						MCube Team";
	...
	function Prinumber_del($bid,$pri){
		$check=$this->db->query("SELECT * FROM prinumbers WHERE number=$pri");
		$res=$check->row();
		$bdetails=$this->get_busValues($bid);
		if($res->type==0){
			$sql=$this->db->query("UPDATE ".$res->bid."_groups SET status=0,prinumber='' WHERE gid=".$res->associateid);
			$this->admin_activitylog($bid,$this->getPRilanding($pri) ." Pri has deleted");
		}elseif($res->type==1){
			$sql=$this->db->query("UPDATE ".$res->bid."_ivrs SET status=0,prinumber='' WHERE ivrsid='".$res->associateid."' AND bid='".$bid."'");
			$this->admin_activitylog($bid,$this->getPRilanding($pri) ." IVRS has deleted");
		}else{
	...
	function update_featuremanage($bid){
		...
		foreach($_POST['featureconfig'] as $key=>$val){
	        ...
	        $m=$this->db->query("SELECT feature_id FROM feature_list")->result_array();
			foreach($m as $f){
					$s=$this->db->query("SELECT feature_id FROM business_feature WHERE bid='".$bid."' AND feature_id = '".$f['feature_id']."'");
						if($s->num_rows()>0){
						$mo=$this->db->query("SELECT modules FROM feature_list WHERE feature_id = '".$f['feature_id']."'");
					    if($mo->num_rows()>0){
						$fetm = $mo->row()->modules;
						$modulenum = explode(",",$fetm); 
						foreach($modulenum as $mod){
													print_r($mod);
							 $query=	$this->db->query("SELECT * FROM ".$bid."_role_mod_opt WHERE bid='".$bid."' AND modid = '".$mod."'"); 
							  if($query->num_rows()==0){
							   $q=$this->db->query("SELECT roleid FROM ".$bid."_user_role WHERE bid='".$bid."'");
								$res=$q->result_array();
								 foreach($res as $rows){
									$query=	$this->db->query("SELECT * FROM ".$bid."_role_mod_opt WHERE bid='".$bid."' AND modid = '".$mod."' AND roleid  = 1");
									if($query->num_rows()==0){
									 $this->db->query("INSERT INTO ".$bid."_role_mod_opt (`bid`, `roleid`, `modid`, `opt_add`, `opt_view`, `opt_download`, `opt_delete`) VALUES ('".$bid."', '1', '".$mod."', '1', '1', '1', '1')");
									}
									else{
									 $this->db->query("INSERT INTO ".$bid."_role_mod_opt (`bid`, `roleid`, `modid`, `opt_add`, `opt_view`, `opt_download`, `opt_delete`) VALUES ('".$bid."', '".$rows['roleid']."', '".$mod."', '1', '1', '0', '0')");
									}
								 } 
							  }
						  } 
	...
	function AddBusinessUser($id=''){
		if($id!=""){
			...
			$this->db->set('busadmdes', $this->input->post('busadmdes')); 
			$this->db->where('bid',$id);
			$this->db->update('business');
		    $this->db->set('accname', $this->input->post('accname')); 
		    $this->db->set('accphone', $this->input->post('accphone')); 
		    $this->db->set('accemail', $this->input->post('accemail')); 
			$this->db->where('bid',$id);
			$this->db->update('businessacc_details');
...
function groupemplist($bid,$gid){
		...
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS e.empnumber,a.empweight,a.empPriority,
								...
								if(a.isfailover=1,'yes','no') as failover,a.status as grpstatus,e.status as empstatus,
								if(e.status='0',0,if(e.selfdisable='1','0','1')) as selfdisable,
...
function update_group($bid,$gid){
  ...
  function get_GroupValues($bid,$id){
    ...
    function getAllLandingnum($bid,$ofset='0',$limit='20'){
		...
		$res=array();
		$res['data']=$this->db->query("  SELECT  g.gid,g.status,g.groupname as GroupName,gr.regionname as Regionname,e.empname as Groupowner,r.rulename
	...
	function getAddons($ofset='0',$limit='20'){
		...
		$res=array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM features WHERE 1 $q
										LIMIT $ofset,$limit
									   ")->result_array();
		$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
	...
	function addAddon($id){
		...
		$this->db->set('rate',$this->input->post('rate1'));
	...
	function number_status($number,$lno){
		$res=$this->getPridetails($number);
		$status = ($res->number_status!=0) ? '0' : '1';
		$this->db->query("UPDATE prinumbers SET number_status='".$status."' WHERE number='".$number."' AND landingnumber='".$lno."'");
		$this->admin_activitylog('0',$lno." Number Status changed ");
		$bdetail=$this->get_busValues($res->bid);
		$message = ($status!=1) ? $lno ." Deactivated of this '".$bdetail[0]['businessname']."'" : $lno ." Activated of this ".$bdetail[0]['businessname'];
		$body = $this->emailmodel->newEmailBody($message,' All');
		$config['charset'] = 'utf-8';
		$config['newline'] = "\r\n";
		$config['mailtype'] = 'html'; 
	...
	function addnwbeacon(){
		$sql = "SELECT beacon_id FROM beacon WHERE beacon_id='".$_POST['beacon_id']."'";
		$rest = $this->db->query($sql);
		if($rest->num_rows()>0){
		return false;
	}else{
		$id=$this->db->query("SELECT COALESCE(MAX(`id`),0)+1 as id FROM beacon")->row()->id;
		$this->db->set('id',$id);
		$this->db->set('assigned','0');
		$this->db->set('beacon_id',$this->input->post('beacon_id'));
		$this->db->insert("beacon");
		$this->admin_activitylog('0',$this->input->post('beacon_id')." Location Id added successfully");
		return true;
	}
        }      
	...
	function getlocationlist($bid,$ofset='0',$limit='20'){
		$q='WHERE bid = '.$bid.'';
		if(isset($_POST['submit'])){
			$this->session->set_userdata('search',$_POST);
		}/*else{
		...
		$q.=(isset($s['beacon_id']) && $s['beacon_id']!='')?" and beacon_id ='".$s['beacon_id']."'":"";
		$res = array();
                $res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS b.beacon_id as SensorId, a.businessname as BusinessName FROM beacon b
		                               LEFT JOIN business a on a.bid=b.business $q LIMIT $ofset,$limit")->result_array();
                $res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
		return $res;	
		
	}
	function getunasdlocation($ofset='0',$limit='20'){
		$q='WHERE assigned = "0"';
		if(isset($_POST['submit'])){
			$this->session->set_userdata('search',$_POST);
		}/*else{
		...
		$q.=(isset($s['beacon_id']) && $s['beacon_id']!='')?" and beacon_id ='".$s['beacon_id']."'":"";
		$res = array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS beacon_id as SensorId FROM beacon
		                                $q LIMIT $ofset,$limit")->result_array();
		$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
		return $res;	
	...
	function getBeaconlist(){
		$res=array();	
		$sql = $this->db->query("SELECT * FROM beacon WHERE assigned = '0'");
		$res['0'] = 'Select';
		if($sql->num_rows()>0){
 			$ress=$sql->result_array();	
			foreach($ress as $rec)
	...
	function assigntosite(){
		$this->db->set('business',$this->input->post('bname'));
		$this->db->set('assigned','1');
		$this->db->where('beacon_id',$this->input->post('sensor_id'));
		$this->db->update('beacon');
	}
	function delete_getunasdloc($sensorid){
		$this->admin_activitylog('0',$sensorid." Sensor id was deleted");
		$sql=$this->db->query("Delete from beacon where assigned=0 and beacon_id='".$sensorid."'");
		return true;
	}

	function getDND($eid,$bid){
		$this->load->helper('mcube_helper');
		$number = $this->db->query("SELECT empnumber as number FROM ".$bid."_employee WHERE bid='".$bid."' AND eid='".$eid."'")->row()->number;
		$dnd = (array)filter_dnd($number);
		if($dnd['dnd']==0){
			return 0;
		}else{
			return 1;
	...
	function getAllEmployee($bid,$ofset='0',$limit='20'){
		$q = ' WHERE 1';
		if(isset($_POST['submit'])){
			$this->session->set_userdata('search',$_POST);
		}else{
		...
		$q.=(isset($s['empname']) && $s['empname']!='')?" AND e.empname LIKE '%".$s['empname']."%'":"";
		$q.=(isset($s['empemail']) && $s['empemail']!='')?" AND e.empemail LIKE '%".$s['empemail']."%'":"";
		$res = array();
		$res['data']=$this->db->query("SELECT  e.*,r.rolename FROM ".$bid."_employee e LEFT JOIN ".$bid."_user_role r ON e.roleid = r.roleid $q ")->result_array();
	   	$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
		return $res;
	}
    function getfeatureAccess($fetId,$bid){
		$res = $this->db->query("SELECT feature_id FROM business_feature WHERE bid='".$bid."' AND feature_id ='".$fetId."'");
		if($res->num_rows() > 0)
			return '1';
		else
			return '0';
	...
	function mtrackEmp($bid){
		$num = isset($_POST['mtracker_licence']) ? $_POST['mtracker_licence'] : 0 ;
		$valid = isset($_POST['mt_record_valid']) ? $_POST['mt_record_valid'] : 0;
		$res = $this->db->query("SELECT bid FROM account_settings WHERE bid='".$bid."'");
		if($res->num_rows() > 0){
			$sql = "UPDATE account_settings SET mtracker_licence='".$num."',mt_record_valid='".$valid."' WHERE bid='".$bid."'";
		}else{
			$sql = "INSERT INTO account_settings SET mtracker_licence='".$num."',mt_record_valid='".$valid."', bid='".$bid."'";
		}
		$res = $this->db->query($sql);
		return $res;
	}
	function getmtrackEmp($bid){
		$res = $this->db->query("SELECT mtracker_licence FROM account_settings WHERE bid='".$bid."'");
		if($res->num_rows() > 0){
			foreach($res->result_array() as $ar){
				$str['mtracker_licence'] = $ar['mtracker_licence'];
			}
			return $str;
		}else
			return '0';
	}
	function total_trackcount($bid){
	    $res = array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS count(c.starttime) as callcount, a.businessname as BusinessName,date(c.starttime) as date  FROM ".$bid."_callhistory c
							   LEFT JOIN business a on a.bid=c.bid
							   WHERE DATE(c.starttime) = CURRENT_DATE() AND DATE(c.endtime) = CURRENT_DATE()")->result_array();
		$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
		return $res;
	...
	function chech_email(){
		...
		$sql=$this->db->query("SELECT username,eid FROM user WHERE username='".$email."'")->row_array();
        if($sql){
		    $res = $this->db->query("SELECT contactemail FROM business WHERE contactemail='".$email."'")->row()->contactemail;
			return $res;
		}else{
			return '0';
		}
	}
   function update_businessemail($bid){
	   $email = $this->input->post('email');
	   $newemail = $this->input->post('contactemail');
	   $eid = $this->input->post('eid');
	   $uid = $this->input->post('uid');
	   $res = $this->db->query("SELECT * FROM business WHERE contactemail='".$email."'");
	   if($res->num_rows() > 0){
		  	$sql = $this->db->query("UPDATE business SET contactemail='".$newemail."',businessemail='".$newemail."' WHERE bid='".$bid."' AND  contactemail='".$email."'");
		  	$sql1 = $this->db->query("UPDATE user SET username='".$newemail."' WHERE bid='".$bid."' AND  username='".$email."'");
		  	$sql2 = $this->db->query("UPDATE ".$bid."_employee SET empemail='".$newemail."' WHERE bid='".$bid."' AND  empemail='".$email."'");
		  	$sql3 = $this->db->query("INSERT INTO deleted_email SET bid='".$bid."',oldemail='".$email."', newemail='".$newemail."', uid='".$uid."', eid='".$eid."'");
		    $this->admin_activitylog($bid,"Email updated by '".$this->session->userdata('username')."' from '".$email."' to '".$newemail."'" );
	   }else{
		    $sql = $this->db->query("UPDATE user SET username='".$newemail."' WHERE bid='".$bid."' AND  username='".$email."'");
		  	$sql1 = $this->db->query("UPDATE ".$bid."_employee SET empemail='".$newemail."' WHERE bid='".$bid."' AND  empemail='".$email."'");
		  	$sql2 = $this->db->query("INSERT INTO deleted_email SET bid='".$bid."',oldemail='".$email."', newemail='".$newemail."', uid='".$uid."', eid='".$eid."'");
		  	$this->admin_activitylog($bid,"Email updated by '".$this->session->userdata('username')."' from '".$email."' to '".$newemail."'" );
	   }
	}
	
	/**************  Feedback Categories UI END **********************/
   function getAllMT_Employee($bid,$ofset='0',$limit='20'){
		$q = ' WHERE 1';
		if(isset($_POST['submit'])){
			$this->session->set_userdata('search',$_POST);
		}else{
		...
		$q.=(isset($s['empemail']) && $s['empemail']!='')?" AND e.empemail LIKE '%".$s['empemail']."%'":"";
		$res = array();
		$res['data']=$this->db->query("SELECT  e.*,me.last_activity,me.last_login,me.model,u.recording,u.mcubecalls FROM ".$bid."_employee e
		                               LEFT JOIN ".$bid."_mt_emptimecheck me on me.eid = e.eid 
		                               LEFT JOIN user u on u.eid = e.eid 
		                               $q GROUP BY e.eid")->result_array();
	   	$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
		return $res;
	}
}
