...
Class Groupmodel extends Model
{
	...
	function create_group(){
		...
		for($i=0;$i<sizeof($arr);$i++){
			if(!in_array($arr[$i],array("update_system","gid","clone","gids","oldprinumber","groupname","prinumber","rules","eid","keyword","primary_rule","record","bday","hdaytext","replymessage","replytocustomer","replytoexecutive","replytoexecutive","timeout","oneditaction","onhangup","replyattmsg","leadaction","oncallaction","supportaction","supportgrp","landingregion","replytocust_missed","replytocust_attended","replytocust_repcal","voicemessagetext","voicmess"))){
	   ...
	   if(isset($_FILES['voicemessage']) && $_FILES['voicemessage']['error']==0){
		...
		for($i=0;$i<sizeof($arr);$i++){
				if(!in_array($arr[$i] , array('custom','clone','update_system','oldprinumber','gid','gids','voicmess'))){
					if(is_array($_POST[$arr[$i]]))
					$val = @implode(',',$_POST[$arr[$i]]); 
					elseif($_POST[$arr[$i]]!="")
						$val=($_POST[$arr[$i]]);
		...
		$this->db->set('groupkey',$groupKey);
	   	if($_FILES['voicemessage']['name'] == ''){
		$voicemessagetext=(isset($_POST['voicemessagetext']))?$this->input->post('voicemessagetext'):"";
		$this->db->set('voicemessagetext',$voicemessagetext);
		}
	...
	function update_group($gid){
	   ...
	   if($_FILES['voicemessage']['error']==0){
		...
		$this->db->set('voicemessagetext',$voicemessagetext);
	    if($_POST['voicmess']){
		$voicmess=(isset($_POST['voicmess']))?"1":"0";
		$this->db->set('custom_voice_file',$voicmess);
	    }
		$replytocust_missed=(isset($_POST['replytocust_missed']))?"1":"0";
		$this->db->set('replytocust_missed',$replytocust_missed);
		$replytocust_attended=(isset($_POST['replytocust_attended']))?"1":"0";
	...
	function getgrouplist($bid,$ofset='0',$limit='20'){
		...
		foreach($fieldset as $field){
			...
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				...
				if($checked){
				if(!in_array($field['fieldname'],array('hdayaudio','greetings','url','voicemessage','replytocust_missed','replytocust_attended','replytocust_repcal'))){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add) ? '<li><a href="EditGroup/'.$r['gid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_view) ? '<li><a href="group/activerecords/'.$r['gid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-file"  title="View Group">&nbsp;ViewGroup</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'group/Delete_group/'.$r['gid'].'" class="deleteClass delnotify" title="Are you sure to Delete the group"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= ($opt_add) ? '<li><a href="AddempGroup/'.$r['gid'].'"><span class="glyphicon glyphicon-plus" title="AddEmployee">&nbsp;AddEmployee</span></a></li>':'';
				$act .= ($opt_view) ? '<li><a href="ListempGroup/'.$r['gid'].'"><span title="List Employees" class="glyphicon glyphicon-user">&nbsp;ListEmployee</span></a>':'';
				$act .= ($opt_add) ? '<li><a href="group/add_group/'.$r['gid'].'/clone"><span title="Clone Group" class="glyphicon glyphicon-th-large">&nbsp;CloneGroup</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			...
			foreach($keys as $k){
				if($k=='eid'){
					...
					$v = '<a href="GrpTrackReport/'.$r['gid'].'">'.$r['groupname'].'</a>';
				}elseif($k=='supportgrp'){
					$v = $r['supportgroup'];
					...
					$t = '';
					foreach($bday as $b => $d){ $t .= (isset($d->day) && $d->day=='1')?$b.'='.$d->st.'-'.$d->et.' ':'';}
					$v = "<a href='javascript://' rel='popover' data-content='".$t."' data-placement='top' data-toggle='popover' data-container='body'>Working Hours</a>";
	...
	function bulkDelete($arr) {
		
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$arr2 = str_replace('on,','',$arr);
		
		foreach(explode(",", $arr) as $groupId) {
			
			$this->db->set('status',0);
			$this->db->where('gid', $groupId); 
			$this->db->update($bid."_group_emp"); 
			$gdetails=$this->configmodel->getDetail('3',$groupId,'',$bid);
			$this->ivrsmodel->updatePri($gdetails['prinumber'], 0);

		}

		$sql = $this->db->query("UPDATE ".$bid."_groups SET status=0 WHERE gid IN(".$arr2.")");
		...
		if($this->db->affected_rows() > 0) {
			...
			if($deleteNotifyStatus[0]['del_notify']) {
				$this->sendGroupBulkDeleteEmail($bid,$arr);
			}
	...
	public function sendGroupBulkDeleteEmail($bid,$groupIdsArray)
    {
    	$groupIdArray = explode(",", $groupIdsArray);
		$message = "The List of Deleted Group names's are as follow : ";
		$message .= '<ol>';
		for($i=0; $i<sizeof($groupIdArray); $i++) {
			$gname=$this->db->query("SELECT groupname from 	".$bid."_groups where gid='".$groupIdArray[$i]."'")->row()->groupname;
			$message .= '<li>'.$gname.'</li>';
		}
		$message .=  '</ol>';
		...
		$subject = 'MCube Deleted Group Details';
	...
	function recent_calls($gid){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS cnt
							...
							WHERE (h.`starttime` >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)) AND h.gid = ".$gid." GROUP BY DAY(h.starttime)");
	...
	function getdeletedgrouplist($bid,$ofset='0',$limit='20'){
		...
		foreach($fieldset as $field){
			...
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				foreach($roleDetail['system'] as $f){if($f['fieldid']==$field['fieldid'])$checked = true;}
				if($checked){
					if(!in_array($field['fieldname'],array('hdayaudio','greetings','url','voicemessage','replytocust_missed',
											'replytocust_attended',
											'replytocust_repcal'))){
						array_push($keys,$field['fieldname']);
						array_push($header,(($field['customlabel']!="")
											? $field['customlabel']
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="'.base_url().'group/UNDelete_group/'.$r['gid'].'" class="deleteClass1" data-toggle="modal" data-target="#modal-responsive" id="'.$r['gid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
	...
	function empllistmt($grid=''){
		...
		$query = "SELECT e.* FROM ".$bid."_employee e  WHERE e.status=1 ".$q." ORDER BY e.empname";
		$query=$this->db->query($query);
		if($query->num_rows()>0){
			foreach($query->result_array() as $rt)
			$res[$rt['eid']]=$rt['empname'];
	...
	function Insert_Same($id){
		...
		$arr=array_keys($_POST);
	    	for($i=0;$i<sizeof($arr);$i++){
				//~ if(!in_array($arr[$i],array("update_system","gid","clone","gids","oldprinumber","groupname","prinumber","rules","eid","keyword"
			//~ ,"primary_rule","record","bday","hdaytext","replymessage","replytocustomer","replytoexecutive","timeout"
		...
		foreach($sf as $f){
			...
			if($f['modid']==3 && isset($_POST[$f['fieldname']]) && !in_array($f['fieldname'],array('oldprinumber','gid','clone','voicmess'))){
				$this->db->set($f['fieldname'], $_POST[$f['fieldname']]);
				if(isset($_POST['url']))$this->db->set('url', $_POST['url']);
				if(isset($_POST['bday']))$this->db->set('bday', json_encode($_POST['bday']));
				if(isset($_POST['access_reports']) && !empty($_POST['access_reports']))$this->db->set('access_reports', @implode(',',$_POST['access_reports']));
	...
	function supportNumberChk($bid){
     ...
     function delvoicemessage($gid,$bid){  
		$sql=$this->db->query("UPDATE ".$bid."_groups SET voicemessage = NULL, custom_voice_file = '0' WHERE gid='".$gid."'");	
		return 1;
	}
    ...
    function get_bpackages($bid,$pid){
		$sql=$this->db->query("SELECT feature_id FROM `business_packageaddons` WHERE bid='".$bid."' AND number ='".$pid."'")->result_array();
		return $sql;
	}
