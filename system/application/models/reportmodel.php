...
Class Reportmodel extends Model
{
	...
	function getReportlist($bid,$ofset='0',$limit='20',$type='a',$tab)
	{
		...
		$q .= ($type=='p') ? " AND a.emotion ='p' ":"";
		$q .= ($type=='n') ? " AND a.emotion ='n' ":"";
		...
		$q.=(isset($s['keyword1']) && $s['keyword1']!='')?" and a.keyword1 like '%".$s['keyword1']."%'":"";
		$q.=(isset($s['keyword2']) && $s['keyword2']!='')?" and a.keyword2 like '%".$s['keyword2']."%'":"";
		...
		if($roleDetail['role']['admin']!=1){
			if($roleDetail['role']['roleid']==4){
				...
				$q .= " AND (a.eid IN (".$this->session->userdata('eid').") 
						OR d.eid='".$this->session->userdata('eid')."' 
						OR a.assignto='".$this->session->userdata('eid')."')";
			}
		}
		$type1 = $DB2->query("SELECT lead_generate FROM business WHERE bid='".$bid."'")->row()->lead_generate;
		if($type1 == '0'){
			$q .= " AND a.pulse > '0'";
		}
		...
		if($uniqueBy == ''){
			$sql = "SELECT SQL_CALC_FOUND_ROWS a.callid,a.hid,
					...
					if(a.pulse=0,'1','0') as missed,l.lead_status,a.leadid FROM 
					".$bid."_callhistory a
			...
			$sql = "SELECT SQL_CALC_FOUND_ROWS h2.callid,h2.hid,
					...
					if(h2.pulse=0,'1','0') as missed,a.cnt,l.lead_status,h2.leadid FROM 
		...
		$ret['header'] = $header;
	    $lisiting = isset($_POST['lisiting'])?$_POST['lisiting']:"";
	    /////// search download ////////////
		if($this->input->post('submit') == 'Search'){
		$ret['search_download'] = $this->search_download($s,$lisiting,$type);
	    }
	    /////// search download ////////////
		$list = array();
		$i = $ofset+1;
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a href="EditTrackReport/'.$r['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;</span></a>
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;">&nbsp;<span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_add) ?'<li><a href="EditTrackReport/'.$r['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/Delete_call/'.$r['callid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= '<li><a href="Report/activerecords/'.$r['callid'].'" class="btn-list" data-toggle="modal" data-target="#modal-list"><span class="glyphicon glyphicon-file"  title="Call Detail">&nbsp;CallDetails</span></a></li>';
				...
				$act .= '<li><a href="Report/empdetail/'.$rec['callid'].'" class="btn-empl" data-toggle="modal" data-target="#modal-empl"><span title="List Employees" class="glyphicon glyphicon-user">&nbsp;ListEmployees</span></a></li>';
				$act .= '<li><a href="Report/followup/'.$rec['callid'].'/0/calltrack" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span></a></li>';		
				$act .= ($r['caller_email']!='')?"<li><a href=\"javascript:void(null)\" onClick=\"window.open('/Email/compose/".$rec['callid']."/calls', 'Send Email', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=970,height=480,resizable=1')\"><span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a></li>":'<li><a><span title="Send Mail" class="glyphicon glyphicon-envelope">&nbsp;SendMail</span></a></li>';
				$act .= "<li>".anchor("Report/clicktoconnect/".$rec['callid']."/1", '<span title="click To Connect" class="glyphicon glyphicon-phone">&nbsp;ClickToConnect</span>',array('class'=>'clickToConnect'))."</li>";
				$act .= "<li>".anchor("Report/sendSms/".$rec['callid']."/calltrack", '<span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;SendSMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"')."</li>";	
				$act .= "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('Report/sendFields/".$rec['callid']."/".$bid."/6/calltrack', 'Send Fields', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=550,height=500,left=200,top=20,resizable=1')\"><span title='Click to Send Fields' class='glyphicon glyphicon-list'>&nbsp;SendFields</span></a></li>";
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
				...
				//~ }elseif($k=='callfrom'){
					//~ $v = anchor("Report/clicktoconnect/".$r['callid']."/1","$r[$k]",array('class'=>'clickToConnect','title' =>'Click to connect'));
			    }elseif($k=='callfrom'){
					 if(strlen($r['callfrom'])>='12'){
						$v = $r['callfrom'];
					}else{
						$v ='0'.$r['callfrom'];
					}
				...
				}elseif($k == 'speech_text'){
					$v = "<a href='javascript://' rel='popover' data-content='".$r[$k]."' data-placement='top' data-toggle='popover' data-container='body'>".substr($r[$k],0,50)."</a>";
				}elseif($k == 'word_cloud'){
					if($r[$k] != '')
						$v = '<a href="'.$r[$k].'" class="btn magnific" title="Please Click to View Word Cloud"><i class="fa fa-krw"></i></a>';
				}else{
					$v = isset($r[$k])? nl2br(wordwrap($r[$k],60,"\n")) :"";
				}
				$leadColor = $rec['lead_status'];
				$v = ($r['lead']!=null && $rec['leadid'] != 0 && $leadColor != 1)? "<font color='#EC54EF'>".$v."</font>"
						:(($rec['leadid'] != 0 && $leadColor == 1) ? "<font color='#007F55'>".$v."</font>"
			...
			array_push($data,$v);
			if($r['onhangupResponse'] != '' || $r['oncallResponse'] != ''){
				$v=	"<a href=\"Javascript:void(null)\" onClick=\"window.open('Report/showresp/".$rec['callid']."', 'API Response', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=980,height=500,resizable=1')\">Show</a>";
			}else{
				$v=	"Show";
			}
			array_push($data,$v);
	    ...
	    /////// search download ////////////
	function search_download($s,$lisiting,$type='a'){
		...
		////   only call history /////
		//~ $sql="SELECT SQL_CALC_FOUND_ROWS a.callid,a.starttime,'ct' as type
				//~ FROM ".$bid."_callhistory a
				...
				//~ LIMIT 0,$limit";	
	    ////   only call history + call archive /////			
 ...
 /* mail functionality for download */
		$sql1 = $this->db->query("SELECT down_notify FROM account_settings WHERE bid='".$bid."'");
		$notify = '';
		if($sql1->num_rows() >0){
			$res = $sql1->row();
		...
		if($notify == 1){
			...
			if((isset($_POST['groupname'])) ){
				...
				if($mail_groupid!='') {
					$sql_groupname = $this->db->query("SELECT groupname FROM ".$bid."_groups WHERE gid IN (".$mail_groupid.")")->result_array();
			...
			if(isset($_POST['empname'])){
				...
				$sql_empname = $this->db->query("SELECT empname FROM ".$bid."_employee WHERE eid IN (".$mail_empid.")")->result_array();
	...
	function getDeletedlist($bid,$ofset='0',$limit='20',$type='a'){//echo $limit;
		...
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$q.=(isset($s['calid']) && $s['calid']!='')?" and a.calid = '".$s['calid']."'":"";
		$q.=(isset($s['gid']) && $s['gid']!='' && $s['gid']!='0')?" and a.gid = '".$s['gid']."'":"";
		$q.=(isset($s['callername']) && $s['callername']!='')?" and a.callername like '%".$s['callername']."%'":"";
		$q.=(isset($s['caller_email']) && $s['caller_email']!='')?" and a.caller_email like '%".$s['caller_email']."%'":"";
		$q.=(isset($s['callfrom']) && $s['callfrom']!='')?" and a.callfrom like '%".$s['callfrom']."%'":"";
		$q.=(isset($s['empid']) && $s['empid']!='' &&  $s['empid']!='0')?" and a.eid = '".$s['empid']."'":"";
		$q.=(isset($s['starttime']) && $s['starttime']!='')?" and date(a.starttime)>= '".$s['starttime']."'":"";
		$q.=(isset($s['endtime']) && $s['endtime']!='')?" and date(a.endtime)<= '".$s['endtime']."'":"";
		$q.=(isset($s['pulse']) && $s['pulse']!='')?" and if(a.pulse>0,ceil(a.pulse/60),a.pulse) ".$s['ptype']." '".$s['pulse']."'":"";	
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		...
		foreach($rst as $rec){
			$data = array($i);
			$r = $this->configmodel->getDetail('6',$rec['callid'],'',$bid);
		    $v='<input type="checkbox" class="blk_check" name="blk[]" value="'.$r['callid'].'"/>';	
			array_push($data,$v);
			if($opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/UnDel/'.$r['callid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>':'';
				$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename']))
					?'<li><a target="_blank" href="'.site_url('sounds/'.$r['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">Recording</span></a></li>'
					:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li> '):"";
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
	...
	function UnDel($callid,$bid){
		...
		$this->auditlog->auditlog_info('Report',"Call /" .$callid. "/ UnDeleted By ".$this->session->userdata('username'));
		return true;
	}
	...
	function getArchivelist($args){
		...
		if($year!=0 && $month!=0){
			...
			foreach($rst as $rec){
				...
				if($opt_add || $opt_view || $opt_delete){
					$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
					$act .= '<li><a href="Report/archive_records/'.$rec['callid'].'" class="btn-list" data-toggle="modal" data-target="#modal-list"><span class="glyphicon glyphicon-file"  title="Call Detail">&nbsp;CallDetails</span></a></li>';
					$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='')
						?'<li><a target="_blank" href="'.$r['archive_path'].'/'.$r['filename'].'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Recording</span></a></li>'
						:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li>'):"";
			...
			$sql = "SELECT SQL_CALC_FOUND_ROWS count(*) `count`,
					...
					ORDER BY DATE_FORMAT(`starttime`,'%Y') DESC,DATE_FORMAT(`starttime`,'%m') DESC  LIMIT $ofset,$limit";
			$rst = $this->db->query($sql)->result_array();
			...
			foreach($rst as $rec){
				...
				foreach($keys as $k){
					$v = isset($rec[$k])?$rec[$k]:"";	
					array_push($data,$v);
					$act = "<a href='CallArchive/".$rec['year']."/".$rec['nmonth']."'>List</a>&nbsp;&nbsp;&nbsp;" .
	...
	function getReportlist1($id,$bid){
		...
		foreach($rst as $rec){
			...
			$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename']))
					...
					:'<a><span class="glyphicon glyphicon-volume-off"></span></a>'):"";
	...
	function delete_call($id,$bid){
		...
		$this->db->update($bid."_callhistory");
		$this->auditlog->auditlog_info('Report',"Call /" .$id. "/ Deleted By ".$this->session->userdata('username'));
	/* mail functionality for Delete */
		$sql1 = $this->db->query("SELECT del_notify FROM account_settings WHERE bid='".$bid."'");
		$notify = '';
		if($sql1->num_rows() >0){
			$res = $sql1->row();
			$notify = $res->del_notify;
		}
		if($notify == 1){
			$sql = $this->db->query("SELECT empemail FROM ".$bid."_employee WHERE roleid=1 and status=1")->result_array();
			$sqlempemail = array();
			...
			$empname_arr = array();	
			if(isset($sqlempemail_val)){
				$message1  = "MCube Track Deleted Record Details:<br/>";
				$message1 .= "<br/>Call ID:".$id;
				$message1 .= "<br/>Deleted By:".$this->empgetname($this->session->userdata('eid'));
				$body = $this->emailmodel->newEmailBody($message1,'');
				$to  = $sqlempemail_val; 
				$subject = ' MCube Track Record Deleted Details ';
				$this->load->library('email');
				$this->email->from('noreply@mcube.com', 'MCube');
				$this->email->to($to);
				$this->email->subject($subject);
	...
	function update_caller_details($id,$bid){
		$_POST = $this->configmodel->add_slashes_recursive($_POST);
		...
		if($sql->num_rows()>0){
			if(isset($_POST['assignto']) && $itemDetail['assignto']!=$_POST['assignto']){
				...
				$sql = $this->db->query("SELECT empemail,empnumber,empname FROM ".$bid."_employee WHERE eid='".$_POST['assignto']."'")->result_array();
		        $message = "You have assigned to New Lead. The New Lead Details are <br/>";
				$message .= "<br/>Name:".$this->input->post('callername');
				$message .= "<br/>Email:".$this->input->post('caller_email');
	...
	function groupnames($url){

		$ret=array();
		if($this->session->userdata('search')){
			$se = $this->session->userdata('search');
		}
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
		...
		$q .= ($url=='a')?"":"";
		$q .= ($url=='m')?" AND pulse=0":"";
		$q .= ($url=='q')?" AND (callername!='' OR callerbusiness!='' OR calleraddress!='' OR caller_email!='' OR remark!='')":"";
		$q .= ($url=='u')?" AND (callername='' AND callerbusiness='' AND calleraddress='' AND caller_email='' AND remark='')":"";
		$q .= ($url=='at')?" AND pulse!=0":"";
		$q .= ($url=='p') ? " AND emotion ='p' ":"";
		$q .= ($url=='n') ? " AND emotion ='n' ":"";

		$q.=(isset($se['gid']) && $se['gid']!='0')?" AND g.gid = '".$se['gid']."'":"";
		$q.= (isset($se['callfrom']) && $se['callfrom']!='')? " AND callfrom='".$se['callfrom']."'":'';
		$q.=(isset($se['empid']) && $se['empid']!='0')?" AND eid = '".$se['empid']."'":"";
		$q.=(isset($se['starttime']) && $se['starttime']!='') ? " AND date(starttime)>= '".$se['starttime']."'":"";
		$q.=(isset($se['endtime']) && $se['endtime']!='')?" AND date(endtime)<= '".$se['endtime']."'":"";
		$q.=(isset($se['callername']) && $se['callername']!='')?" AND callername like '%".$se['callername']."%'":"";
		$q.=(isset($se['caller_email']) && $se['caller_email']!='')?" AND caller_email like '%".$se['caller_email']."%'":"";
		$q.=(isset($se['pulse']) && $se['pulse']!='')?" AND if(pulse>0,ceil(pulse/60),pulse) ".$se['ptype']." '".$se['pulse']."'":"";	
		$q.=(isset($se['dialstatus']) && $se['dialstatus']!='')?" and dialstatus like '%".$se['dialstatus']."%'":"";
		$q.=(isset($se['calid']) && $se['calid']!='')?" and calid = '".$se['calid']."'":"";
		$q.=(isset($se['refid']) && $se['refid']!='')?" and refid = '".$se['refid']."'":"";
		$q.=(isset($se['source']) && $se['source']!='')?" and source like '%".$se['source']."%'":"";
		$q.=(isset($se['keyword']) && $se['keyword']!='')?" and keyword like '%".$se['keyword']."%'":"";
		$q.=(isset($se['keyword1']) && $se['keyword1']!='')?" and keyword1 like '%".$se['keyword1']."%'":"";
		$q.=(isset($se['keyword2']) && $se['keyword2']!='')?" and keyword2 like '%".$se['keyword2']."%'":"";
		$q.=(isset($se['last_modified']) && $se['last_modified']!='')?" and last_modified like '%".$se['last_modified']."%'":"";
        $q.=(isset($se['assignto']) && $se['assignto']!='')?" and k.empname like '%".$se['assignto']."%'":"";
		
		$sql=$this->db->query("SELECT g.gid,g.groupname,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2  AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 0 DAY)) ".$q." ) as day0,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND  date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY)) ".$q." ) as day1,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 2 DAY)) ".$q." ) as day2,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 3 DAY)) ".$q." ) as day3,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 4 DAY)) ".$q." ) as day4,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 5 DAY)) ".$q." ) as day5,
			(SELECT count(callid) FROM ".$this->session->userdata('bid')."_callhistory 
			WHERE gid=g.gid AND status!=2 AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 6 DAY)) ".$q." ) as day6
	...
	function total_calls($gettimelimit){
		$q='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
	    ...
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
		$date=date('Y-m-d',strtotime('-6 days'));
	    }elseif($gettimelimit == '1'){
	...
	function groupwise_todaycall($gettimelimit){
		$q=''; $p = '';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			$q .= " AND (h.eid='".$this->session->userdata('eid')."' or g.eid='".$this->session->userdata('eid')."')";
		...
		$sql=$this->db->query("SELECT g.gid,g.groupname,COALESCE(h.cnt,0) as count FROM `".$this->session->userdata('bid')."_groups` g
							...
							WHERE h.status!=2 and date(h.starttime)>='$date' ".$q." 
	...
	function callbytime($type,$gettimelimit){
		$q='';
		$c1='';
		$p1='';
		$iv1='';
		...
		$q .= ($type=='a')?"":"";
		$q .= ($type=='m')?" AND c.pulse=0":"";
		$q .= ($type=='q')?" AND (c.callername!='' OR c.callerbusiness!='' OR c.calleraddress!='' OR c.caller_email!='' OR c.remark!='')":"";
		$q .= ($type=='u')?" AND (c.callername='' AND c.callerbusiness='' AND c.calleraddress='' AND c.caller_email='' AND c.remark='')":"";
		$q .= ($type=='at')?" AND c.pulse!=0":"";
		$q .= ($type=='p') ? " AND c.emotion ='p' ":"";
		$q .= ($type=='n') ? " AND c.emotion ='n' ":"";
		$res=array();
		$rec=array();
		
     	
		if($gettimelimit == '3'){
		$q .= " AND date(c.starttime)>='". date('Y-m-d', strtotime('-30 days'))."'"; 
	    }elseif($gettimelimit == '2'){
		$q .= " AND date(c.starttime)>='". date('Y-m-d', strtotime('-6 days'))."'";
	    }elseif($gettimelimit == '1'){
     	$q .= " AND date(c.starttime)>='". date('Y-m-d', strtotime('now'))."'";
	    }else{
     	$q .= (isset($s['stime']) && $s['stime']!='') ? " AND date(c.starttime)>='".$s['stime']."'" : " AND date(c.starttime)>='".date('Y-m-d',strtotime('-6 days'))."'";
	    }
		
		
		$q .= (isset($s['etime']) && $s['etime']!='')? " AND date(c.starttime)<='".$s['etime']."'":'';
		
		$q .= (isset($se['gid']) && $se['gid']!='' && $se['gid']!='0')? " AND c.gid='".$se['gid']."'":'';
		$q .= (isset($se['callfrom']) && $se['callfrom']!='')? " AND c.callfrom='".$se['callfrom']."'":'';
		$q.=(isset($se['empid']) && $se['empid']!='0')?" AND c.eid = '".$se['empid']."'":"";
		$q.=(isset($se['starttime']) && $se['starttime']!='') ? " AND date(c.starttime)>= '".$se['starttime']."'":"";
		$q.=(isset($se['endtime']) && $se['endtime']!='')?" AND date(c.endtime)<= '".$se['endtime']."'":"";
		$q.=(isset($se['callername']) && $se['callername']!='')?" AND c.callername like '%".$se['callername']."%'":"";
		$q.=(isset($se['caller_email']) && $se['caller_email']!='')?" AND c.caller_email like '%".$se['caller_email']."%'":"";
		$q.=(isset($se['pulse']) && $se['pulse']!='')?" AND if(c.pulse>0,ceil(c.pulse/60),c.pulse) ".$se['ptype']." '".$se['pulse']."'":"";	
		$q.=(isset($se['dialstatus']) && $se['dialstatus']!='')?" and c.dialstatus like '%".$se['dialstatus']."%'":"";
		$q.=(isset($se['calid']) && $se['calid']!='')?" and c.calid = '".$se['calid']."'":"";
		$q.=(isset($se['refid']) && $se['refid']!='')?" and c.refid = '".$se['refid']."'":"";
		$q.=(isset($se['source']) && $se['source']!='')?" and c.source like '%".$se['source']."%'":"";
		$q.=(isset($se['keyword']) && $se['keyword']!='')?" and c.keyword like '%".$se['keyword']."%'":"";
		$q.=(isset($se['keyword1']) && $se['keyword1']!='')?" and c.keyword1 like '%".$se['keyword1']."%'":"";
		$q.=(isset($se['keyword2']) && $se['keyword2']!='')?" and c.keyword2 like '%".$se['keyword2']."%'":"";
		$q.=(isset($se['last_modified']) && $se['last_modified']!='')?" and c.last_modified like '%".$s['last_modified']."%'":"";
		$q.=(isset($se['assignto']) && $se['assignto']!='')?" and k.empname like '%".$se['assignto']."%'":"";
		
		$sql=$this->db->query("SELECT c.gid,hour(c.starttime) as hor,count(*) as cnt,g.groupname as name
							FROM ".$this->session->userdata('bid')."_callhistory c
							LEFT JOIN ".$this->session->userdata('bid')."_groups g on c.gid=g.gid
							LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = c.eid 
							LEFT JOIN ".$this->session->userdata('bid')."_employee k ON k.eid = c.assignto 
							LEFT JOIN series_list1 s on 
							(substr(c.callfrom,1,4)=s.scode
							OR substr(c.callfrom,1,3)=s.scode
							OR substr(c.callfrom,1,2)=s.scode)
							WHERE 1 AND c.status!=2  $q 
			...
			foreach($rec as $r){
				if($gid != $r['gid']){
					$gid = $r['gid'];
					$res[$gid] = array('name'=>$r['name']);
				}
				$res[$gid][$r['hor']] = $r['cnt'];
				//$res[$r['gid']] = array($r['hor']=>$r['cnt'],'name'=>$r['name']);
			}
		}	  
	    $result = array();
		foreach($res as $key){
	...
	function callbyweek($type,$gettimelimit){
		$q='';
		$c1='';
	//	print_r($gettimelimit);
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			$q .= " AND (c.eid='".$this->session->userdata('eid')."' or g.eid='".$this->session->userdata('eid')."')";
		}
		...
		$sql=$this->db->query("SELECT c.gid,DAYNAME(c.starttime) as hor,count(*) as cnt,g.groupname
							...
							OR substr(c.callfrom,1,3)=s.scode
							OR substr(c.callfrom,1,2)=s.scode)
							WHERE c.status!=2 AND date(c.starttime)>='".$date."' $c1 $q
		...
		foreach($res as $key){
			   ...
			   if (in_array('name', $keys)){
				...
				for($i = 0; $i <= 6; $i++){
					if (in_array($week[$i], $keys)){
					 $re[$week[$i]] = $key[$week[$i]];
					}else{
					 $re[$week[$i]] = 0;
					}
               }
	...
	function callbyregion($type,$gettimelimit){
		$q='';
		$c1='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
		...
		$query = "SELECT s.area,count(*) as cnt
				FROM ".$this->session->userdata('bid')."_callhistory c
			    LEFT JOIN ".$this->session->userdata('bid')."_groups g on c.gid=g.gid
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = c.eid 
				LEFT JOIN ".$this->session->userdata('bid')."_employee k ON k.eid = c.assignto 
	...
	function recent_calls($type,$gettimelimit){
		$res=array();
		$ret=array();
		if($this->session->userdata('search')){
			$se = $this->session->userdata('search');
		...
		$q .= (isset($se['gid']) && $se['gid']!='' && $se['gid']!='0')? " AND h.gid='".$se['gid']."'":'';
		$q .= (isset($se['callfrom']) && $se['callfrom']!='')? " AND h.callfrom='".$se['callfrom']."'":'';
		$q.=(isset($se['empid']) && $se['empid']!='0')?" AND h.eid = '".$se['empid']."'":"";
		$q.=(isset($se['starttime']) && $se['starttime']!='') ? " AND date(h.starttime)>= '".$se['starttime']."'":"";
		$q.=(isset($se['endtime']) && $se['endtime']!='')?" AND date(h.endtime)<= '".$se['endtime']."'":"";
		$q.=(isset($se['callername']) && $se['callername']!='')?" AND h.callername like '%".$se['callername']."%'":"";
		$q.=(isset($se['caller_email']) && $se['caller_email']!='')?" AND h.caller_email like '%".$se['caller_email']."%'":"";
		$q.=(isset($se['pulse']) && $se['pulse']!='')?" AND if(h.pulse>0,ceil(h.pulse/60),h.pulse) ".$se['ptype']." '".$se['pulse']."'":"";	
		$q.=(isset($se['dialstatus']) && $se['dialstatus']!='')?" and h.dialstatus like '%".$se['dialstatus']."%'":"";
		$q.=(isset($se['calid']) && $se['calid']!='')?" and h.calid = '".$se['calid']."'":"";
		$q.=(isset($se['refid']) && $se['refid']!='')?" and h.refid = '".$se['refid']."'":"";
		$q.=(isset($se['source']) && $se['source']!='')?" and h.source like '%".$se['source']."%'":"";
		$q.=(isset($se['keyword']) && $se['keyword']!='')?" and h.keyword like '%".$se['keyword']."%'":"";
		$q.=(isset($se['keyword1']) && $se['keyword1']!='')?" and h.keyword1 like '%".$se['keyword1']."%'":"";
		$q.=(isset($se['keyword2']) && $se['keyword2']!='')?" and h.keyword2 like '%".$se['keyword2']."%'":"";
		$q.=(isset($se['last_modified']) && $se['last_modified']!='')?" and h.last_modified like '%".$s['last_modified']."%'":"";
		$q.=(isset($se['assignto']) && $se['assignto']!='')?" and k.empname like '%".$se['assignto']."%'":"";
		
		$sql = $this->db->query("SELECT count(h.callid) as count,h.gid,g.groupname as gname 
									FROM ".$this->session->userdata('bid')."_callhistory h 
									LEFT JOIN ".$this->session->userdata('bid')."_groups g on h.gid=g.gid
									LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = h.eid 
									LEFT JOIN ".$this->session->userdata('bid')."_employee k ON k.eid = h.assignto 
									WHERE h.status!=2 AND date(h.starttime)>='$date' $q GROUP BY h.gid");
	...
	function empgroupwise_todaycalls($gettimelimit){
		$res=array();
		$q='';
		$date = '';
		if($gettimelimit == '3'){
		...
		$sql = $this->db->query("SELECT a.eid,a.empname,count(a.tcall) as ucount,sum(a.tcall) as count FROM 
				(SELECT count(c.callid) as tcall,e.empname,e.eid FROM ".$this->session->userdata('bid')."_callhistory c
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON c.eid=e.eid
				WHERE e.status=1 AND c.status=1 $q
				and date(c.starttime)>='$date'
				AND e.eid IS NOT NULL 
				GROUP BY c.eid,c.callfrom) a
				GROUP BY a.eid");
	...
	function call_types($type,$c='0',$gettimelimit){
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
		...
		$q.=($type=='q')?" AND (a.callername!='' OR a.callerbusiness!='' OR a.calleraddress!='' OR a.caller_email!='' OR a.remark!='')":"";
		$q.=($type=='u')?" AND (a.callername='' AND a.callerbusiness='' AND a.calleraddress='' AND a.caller_email='' AND a.remark='')":"";
		//~ $q.=($type=='p') ? " AND (a.emotion = 'p' OR a.emotion ='P')":"";
		//~ $q.=($type=='n') ? " AND (a.emotion = 'n' OR a.emotion ='N')":"";
		$q.=($type=='at')?" AND a.pulse!=0":"";
		$q.= " AND date(a.starttime)>='".$date."' ";
		$type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$this->session->userdata('bid')."'")->row()->lead_generate;
		if($type == '0'){
	...
	function call_last_calls($gettimelimit){
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
		...
		$res1=array();
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$query = "SELECT c.callfrom as callfrom,COALESCE(g.groupname,0) as groupname,COALESCE(e.empname,0) as empname,c.starttime as starttime 
				FROM `".$this->session->userdata('bid')."_callhistory` c
				LEFT JOIN ".$this->session->userdata('bid')."_groups g on c.gid=g.gid
				LEFT JOIN ".$this->session->userdata('bid')."_employee e on c.eid=e.eid
				WHERE c.status!=2 AND date(c.starttime)>='$date'";
		if($roleDetail['role']['admin']!=1){
			$query .= " AND (c.eid='".$this->session->userdata('eid')."' or g.eid='".$this->session->userdata('eid')."')";
		}		
		$type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$this->session->userdata('bid')."'")->row()->lead_generate;
	...
	function for_pieinfo($type,$gettimelimit)
	{
		$res=array();
		$date = '';
		if($gettimelimit == '3'){
		...
		$q.=($type=='u')?" AND (a.callername='' AND a.callerbusiness='' AND a.calleraddress='' AND a.caller_email='' AND a.remark='')":'';
		$q.=($type=='at')?" AND a.pulse!=0":"";
		$q .= (isset($se['gid']) && $se['gid']!='')? " AND a.gid='".$se['gid']."'":'';
		$q .= (isset($se['callfrom']) && $se['callfrom']!='')? " AND a.callfrom='".$se['callfrom']."'":'';
		$sql=$this->db->query("select count(a.callid) as cnt,d.groupname as gname	 
					from ".$this->session->userdata('bid')."_callhistory a
...
//~ 
	...
	function listcontacts($ofset='0',$limit='20'){
		...
		$q.=(isset($s['remarks']) && $s['remarks']!='')?" and remarks like '%".$s['remarks']."%'":"";
	...
	function call_followUps($gettimelimit){
		$d = '';
		$date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('+30 days')); 
	    ...
	    $d.= " AND date(f.followupdate) >= CURRENT_DATE() AND date(f.followupdate) <='".$date."' ";
		$res=array();
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$bid = $this->session->userdata('bid');
		$eid = $this->session->userdata('eid');
	...
	function getClickToCalls($bid,$ofset='0',$limit='20'){
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS *  FROM ".$bid."_c2c a 
				...
				WHERE a.status = 1 $q limit $ofset,$limit ";
		$rst = $this->db->query($sql)->result_array();
		$rst1 = $this->db->query("SELECT FOUND_ROWS() as cnt");
		$ret['count'] = $rst1->row()->cnt;
		foreach($roleDetail['modules'] as $mod){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_add) ? '<li><a href="Report/c2cedit/'.$rec['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/c2c_delete/'.$rec['callid'].'" class="deleteClass" title="Are you sure to Delete the group"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= '<li><a href="Report/c2cActive/'.$rec['callid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-user"  title="List Employee Group">&nbsp;Details</span></a></li>';
				$act .= ($roleDetail['role']['accessrecords']=='0') ? (($rec['filename']!='' && file_exists('sounds/'.$rec['filename']))
					?'<li><a target="_blank" href="'.site_url('sounds/'.$rec['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">Recording</span></a></li>'
					:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li> '):"";
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
  ...
  function clicktoconnectedit($id,$bid){
		$itemDetail = $this->configmodel->getDetail('58',$id,'',$bid);
		$rate=0;
		$val='';
		$return = '';
	    $arr=array_keys($_POST);
		for($i=0;$i<sizeof($arr);$i++){
			if(! @in_array($arr[$i],array("update_system","custom","convertlead","assignto","lgid","lassignto","lalerttype","updatelead","number"))){
				/* Changed for custom fields */
				$val = (is_array($_POST[$arr[$i]])) ? @implode(',',$_POST[$arr[$i]]) : $_POST[$arr[$i]];
				$this->db->set($arr[$i],$val);
				/* Changed for custom fields end */
		...
		$this->db->set('last_modified',date("Y-m-d H:i:s"));
		$this->db->where('callid',$id);
		$this->db->update($bid.'_outboundcalls'); 
		$this->auditlog->auditlog_info('Report',$id. "Click2Connect updated by ".$this->session->userdata('username'));
		if($this->input->post('convertlead') || $this->input->post('updatelead')){
			$source = array("type"=>"click2connect","id"=>$id,"bid"=>$bid,"keyword"=>$itemDetail['keyword']);
			$res = $this->configmodel->callconvert($source);
		}
		$sql = "SELECT * FROM ".$bid."_customfields WHERE modid= 58 AND fieldtype in ('checkbox','dropdown','radio')";
		$rst = $this->db->query($sql)->result_array();
		foreach($rst as $rec){
			if(@in_array($rec['field_key'],$arr)){
				$itemDetailNew = $this->configmodel->getDetail('58',$id,'',$bid);
				$fieldset = $this->configmodel->getFields('58',$bid);
				$data = array();
				foreach($fieldset as $field){
					if($field['type']=='s' && !$field['is_hidden']){
						$data[$field['customlabel']]=$itemDetailNew[$field['fieldname']];
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'Report/UnDelC2C/'.$r['callid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;Undelete</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
	...
	function getDetail($bid,$dataid,$module,$eid){
		$ret = '0';
	 ...
	 function outbound_calls_list($bid,$ofset='0',$limit='20'){
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS a.* FROM(SELECT o.* FROM ".$bid."_outboundcalls o $q $q1
				 ...
				 UNION
				 SELECT p1.* FROM ".$bid."_outboundcalls p1 WHERE p1.eid ='".$this->session->userdata('eid')."'
				 ) a ORDER BY starttime DESC LIMIT $ofset,$limit";
	    $rst = $this->db->query($sql)->result_array();
		$rst1 = $this->db->query("SELECT FOUND_ROWS() as cnt");
		$ret['count'] = $rst1->row()->cnt;
		$opt_add 	= $roleDetail['modules']['58']['opt_add'];
		$opt_view 	= $roleDetail['modules']['58']['opt_view'];
		$opt_delete = $roleDetail['modules']['58']['opt_delete'];
		$fieldset = $this->configmodel->getFields('58',$bid);
		$keys = array();
		$header = array();
	    $header = array("<a href='javascript://'><span id='c_all' class='glyphicon glyphicon-gok'></span></a>");
		foreach($fieldset as $field){
			...
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				foreach($roleDetail['system'] as $f){if($f['fieldid']==$field['fieldid'])$checked = true;}
				if($checked && !in_array($field['fieldname'],array('filename','callid'))){
					array_push($keys,$field['fieldname']);
		...
		foreach($rst as $rec){
			$data['sl'] = $i;
			$r = $this->configmodel->getDetail('58',$rec['callid'],'',$bid);
			$data = array($i);
			$v = '<input type="checkbox" class="blk_check" name="blk[]" value="'.$rec['callid'].'"/>';
			array_push($data,$v);	
			foreach($keys as $k){
				if($k == 'modid'){
				$v = isset($source_arr[$r[$k]])?$source_arr[$r[$k]]:'';
				}elseif($k == 'status'){
				$v = isset($status[$r[$k]])?$status[$r[$k]]:'';
				}else{
				$v = isset($r[$k])?nl2br(wordwrap($r[$k],80,"\n")):"";
			     }
			array_push($data,$v);
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
						<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
						<ul class="dropdown-menu" style="text-align:left;">';
			$act .= '<li><a href="EditC2CReport/'.$rec['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>';
			$act .= '<li><a href="'.base_url().'mtracker/mtDelete/'.$rec['callid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>';
			$act .= ($roleDetail['role']['accessrecords']=='0') ? (($rec['filename']!='' && file_exists('sounds/'.$rec['filename']))
					?'<li><a target="_blank" href="'.site_url('sounds/'.$rec['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">Recording</span></a></li>'
					:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li> '):"";		
			$act .= '</ul></div>';
	...
	function getfollowups($ofset,$limit,$bid){
		...
		if(!(isset($s['followupdate1']) && isset($s['followupdate2']))){
			$q .= " AND DATE(f.followupdate) >= CURRENT_DATE() ";
		}
		if(isset($s)){
            $arr = array_keys($s);
            for ($n =0;$n<count($arr);$n++){
		...
		$q .= (isset($s['followupdate1']) && $s['followupdate1']!='')?" AND DATE(f.followupdate) >= '".$s['followupdate1']."'":"";
		$q .= (isset($s['followupdate2']) && $s['followupdate2']!='')?" AND DATE(f.followupdate) <= '".$s['followupdate2']."'":"";
		$q .= (isset($s['followupdate3']) && $s['followupdate3']!='')?" AND DATE(f.followupdate) = DATE('".$s['followupdate3']."')":"";
		$q .= (isset($s['src1']) && $s['src1']!='')?" AND f.type = '".$s['src1']."'":"";
		$sql="SELECT SQL_CALC_FOUND_ROWS f.callid as detId,f.eid,e.empname,f.comment,f.cdate as date,
				   ...
				   LEFT JOIN ".$bid."_employee e ON e.eid=f.eid 
				   WHERE 1 $q ".$con." ORDER BY f.followupdate DESC LIMIT $ofset,$limit";
			...
			foreach($keys as $k){	
				...
				elseif($k == 'source'){
					...
					case 'support':
						 ...
						 break;
					case 'mtracker':
						 $sql = $this->db->query("SELECT m.name AS callername,m.callto as callfrom FROM ".$bid."_mtracker m WHERE callid='".$rec['detId']."'")->result_array();
						 if(count($sql) > 0){
							 //$link="support/activeSupportTkt/".$r['detId']."/1";
							 $link = '#';
							 $links = ($sql[0]['callfrom'] != '') ? "<a class='btn-danger' data-toggle='modal' data-target='#modal-responsive' href='".$link."'>".$sql[0]['callfrom']."</a>" : ' ';
...
function followupdownload($bid){
	...
	function sendF($call_id,$bid,$mod,$type){
		...
		$senderid = $this->configmodel->busSenderId($bid);
		if($type == 'calltrack'){
		...
		if($type == "ivrs"){
			...
			$arr=array(
			 ...
			 $arr=array(
				...
				"Ticket Raised On" => "Tkt raised",
				"Ticket Updated On" => "Upd",
				"Ticket Escalation Time" => "EscTm",
				"Ticket Level" => "Tkt Lvel",
				"Ticket Status" => "Tkt Sts",
				"Assign To" => "AsgnTo",
		...
		switch($case){
			case '1':
				if($_POST['smsBal']>0){
					$reply=sms_send($empDetail['empnumber'],$message,$senderid,'sendsms');
	...
	function getBreakTimings($bid,$ofset,$limit){
		...
		$q = '';
		$q.=(isset($s['eid']) && $s['eid']!='' && $s['eid']!='0')?" AND eb.eid = '".$s['eid']."'":"";
	...
	function ivrs_last_calls($gettimelimit){
		$res=array();
		$q='';
		$date ='';
	    if($gettimelimit == '3'){
		...
		$query="SELECT i.callfrom AS callfrom,COALESCE(iv.title,0) as groupname,COALESCE(e.empname,0) as empname 
				FROM `".$this->session->userdata('bid')."_ivrshistory` i
				LEFT JOIN `".$this->session->userdata('bid')."_ivrs` iv ON i.ivrsid = iv.ivrsid AND iv.bid =1
				LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON i.eid = e.eid
	 ...
	 * 
	 */
	function ivrs_total_calls($gettimelimit){
		$q='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			$q .= " AND e.eid=".$this->session->userdata('eid')." ";
		...
		$sl=$this->db->query("SELECT count( iv.ivrsid ) AS count FROM ".$this->session->userdata('bid')."_ivrshistory iv 
							  LEFT JOIN ".$this->session->userdata('bid')."_employee e on e.eid=iv.eid 
	 ...
	 * 
	 */
	function ivrs_groupwise_todaycall($gettimelimit){
		$q='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			$q .= " AND e.eid=".$this->session->userdata('eid')." ";
		...
		$sql=$this->db->query("SELECT count( i.ivrsid ) as count ,iv.title as title FROM ".$this->session->userdata('bid')."_ivrshistory i
							 ...
							 LEFT JOIN ".$this->session->userdata('bid')."_employee e on e.eid=i.eid 
							 WHERE i.status !=2
							 AND 
							 date( i.datetime ) >= '$date' $q
							 GROUP BY i.ivrsid");
	 ...
	 * 
	 */
	function ivrs_followUps($gettimelimit){
		$res=array();
	    $date = '';
	    $d = '';
		if($gettimelimit == '3'){
	 ...
	 * 
	 */
	function ivrs_callbytime($gettimelimit){
		$q='';
		$c1='';
		$p1='';
		$iv1='';
		...
		if($this->session->userdata('filter')){
			$se = $this->session->userdata('filter');
		}
        $con = "";
	    if($this->session->userdata('search')){
			$s = $this->session->userdata('search');
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $con.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND i.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
        }
		$con .= (isset($s['ivrstitle']) && $s['ivrstitle']!='')?" AND iv.title like '%".$s['ivrstitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND i.callfrom like '%".$s['callfrom']."%'":"";
		$con .= (isset($s['name']) && $s['name']!='')?" AND i.name like '%".$s['name']."%'":"";
		$con .= (isset($s['email']) && $s['email']!='')?" AND i.email like  '%".$s['email']."%'":"";
		$con .= (isset($s['datetime']) && $s['datetime']!='')?" and date(i.datetime)>= '".$s['datetime']."'":"";
		$con .= (isset($s['endtime']) && $s['endtime']!='')?" and date(i.endtime)<= '".$s['endtime']."'":"";
		$res=array();
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
		...
		$date=(isset($se['stime']) && $se['stime']!='')?$se['stime']:date('Y-m-d',strtotime('-6 days'));
	    }
		$iv1=(isset($se['etime']) && $se['etime']!='')?" AND date(i.datetime)<='".$se['etime']."'":'';
		$sql=$this->db->query("select i.ivrsid as gid,hour(i.datetime) as hor ,count(*) as cnt,iv.title as name
							FROM ".$this->session->userdata('bid')."_ivrshistory i
							LEFT JOIN ".$this->session->userdata('bid')."_ivrs iv on i.ivrsid=iv.ivrsid
							LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = i.eid
							WHERE date(i.datetime)>='".$date."' $q $iv1 $con
	 ...
	 * 
	 */
	function ivrs_callbyweek($gettimelimit){
		$q='';
		$c1='';
		...
		$iv1=(isset($se['etime']) && $se['etime']!='')?" AND date(i.datetime)<='".$se['etime']."'":'';
		$sql=$this->db->query("select i.ivrsid as gid,DAYNAME(i.datetime) as hor ,count(*) as cnt,iv.title as name
							FROM ".$this->session->userdata('bid')."_ivrshistory i
							LEFT JOIN ".$this->session->userdata('bid')."_ivrs iv on i.ivrsid=iv.ivrsid
							LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = i.eid
							WHERE date(i.datetime)>='".$date."' $q $iv1 $con
	 ...
	 * 
	 */
	function ivrs_recent_calls($gettimelimit){
		$res=array();
		$ret=array();
		$date = '';
		if($gettimelimit == '3'){
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $con.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND iv.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
        }
		$con .= (isset($s['ivrstitle']) && $s['ivrstitle']!='')?" AND i.title like '%".$s['ivrstitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND iv.callfrom like '%".$s['callfrom']."%'":"";
		$con .= (isset($s['name']) && $s['name']!='')?" AND iv.name like '%".$s['name']."%'":"";
		$con .= (isset($s['email']) && $s['email']!='')?" AND iv.email like  '%".$s['email']."%'":"";
		$con .= (isset($s['datetime']) && $s['datetime']!='')?" and date(iv.datetime)>= '".$s['datetime']."'":"";
		$con .= (isset($s['endtime']) && $s['endtime']!='')?" and date(iv.endtime)<= '".$s['endtime']."'":"";
		$sql=$this->db->query("SELECT i.title AS groupname, coalesce( iv.cnt, 0 ) AS count
							   FROM ".$this->session->userdata('bid')."_ivrs i 
							   LEFT JOIN (SELECT count( iv.ivrsid ) AS cnt, iv.ivrsid  FROM ".$this->session->userdata('bid')."_ivrshistory iv 
							   LEFT JOIN ".$this->session->userdata('bid')."_ivrs i ON iv.ivrsid = i.ivrsid
							   LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = iv.eid
							   WHERE date( iv.datetime ) >= '".$date."' $q $con GROUP BY iv.ivrsid)iv ON i.ivrsid = iv.ivrsid WHERE 1  ");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
			foreach($res as $rec){
				if($rec['count']!='0'){
	...
	function ivrs_groupnames(){
		...
		$con .= (isset($s['ivrstitle']) && $s['ivrstitle']!='')?" AND i.title like '%".$s['ivrstitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND callfrom like '%".$s['callfrom']."%'":"";
		...
		$con .= (isset($s['endtime']) && $s['endtime']!='')?" and date(endtime)<= '".$s['endtime']."'":"";
		$sql=$this->db->query("SELECT i.title as groupname,i.ivrsid as gid,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 0 DAY)) ".$con.") as day0,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY)) ".$con.") as day1,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 2 DAY)) ".$con.") as day2,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 3 DAY)) ".$con.") as day3,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 4 DAY)) ".$con.") as day4,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 5 DAY)) ".$con.") as day5,
				(SELECT count(hid) FROM ".$this->session->userdata('bid')."_ivrshistory 
					WHERE ivrsid=i.ivrsid AND date(datetime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 6 DAY)) ".$con.") as day6
		from ".$this->session->userdata('bid')."_ivrs i where i.bid=".$this->session->userdata('bid'));
		if($sql->num_rows()>0){
			$res=$sql->result_array();
			foreach($res as $rec){
				if($rec['day0']!='0' || $rec['day1']!='0' ||
	...
	function pbx_last_calls($gettimelimit)
	{
		$res=array();
		$res1=array();
		$date = '';
		...
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$query ="SELECT pr.callfrom as callfrom,COALESCE(p.title,0) as groupname,'0' as empname,pr.starttime as starttime 
				FROM `".$this->session->userdata('bid')."_pbxreport` pr
				LEFT JOIN ".$this->session->userdata('bid')."_pbx p on p.pbxid=pr.pbxid
				WHERE p.status !=2 AND date(pr.starttime)>='$date'";
		$type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$this->session->userdata('bid')."'")->row()->lead_generate;
		if($type == '0'){
			$query .= " AND c.pulse > '0'";
		}
	 ...
	 * 
	 */
	function pbx_total_calls($gettimelimit){
		$q='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			//$q .= " AND (c.eid='".$this->session->userdata('eid')."' or g.eid='".$this->session->userdata('eid')."')";
	 ...
	 * 
	 */
	function pbx_groupwise_todaycall($gettimelimit){
		$q=''; $p = '';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
			//$q .= " AND (h.eid='".$this->session->userdata('eid')."' or g.eid='".$this->session->userdata('eid')."')";
	 ...
	 * 
	 */
	function pbx_followUps($gettimelimit){
		$res=array();
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$bid = $this->session->userdata('bid');
		$eid = $this->session->userdata('eid');
		...
		LEFT JOIN ".$bid."_pbxreport p on f.callid =p.callid
		LEFT JOIN ".$bid."_pbx px on p.pbxid=px.pbxid
		LEFT JOIN ".$bid."_employee e on e.eid=f.eid WHERE $where3 f.type='pbx' $d";
		$sql=$this->db->query($q);
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
	 ...
	 * 
	 */
	function pbx_callbyweek($gettimelimit){
		$q='';
		$c1='';
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $con.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND p.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
        }
		$con .= (isset($s['pbxtitle']) && $s['pbxtitle']!='')?" AND pp.title like '%".$s['pbxtitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND p.callfrom like '%".$s['callfrom']."%'":"";
		$con .=(isset($s['pulse']) && $s['pulse']!='')?" and if(p.pulse>0,ceil(p.pulse/60),p.pulse) ".$s['ptype']." '".$s['pulse']."'":"";	
		$con .= (isset($s['name']) && $s['name']!='')?" AND p.name like '%".$s['name']."%'":"";
		$con .= (isset($s['email']) && $s['email']!='')?" AND p.email like '%".$s['email']."%'":"";
		$con .=(isset($s['starttime']) && $s['starttime']!='')?" and date(p.starttime)>= '".$s['starttime']."'":"";
		$con .=(isset($s['endtime']) && $s['endtime']!='')?" and date(p.endtime)<= '".$s['endtime']."'":"";
		$res=array();
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
		...
		$p1=(isset($se['etime']) && $se['etime']!='')?" AND date(p.starttime)<='".$se['etime']."'":'';
		$sql=$this->db->query("SELECT p.pbxid as gid ,DAYNAME(p.starttime) as hor,count(*) as cnt ,pp.title as name
							FROM ".$this->session->userdata('bid')."_pbxreport p
							LEFT JOIN ".$this->session->userdata('bid')."_pbx pp on p.pbxid=pp.pbxid
							LEFT JOIN prinumber pr on pp.prinumber=pr.number
							WHERE pp.status!=2 AND date(p.starttime)>='".$date."' $p1 $con
	 ...
	 * 
	 */
	function pbx_callbytime($gettimelimit){
		$q='';
		$c1='';
		$p1='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
	  ...
	  if(isset($_POST['submit'])){
		...
		$p1=(isset($se['etime']) && $se['etime']!='')?" AND date(p.starttime)<='".$se['etime']."'":'';
		$sql=$this->db->query("
							SELECT p.pbxid as gid ,hour(p.starttime) as hor,count(*) as cnt ,pp.title as name
							FROM ".$this->session->userdata('bid')."_pbxreport p
							LEFT JOIN ".$this->session->userdata('bid')."_pbx pp on p.pbxid=pp.pbxid
							WHERE pp.status!=2 and date(p.starttime)>='".$date."' $p1 $con
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $con.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND ".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
        }
		$con .= (isset($s['pbxtitle']) && $s['pbxtitle']!='')?" AND c.title like '%".$s['pbxtitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND callfrom like '%".$s['callfrom']."%'":"";
		$con .=(isset($s['pulse']) && $s['pulse']!='')?" and if(pulse>0,ceil(pulse/60),pulse) ".$s['ptype']." '".$s['pulse']."'":"";	
		$con .= (isset($s['name']) && $s['name']!='')?" AND name like '%".$s['name']."%'":"";
		$con .= (isset($s['email']) && $s['email']!='')?" AND email like '%".$s['email']."%'":"";
		$con .=(isset($s['starttime']) && $s['starttime']!='')?" and date(starttime)>= '".$s['starttime']."'":"";
		$con .=(isset($s['endtime']) && $s['endtime']!='')?" and date(endtime)<= '".$s['endtime']."'":"";
		$sql=$this->db->query("
				SELECT c.pbxid as gid,c.title as groupname,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 0 DAY)) ".$con.") as day0,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY)) ".$con.") as day1,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 2 DAY)) ".$con.") as day2,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 3 DAY)) ".$con.") as day3,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 4 DAY)) ".$con.") as day4,	
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 5 DAY)) ".$con.") as day5,
				(SELECT count(callid) FROM ".$this->session->userdata('bid')."_pbxreport 
					WHERE pbxid=c.pbxid AND date(starttime)=(DATE_SUB(CURRENT_DATE(),INTERVAL 6 DAY)) ".$con.") as day6
	 ...
	 * 
	 */
	function pbx_recent_calls($gettimelimit){
		$res=array();
		$ret=array();
		$date = '';
		if($gettimelimit == '3'){
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $con.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND pr.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
        }
		$con .= (isset($s['pbxtitle']) && $s['pbxtitle']!='')?" AND p.title like '%".$s['pbxtitle']."%'":"";
		$con .= (isset($s['callfrom']) && $s['callfrom']!='')?" AND pr.callfrom like '%".$s['callfrom']."%'":"";
		$con .=(isset($s['pulse']) && $s['pulse']!='')?" and if(pr.pulse>0,ceil(pr.pulse/60),pr.pulse) ".$s['ptype']." '".$s['pulse']."'":"";	
		$con .= (isset($s['name']) && $s['name']!='')?" AND pr.name like '%".$s['name']."%'":"";
		$con .= (isset($s['email']) && $s['email']!='')?" AND pr.email like '%".$s['email']."%'":"";
		$con .=(isset($s['starttime']) && $s['starttime']!='')?" and date(pr.starttime)>= '".$s['starttime']."'":"";
		$con .=(isset($s['endtime']) && $s['endtime']!='')?" and date(pr.endtime)<= '".$s['endtime']."'":"";
		$sql=$this->db->query("
							  SELECT p.title AS groupname, COALESCE( pr.cnt, 0 ) AS count
								...
								LEFT JOIN (
									...
									FROM ".$this->session->userdata('bid')."_pbxreport pr
									LEFT JOIN ".$this->session->userdata('bid')."_pbx p ON pr.pbxid = p.pbxid
									WHERE p.status!=2 AND date( pr.starttime ) >= '".$date."' $q $con
								GROUP BY pr.pbxid
								)pr ON p.pbxid = pr.pbxid
							 ");
		if($sql->num_rows()>0){
	  ...
	  /******************function for C2C Dashboard starts *************************/
	 ...
	 * name: c2c_last_calls
	 * @param
	 * @return last 7 days c2c calls report
	 * 
	 */
	...
	function c2c_last_calls($gettimelimit)
	{
		...
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$query ="SELECT pr.customer as callfrom,e.empname,pr.starttime as starttime 
				FROM `".$this->session->userdata('bid')."_outboundcalls` pr
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=pr.eid
				WHERE 1 AND date(pr.starttime)>='".$date."'";
		//~ $type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$this->session->userdata('bid')."'")->row()->lead_generate;
		//~ if($type == '0'){
			//~ $query .= " AND c.pulse > '0'";
		//~ }
	 ...
	 * 
	 */
	function c2c_calltypes($type,$c='0',$gettimelimit){
		$date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
		...
		$group = ($c=='1') ? " GROUP BY a.callfrom,a.gid,a.eid ": "";
		$sql=$this->db->query("SELECT COUNT(a.callid) as cnt
								FROM ".$this->session->userdata('bid')."_outboundcalls a
								WHERE 1 $q");
		return ($sql->num_rows()>0) ? (($c=='1')?$sql->num_rows():$sql->row()->cnt):0;
	}
	
	
	  /******************end of C2C Dashboard *************************/
	 ...
	 * 
	 */
	function lead_empgroupwise_todaycalls($gettimelimit){
		$res=array();
		$q='';
	    $date = '';
		if($gettimelimit == '3'){
	 ...
	 * 
	 */
	function leads_followUps($gettimelimit){
		$res=array();
	    $d = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('+30 days')); 
	    ...
	    $d.= " date(lf.followupdate) >= CURRENT_DATE() AND date(lf.followupdate) <='".$date."' ";
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$bid = $this->session->userdata('bid');
		$eid = $this->session->userdata('eid');
		$where3 = ($roleDetail['role']['admin']!=1)?" AND (lf.eid='".$this->session->userdata('eid')."')" : "";
		$q="SELECT l.name as callername,l.number as callfrom,lf.followupdate as followupdate,lf.callid as callid,
			...
			LEFT JOIN ".$bid."_leads_groups g on l.gid = g.gid 
			LEFT JOIN ".$bid."_employee e on e.eid=l.assignto
			WHERE 1 $where3 AND lf.type = 'leads'";
	 ...
	 * 
	 */
	function leads_assigned_detail($gettimelimit){
		$res=array();
		$date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('+30 days')); 
	 ...
	 * 
	 */
	function lead_groupwiseleads($type,$gettimelimit){
	    $res=array();
		$date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
		...
		if(isset($s)){
			...
			for ($n =0;$n<count($arr);$n++){
				if(strstr($arr[$n],'c_')){
					...
					$q.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND l.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
				}
			}
		}
		$q.=(isset($s['gid']) && $s['gid']!='')?" and l.gid = '".$s['gid']."'":"";
		$q.=(isset($s['assignto']) && $s['assignto']!='')?" and l.assignto = '".$s['assignto']."'":"";
		$q.=(isset($s['enteredby']) && $s['enteredby']!='')?" and l.enteredby = '".$s['enteredby']."'":"";
		$q.=(isset($s['convertedby']) && $s['convertedby']!='')?" and l.convertedby = '".$s['convertedby']."'":"";
		$q.=(isset($s['name']) && $s['name']!='')?" and l.name like '%".$s['name']."%'":"";
		$q.=(isset($s['number']) && $s['number']!='')?" and l.number like '".$s['number']."%'":"";
		$q.=(isset($s['email']) && $s['email']!='')?" and l.email like '%".$s['email']."%'":"";
		$q.=(isset($s['caller_add']) && $s['caller_add']!='')?" and l.caller_add like '%".$s['caller_add']."%'":"";
		$q.=(isset($s['caller_bus']) && $s['caller_bus']!='')?" and l.caller_bus like '%".$s['caller_bus']."%'":"";
		$q.=(isset($s['keyword']) && $s['keyword']!='')?" and l.keyword like '%".$s['keyword']."%'":"";
		$q.=(isset($s['refId']) && $s['refId']!='')?" and l.refId like '%".$s['refId']."%'":"";
		$q.=(isset($s['lead_status']) && $s['lead_status']!='')?" and l.lead_status ='".$s['lead_status']."'":"";
		$q.=(isset($s['leadowner']) && $s['leadowner']!='')?" and l.leadowner ='".$s['leadowner']."'":"";
		if(isset($s['createdon_from']) && $s['createdon_from']!='' && isset($s['createdon_to']) && $s['createdon_to']!=''){
			$q .= " and (date(l.createdon) BETWEEN '".$s['createdon_from']."' AND '".$s['createdon_to']."')";
		}else{
			$q.=(isset($s['createdon_from']) && $s['createdon_from']!='')?" and l.createdon >= '".$s['createdon_from']."'":"";
			$q.=(isset($s['createdon_to']) && $s['createdon_to']!='')?" and l.createdon <= '".$s['createdon_to']."'":"";
		}
		
		if(isset($s['lastmodified_from']) && $s['lastmodified_from']!='' && isset($s['lastmodified_to']) && $s['lastmodified_to']!=''){
			$q .= " and (date(l.lastmodified) BETWEEN '".$s['lastmodified_from']."' AND '".$s['lastmodified_to']."')";
		}else{
			$q.=(isset($s['lastmodified_from']) && $s['lastmodified_from']!='')?" and l.lastmodified >= '".$s['lastmodified_from']."'":"";
			$q.=(isset($s['lastmodified_to']) && $s['lastmodified_to']!='')?" and l.lastmodified <= '".$s['lastmodified_to']."'":"";
		}
		
		if(isset($s['convertedon_from']) && $s['convertedon_from']!='' && isset($s['convertedon_to']) && $s['convertedon_to']!=''){
			$q .= " and (date(l.convertedon) BETWEEN '".$s['convertedon_from']."' AND '".$s['convertedon_to']."')";
		}else{
			$q.=(isset($s['convertedon_from']) && $s['convertedon_from']!='')?" and l.convertedon >= '".$s['convertedon_from']."'":"";
			$q.=(isset($s['convertedon_to']) && $s['convertedon_to']!='')?" and l.convertedon <= '".$s['convertedon_to']."'":"";
		}
		$q.=(isset($s['source']) && $s['source']!='')?" and l.source like '%".$s['source']."%'":"";
		$ge = " ";		
		if($roleDetail['role']['admin']!=1){
			if($roleDetail['role']['roleid']==4){
				$q .= " AND ge.eid is not null ";
				$ge = " LEFT JOIN ".$bid."_leads_grpemp ge ON (lg.gid=ge.gid AND ge.eid= '".$this->session->userdata('eid')."') ";
			}else{
				$q .= " AND (l.enteredby='".$this->session->userdata('eid')."' OR l.convertedby ='".$this->session->userdata('eid')."' OR lg.eid='".$this->session->userdata('eid')."' OR l.assignto='".$this->session->userdata('eid')."')";
			}
		}
		$type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$bid."'")->row()->lead_generate;
		if($type == '0'){
		...
		if($lead_status != 0 && $lead_status != 'all')
			$q .= " AND l.lead_status = '".$lead_status."'" ;
		if($roleDetail['role']['admin']!=1){
	...
	function lead_source($type){
		...
		$sql=$this->db->query("SELECT count( l.gid ) AS count, l.gid, l.source
								FROM `".$this->session->userdata('bid')."_leads` l
								WHERE date(l.createdon ) >= '".$date."' 
								AND l.status !=0 $q
								GROUP BY l.source");
	...
	function lead_bytype($type){
		...
		$sql=$this->db->query("SELECT count( l.leadid ) AS count, l.gid, 
								if(l.lead_status=1,'Prospects',
								if(l.lead_status=2,'Open',
								if(l.lead_status=3,'Close',
								if(l.lead_status=4,'Dead',
								if(l.lead_status=5,'Junk','None'))))) as lead_status
								FROM `".$this->session->userdata('bid')."_leads` l
								LEFT JOIN `".$this->session->userdata('bid')."_leads_groups` lg ON l.gid = lg.gid
								WHERE date(l.createdon ) >= '".$date."' 
								AND lg.status =1
	...
	function lead_employee($type){
		...
		$sql=$this->db->query("SELECT count( l.leadid ) AS count, e.empname, l.assignto
								FROM `".$this->session->userdata('bid')."_leads` l
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON l.assignto = e.eid
								WHERE date(l.createdon ) >= '".$date."' 
								AND e.status =1
	 ...
	 * 
	 */	
	function lead_types($type,$gettimelimit){
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
	 ...
	 * 
	 */
	function supports_assigned_detail($gettimelimit)
	{
		$res=array();
	    $date = '';
		if($gettimelimit == '3'){
	 ...
	 * 
	 */
	function support_followUps($gettimelimit){
		$res=array();
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$bid = $this->session->userdata('bid');
		$eid = $this->session->userdata('eid');
	 ...
	 * 
	 */
	function support_empgroupwise_todaycalls($gettimelimit){
		$res=array();
		$q='';
		$date = '';
		if($gettimelimit == '3'){
	 ...
	 * 
	 */
	function support_groupwisesupports($gettimelimit){
		$res=array();
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
		...
		if(isset($s)){
            ...
            for ($n =0;$n<count($arr);$n++){
                if(strstr($arr[$n],'c_')){
                    ...
                    $q.=(isset($s[$arr[$n]]) && ($s[$arr[$n]]!='')  && ($s[$arr[$n]]!=' ') && ($s[$arr[$n]]!='0')) ? " AND st.".$arr[$n]." LIKE '%".$s[$arr[$n]]."%'":"";
                }
            }
          }
		$q.=(isset($s['ticket_id']) && $s['ticket_id']!='')?" and st.ticket_id = '".$s['ticket_id']."'":"";
		$q.=(isset($s['gid']) && $s['gid']!='')?" and st.gid = '".$s['gid']."'":"";
		$q.=(isset($s['assignto']) && $s['assignto']!='')?" and st.assignto = '".$s['assignto']."'":"";
		$q.=(isset($s['enteredby']) && $s['enteredby']!='')?" and st.enteredby = '".$s['enteredby']."'":"";
		$q.=(isset($s['convertedby']) && $s['convertedby']!='')?" and a.convertedby = '".$s['convertedby']."'":"";
		$q.=(isset($s['name']) && $s['name']!='')?" and st.name like '%".$s['name']."%'":"";
		$q.=(isset($s['number']) && $s['number']!='')?" and st.number ='".$s['number']."'":"";
		$q.=(isset($s['email']) && $s['email']!='')?" and st.email like '%".$s['email']."%'":"";
		$q.=(isset($s['source']) && $s['source']!='')?" and st.source like '%".$s['source']."%'":"";
		$q.=(isset($s['caller_add']) && $s['caller_add']!='')?" and st.caller_add like '%".$s['caller_add']."%'":"";
		$q.=(isset($s['caller_bus']) && $s['caller_bus']!='')?" and st.caller_bus like '%".$s['caller_bus']."%'":"";
		$q.=(isset($s['keyword']) && $s['keyword']!='')?" and st.keyword like '%".$s['keyword']."%'":"";
		$q.=(isset($s['refId']) && $s['refId']!='')?" and st.refId like '%".$s['refId']."%'":"";
		$q.=(isset($s['tkt_status']) && $s['tkt_status']!='0')?" AND st.tkt_status ='".$s['tkt_status']."'":"";
		$q.=(isset($s['tkt_criticality']) && $s['tkt_criticality']!='0')?" AND st.tkt_criticality ='".$s['tkt_criticality']."'":"";
		$q.=(isset($s['tkt_level']) && $s['tkt_level']!='0'&& $s['tkt_level']!='')?" AND st.tkt_level ='".$s['tkt_level']."'":"";
		$q.=(isset($s['tkt_esc_time']) && $s['tkt_esc_time']!='0'&& $s['tkt_esc_time']!='')?" AND st.tkt_esc_time ='".$s['tkt_esc_time']."'":"";
		if(isset($s['createdon_from']) && $s['createdon_from']!='' && isset($s['createdon_to']) && $s['createdon_to']!=''){
			$q .= " and (date(st.createdon) BETWEEN '".$s['createdon_from']."' AND '".$s['createdon_to']."')";
		}else{
		...
		if(isset($s['lastmodified_from']) && $s['lastmodified_from']!='' && isset($s['lastmodified_to']) && $s['lastmodified_to']!=''){
			$q .= " and (date(st.lastmodified) BETWEEN '".$s['lastmodified_from']."' AND '".$s['lastmodified_to']."')";
		}else{
		...
		$sql=$this->db->query("SELECT count( st.gid ) AS count, st.gid, sg.groupname
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								LEFT JOIN `".$this->session->userdata('bid')."_support_groups` sg ON st.gid = sg.gid
								WHERE date( st.createdon ) >='".$date."'
								AND sg.status =1
								AND st.status !=0 AND st.status !=2 $q
								GROUP BY st.gid");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
	...
	if($this->session->userdata('search')){
		...
		$sql=$this->db->query("SELECT count( st.assignto ) AS count, st.assignto, e.empname
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								LEFT JOIN `".$this->session->userdata('bid')."_employee` e ON st.assignto = e.eid
	...
	if($this->session->userdata('search')){
		...
		$sql=$this->db->query("SELECT count( st.tkt_status ) AS count,
		                        if(st.tkt_status=1,'Open',
		                        if(st.tkt_status=2,'Pending',
		                        if(st.tkt_status=3,'Resolved',
								if(st.tkt_status=4,'Closed','None')))) as status
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								WHERE date( st.createdon ) >= date_sub( current_date() , INTERVAL 6
								DAY )
								AND st.status !=0 AND st.status !=2 $q
								GROUP BY st.tkt_status");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
		return $res;
	}
    function support_source(){
		$res=array();
		$date=date('Y-m-d',strtotime('-6 days'));
		$q = "";
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
	if($this->session->userdata('search')){
		...
		$sql=$this->db->query("SELECT count( st.tktid ) AS count, st.source
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								WHERE date( st.createdon ) >= date_sub( current_date() , INTERVAL 6
								DAY )
								AND st.status !=0 AND st.status !=2 $q
	...
	if($this->session->userdata('search')){
		...
		$sql=$this->db->query("SELECT count( st.tktid ) AS count,
		                        if(st.tkt_level=1,'Level 1',
		                        if(st.tkt_level=2,'Level 2',
								if(st.tkt_level=3,'Level 3','None'))) as level
								FROM `".$this->session->userdata('bid')."_support_tickets` st
								WHERE date( st.createdon ) >= date_sub( current_date() , INTERVAL 6
								DAY )
								AND st.status !=0 AND st.status !=2 $q
								GROUP BY st.tkt_level");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
		return $res;
	 ...
	 * 
	 */	
	function support_types($type,$gettimelimit){
	    $date = '';
		if($gettimelimit == '3'){
		$date=date('Y-m-d', strtotime('-30 days')); 
	    }elseif($gettimelimit == '2' || $gettimelimit == '' || $gettimelimit == '0'){
	...
	function blk_del($arr) {
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$arr2 = str_replace('on,','',$arr);

		$sql = $this->db->query("UPDATE ".$bid."_callhistory SET status=2 WHERE callid IN(".$arr2.")");

		if($this->db->affected_rows() > 0) {
			// Check if Delete notification is ON / OFF
			$deleteNotifyStatus = $this->db->query("SELECT del_notify FROM account_settings WHERE bid='".$this->session->userdata('bid')."'")->result_array();
			if($deleteNotifyStatus[0]['del_notify']) {
				$this->sendBulkDeleteEmail($arr);	
			}
			$this->auditlog->auditlog_info('Report'," Calls / $arr / Deleted By ".$this->session->userdata('username'));
			return 1;
		}
		else
			return 0;
	...
	public function sendBulkDeleteEmail($callIdsArray)
    {
    	$callIdArray = explode(",", $callIdsArray);
    	
		$message = "The List of Deleted Call ID's are as follow : ";
		$message .= '<ol>';
		for($i=0; $i<sizeof($callIdArray); $i++) {
			$message .= '<li>'.$callIdArray[$i].'</li>';
		}
		$message .=  '</ol>';
		$message .=  'By '.$this->session->userdata('username');
		
		$body = $this->emailmodel->newEmailBody($message,'');
		$adminResultSet = $this->db->query("SELECT empemail FROM ".$this->session->userdata('bid')."_employee WHERE roleid=1 and status=1")->result_array();
		$to = "";
		foreach($adminResultSet as $adminResult) {
			if(empty($to)) {
				$to = $adminResult['empemail'];
			} else {
				$to = $to.','.$adminResult['empemail'];
			}
		}
		
		$subject = 'MCube Deleted Track Record Details';
		$this->load->library('email');
		$this->email->from('noreply@mcube.com', 'MCube');
		$this->email->to($to);
		$this->email->subject($subject);
	...
	function blk_del_callrep($arr){
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$sql = $this->db->query("UPDATE ".$bid."_callhistory SET status=1 WHERE callid IN(".$arr.")");
		if($this->db->affected_rows() >0){
			$this->auditlog->auditlog_info('Report'," Calls / $arr / UnDeleted By ".$this->session->userdata('username'));
			return 1;
		}
		else
			return 0;	
	...
	function list_outbound($bid,$ofset,$limit){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				?'<li><a target="_blank" href="'.site_url('sounds/'.$r['recordfile']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Recording</span></a></li>'
				:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
	...
	function trackd(){
		...
		$q = '';
		if($roleDetail['role']['admin']!=1){
			$q .= " AND (h.eid='".$this->session->userdata('eid')."' OR d.eid='".$this->session->userdata('eid')."')";
		}
		$sql = $this->db->query("SELECT COALESCE(COUNT(callid), 0) AS day 
							FROM ".$bid."_callhistory h 
							LEFT JOIN ".$bid."_groups d ON d.gid = h.gid 
							WHERE (DATE(h.`starttime`) = DATE(NOW())) $q " ) ;	
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function trackw(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS week 
							FROM ".$bid."_callhistory h 
							LEFT JOIN ".$bid."_groups d ON d.gid = h.gid 
							WHERE (DATE(h.`starttime`)  > DATE(DATE_SUB(NOW(), INTERVAL 1 WEEK))) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->week;
		}else{
	...
	function trackm(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_callhistory h 
							LEFT JOIN ".$bid."_groups d ON d.gid = h.gid 
							WHERE (MONTH(h.`starttime` ) = MONTH(NOW()) ) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function leadd(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_leads a 
							LEFT JOIN ".$bid."_leads_groups d ON d.gid = a.gid 
							WHERE (DATE(a.`createdon`) = DATE(NOW())) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
		}else{
	...
	function leadw(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS week 
							FROM ".$this->session->userdata('bid')."_leads a 
							LEFT JOIN ".$bid."_leads_groups d ON d.gid = a.gid 
							WHERE (a.`createdon`  > DATE_SUB(NOW(), INTERVAL 1 WEEK)) $q ");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->week;
		}else{
	...
	function leadm(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS month 
							FROM ".$this->session->userdata('bid')."_leads a 
							LEFT JOIN ".$bid."_leads_groups d ON d.gid = a.gid 
							WHERE (MONTH(a.`createdon` ) = MONTH(NOW())) $q");				
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->month;
		}else{
	...
	function supportd(){
		...
		$sql=$this->db->query("SELECT COALESCE(COUNT(tktid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_support_tickets a 
							LEFT JOIN ".$bid."_support_groups d ON d.gid = a.gid 
							WHERE (DATE(a.`createdon`) = DATE(NOW())) $q ");			
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
	...
	function mtrackd(){
		$res=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS day 
							FROM ".$this->session->userdata('bid')."_mtracker h 
							WHERE (DATE(h.`starttime`) = DATE(NOW()))");	
		if($sql->num_rows()>0){
			$res = $sql->row();
			return $res->day;
	...
	function emotionCalls($bid){
		...
		$type = $this->db->query("SELECT lead_generate FROM business WHERE bid='".$this->session->userdata('bid')."'")->row()->lead_generate;
		$sql=$this->db->query("SELECT h.emotion,COALESCE(h.callid,0) as count 
							FROM `".$bid."_callhistory` h 
							LEFT JOIN ".$bid."_groups g on g.gid=h.gid
							WHERE date(h.starttime)>='".$date."' $q GROUP BY h.emotion");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
		return $res;
	}
	function arealist($bid,$s){
		$res = array();
		$q = ' 1 ';
		//~ $sql = $this->db->query("SELECT COALESCE(count(h.callid),0) as count,a.place,h.region FROM `".$bid."_callhistory` h 
				//~ LEFT JOIN area a ON a.code = h.region GROUP BY h.region");
		$q .= (isset($s['stime']) && $s['stime']!='') ? " AND date(h.starttime)>='".$s['stime']."'" : " AND  date(h.starttime)>='".date('Y-m-d',strtotime('-6 days'))."'";
		$q .= (isset($s['etime']) && $s['etime']!='')?" AND date(h.starttime)<='".$s['etime']."'":'';
		$sql = $this->db->query("SELECT COALESCE(count(h.callid),0) as count,a.place,IF(rg.area!='',a.code,'NA') AS region 
								 FROM `".$bid."_callhistory` h 
								 LEFT JOIN series_list1 rg ON 
								 (rg.scode = substr(h.callfrom,1,4)
								 OR rg.scode = substr(h.callfrom,1,3)
								 OR rg.scode = substr(h.callfrom,1,2))
				                 LEFT JOIN area a ON a.code = rg.area 
				                 WHERE $q GROUP BY rg.area");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
		$result = array();
		foreach ($res as $rec){
			if($rec['region'] != '' && $rec['region'] != 'NA' && $rec['region'] != 'ZZ'){
				$result[$rec['region']] = array(
										'place' => $rec['place'],
										'count' => $rec['count']
									);
			}
		...
		if(@in_array('KO',$keys)){
			if(isset($result['WB'])){
				$result['WB']['count'] += $result['KO']['count'];
			}else{
				$result['WB']['place'] = "West Bengal";
				$result['WB']['count'] = $result['KO']['count'];
			}
			unset($result['KO']);
		}
		if(@in_array('MU',$keys)){
			if(isset($result['MH'])){
				$result['MH']['count'] += $result['MU']['count'];
			}else{
				$result['MH']['place'] = "Maharashtra";
				$result['MH']['count'] = $result['MU']['count'];
			}
			unset($result['MU']);
		}
		if(@in_array('UE',$keys)){
			if(isset($result['UP'])){
				$result['UP']['count'] += $result['UE']['count'];
			}else{
				$result['UP']['place'] = "Uttar Pradesh";
				$result['UP']['count'] = $result['UE']['count'];
			}
			unset($result['UE']);
		}
		if(@in_array('UW',$keys)){
			if(isset($result['UP'])){
				$result['UP']['count'] += $result['UW']['count'];
			}else{
				$result['UP']['place'] = "Uttar Pradesh";
				$result['UP']['count'] = $result['UW']['count'];
			}
		...
		if(@in_array('CH',$keys)){
			if(isset($result['TN'])){
				$result['TN']['count'] += $result['CH']['count'];
			}else{
				$result['TN']['place'] = "Uttar Pradesh";
				$result['TN']['count'] = $result['CH']['count'];
			}
			unset($result['CH']);
		}
  ...
  function empgroupwise_mtrack($gettimelimit){
		...
		$sql=$this->db->query("SELECT e.eid,e.status,e.empname,count(h.callid) as count
								FROM ".$this->session->userdata('bid')."_mtracker h
								LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=h.eid
								WHERE h.status!=2 and date(h.starttime)>='".$date."' $q GROUP BY h.eid ORDER BY e.empname");
	...
	function mtrack_recent_calls($gettimelimit){
		...
		$res1=array();
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$query ="SELECT mt.callto,mt.name,e.empname,mt.starttime as starttime 
				FROM `".$this->session->userdata('bid')."_mtracker` mt
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=mt.eid
				WHERE 1 AND mt.status!=2 AND date(mt.starttime)>='".$date."' $q";
	...
	function getc2cArchivelist($args){
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$args['bid'];
		$ofset 		= isset($args['ofset']) ? $args['ofset'] : '0';
        ...
        $q=' ';$q1 = '';$q2 = '';$q3 = '';
		if($year!=0 && $month!=0){
	    $q = " AND DATE_FORMAT(o.`starttime`,'%m')='".$month."'";
	    $q .= " AND DATE_FORMAT(o.`starttime`,'%Y')='".$year."'";
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		$limit = ($roleDetail['role']['recordlimit']>($ofset+$limit))?$limit:((($roleDetail['role']['recordlimit'] - $ofset)>0)?($roleDetail['role']['recordlimit'] - $ofset):0);
		if($roleDetail['role']['admin']!=1){
			$q1 .= " AND eid IN(
		...
		$res=array();
		$sql = "SELECT SQL_CALC_FOUND_ROWS a.* FROM(SELECT o.* FROM ".$bid."_ob_archive o $q1 WHERE 1 $q 
				 UNION
				 SELECT o.* FROM ".$bid."_ob_archive o  $q2 WHERE 1 $q 
				 UNION
				 SELECT o.* FROM ".$bid."_ob_archive o  $q3 WHERE 1 $q 
				 ) a  ORDER BY starttime DESC LIMIT $ofset,$limit";
		...
		$res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
			...
			foreach($rst as $rec){
				...
				foreach($keys as $k){
					$v = isset($rec[$k])?$rec[$k]:"";	
					array_push($data,$v);
					$act = "<a href='C2CArchive/".$rec['year']."/".$rec['nmonth']."'>List</a>&nbsp;&nbsp;&nbsp;" .
						  "<a href='Report/c2carchive_download/".$rec['year']."/".$rec['nmonth']."'>Download</a>";
					$data['action'] = $act;
				}
				$i++;
				array_push($list,$data);
	...
	function c2carchiveDownload($args){
	    $roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
	    $bid=(isset($cbid) && $cbid!="")?$cbid:$args['bid'];
		$ofset 		= isset($args['ofset']) ? $args['ofset'] : '0';
		$limit 		= isset($args['limit']) ? $args['limit'] : '20';
	...
	$source = array(
		...
		chmod('reports/'.$name,0777);
		$list = array();
		$data_file = 'reports/'.$name.'/c2carchive.csv';
		$fp = fopen($data_file,'a+');
		$hkey = array('executive',
						'customer',
						'source',
						'starttime',
						'endtime',
	...
	function empwise_missed($gettimelimit){
		$res=array();
		$res1=array();
	    $date = '';
	    $q = '';
		...
		$query ="SELECT e.empname,count(mt.callid) as cnt
				FROM `".$this->session->userdata('bid')."_mtracker` mt
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=mt.eid
				WHERE mt.status!=2 AND mt.calltype = 0 AND date(mt.starttime)>='".$date."' $q GROUP BY mt.eid ORDER BY mt.eid";
		$sql=$this->db->query($query);
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
	...
	function empwise_outbound($gettimelimit){
		$res=array();
	    $date = '';
	   	$q = '';
		if($gettimelimit == '3'){
		...
		$query ="SELECT e.empname,count(mt.callid) as cnt
				FROM `".$this->session->userdata('bid')."_mtracker` mt
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=mt.eid
				WHERE mt.status!=2 AND mt.calltype = 2 AND date(mt.starttime)>='".$date."' $q GROUP BY mt.eid ORDER BY mt.eid";
		$sql=$this->db->query($query);
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
	...
	function empwise_inbound($gettimelimit){
		$res=array();
		$res1=array();
		$date = '';
		$q = '';
		...
		$query ="SELECT e.empname,count(mt.callid) as cnt
				FROM `".$this->session->userdata('bid')."_mtracker` mt
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=mt.eid
				WHERE mt.status!=2 AND mt.calltype = 1 AND date(mt.starttime)>='".$date."' $q GROUP BY mt.eid ORDER BY mt.eid";
		$sql=$this->db->query($query);
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
	...
	function empwise_total($gettimelimit){
		$res=array();
		$res1=array();
	    $date = '';
	    $q = '';
		...
		$query ="SELECT e.empname,count(mt.callid) as cnt
				FROM `".$this->session->userdata('bid')."_mtracker` mt
				LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid=mt.eid
				WHERE mt.status!=2 AND date(mt.starttime)>='".$date."' $q GROUP BY mt.eid ORDER BY mt.eid";
		$sql=$this->db->query($query);
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
   ...
   function employee_list(){
	...
	function apiRes($id,$bid){
		$q = '';
		$sql = "SELECT onhangupResponse,oncallResponse FROM ".$bid."_callhistory WHERE callid = '".$id."'";							 
		$rst = $this->db->query($sql)->result_array();
		$header = array('#'
						,'OnCall Response'
						,'Hangup Response'
	...
	function callbyemployee($type,$gettimelimit){
		$q='';
		$c1='';
		$roleDetail = $this->empmodel->getRoledetail($this->session->userdata('roleid'));
		if($roleDetail['role']['admin']!=1){
		...
		$q .=(isset($se['last_modified']) && $se['last_modified']!='')?" and c.last_modified like '%".$s['last_modified']."%'":"";
		$q .=(isset($se['assignto']) && $se['assignto']!='')?" and k.empname like '%".$se['assignto']."%'":""; 
		$query =    "SELECT e.empname,count(c.callid) as cnt
					FROM ".$this->session->userdata('bid')."_callhistory c
					LEFT JOIN ".$this->session->userdata('bid')."_groups g on c.gid=g.gid
					LEFT JOIN ".$this->session->userdata('bid')."_employee e ON e.eid = c.eid 
					LEFT JOIN ".$this->session->userdata('bid')."_employee k ON k.eid = c.assignto 
	...
	function emp_check1($id){
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$emp = $this->db->query("SELECT assignto,eid FROM ".$bid."_callhistory WHERE callid='".$id."'");
		if($emp->num_rows()>0){
    ...
    function callbackuser(){
		...
		//$call_now_st = isset($_POST['call_now_st'])?$_POST['call_now_st']:'';
		$call_later_st = isset($_POST['call_later_st'])?$_POST['call_later_st']:'';
		$pretime = $call_later_st;
		$to = 'support@vmc.in';
		...
		$message = '<html>
			...
			<body>
				You have just received a Mail from ' . $username .' to call back.
				<br/>
				<br/>
		...
		$headers .= 'To: Support MCube <support@vmc.in>' . "\r\n";
		$headers .= 'From: No reply MCUBE <noreply@mcube.com>' . "\r\n";
		$retval = mail($to,$subject,$message,$headers,'support@vmc.in');	
		return true;
	}
    function dash_limit_set($val){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$this->db->set('dashboard_limit',$val);
	...
	function empDetail($eid){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$sql=$this->db->query("SELECT a.empname,a.empnumber,a.empemail,b.businessname FROM ".$bid."_employee a
		                       LEFT JOIN business b on b.bid = a.bid
		                       WHERE a.eid=$eid");
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}	
