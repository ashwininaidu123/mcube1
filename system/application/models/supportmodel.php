...
Class Supportmodel extends Model
{
	...
	function listSupportTkt($supstatus,$ofset='0',$limit='20',$tab){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add) ?'<li><a href="EditSupTkt/'.$rec['tktid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'support/delSupportTkt/'.$rec['tktid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= '<li><a href="support/activeSupportTkt/'.$rec['tktid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-file"  title="Support Ticket Details">&nbsp;TicketDetails</span></a></li>';
				...
				$act .= '<li><a href="Report/followup/'.$rec['tktid'].'/0/support" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span></a></li>';	
				$act .= ($r['email']!='')?"<li><a href=\"Javascript:void(null)\" onClick=\"window.open('/Email/compose/".$rec['tktid']."/support', 'Counter', 'toolbar=0,scrollbars=0,location=0,status=1,menubar=0,width=950,height=480,resizable=1')\"><span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a>":'<span title="Send Mail" class="glyphicon glyphicon-envelope">&nbsp;SendMail</span></li>';
				$act .= '<li>'.anchor("Report/clicktoconnect/".$rec['tktid']."/5", '<span title="click To Connect" class="glyphicon glyphicon-phone">&nbsp;ClickToConnect</span>',array('class'=>'clickToConnect')).'</li>';
				$act .= '<li>'.anchor("Report/sendSms/".$rec['tktid']."/support", '<span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;SendSMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"').'</li>';	
				$act .= "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('Report/sendFields/".$rec['tktid']."/".$bid."/40/support', 'Send Fields', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=550,height=500,left=200,top=20,resizable=1')\"><span title='Click to Send Fields' class='glyphicon glyphicon-list'>&nbsp;SendFields</span></a></li>";
				$act .= '<li>'.anchor("support/comments/".$rec['tktid'], '<span title="Support Ticket Comments" class="fa fa-comment">&nbsp;Comments</span>','class="btn-danger" data-toggle="modal" data-target="#modal-responsive"').'</li>';
				$act .= '<li>'.anchor("support/remarks/".$rec['tktid']."/", '<span title="Click to Send Fields" class="glyphicon glyphicon-asterisk">&nbsp;Remarks</span>',' class="btn-danger" data-toggle="modal" data-target="#modal-responsive"').'</li>';
				$act .= "</ul></div>";
				$data['action'] = $act;
			}
			foreach($keys as $k){
	...
	function editSupportTkt($id,$bid){
		...
		if($itemDetail['tkt_status'] != $_POST['tkt_status']){
			...
			if($smschk['sms'] == 1 && $smschk['smscontent'] != ''){
				...
				if($smsbal > 0){
					...
					$sms = sms_send($this->input->post('number'),$remsg,$senderid,'tktassign');
					//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
					//~ $remsg = str_replace('@ticketid@',$ticketnumber,$smschk['smscontent']);
					//~ $sms = $api."&to=".substr($this->input->post('number'),-10,10)."&text=".urlencode($remsg);
					//~ $sms = file($sms);
		...
		if(isset($_POST['alert_type']) && ($this->input->post('alert_type') =='3' || $this->input->post('alert_type') =='2')){
			...
			if($smsbal > 0){
				...
				$this->load->helper('mcube_helper');
				$senderid = $this->configmodel->busSenderId($bid);
				$sms = sms_send($sql[0]['empnumber'],$message,$senderid,'tktassign');
				//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="'.base_url().'support/undelSupportTkt/'.$r['tktid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							...
							<ul class="dropdown-menu" style="text-align:left;">';
				if($type == 'del'){
					$act .= '<li><a href="'.base_url().'support/delSupportGrp/'.$rec['gid'].'/1"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				}else{
					$act .= ($opt_add) ?'<li><a href="support/addSupportGrp/'.$rec['gid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
					$act .= ($opt_delete) ? '<li><a href="'.base_url().'support/delSupportGrp/'.$rec['gid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
					$act .= '<li><a href="AddempSupGroup/'.$rec['gid'].'" ><span class="glyphicon glyphicon-plus" title="Add Employee">&nbsp;AddEmployee</span></a></li>';
					$act .= '<li><a href="ListempSupGroup/'.$rec['gid'].'" ><span title="List Employees" class="glyphicon glyphicon-user">&nbsp;ListEmployee</span></a></li>';
				}
				$act .= "</ul></div>";
				$data['action'] = $act;
			}
	...
	function listGrpEmp($sgid,$ofset='0',$limit='20'){
		...
		$res=array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS g.groupname,e.empname,a.weight,a.counter,a.eid,a.gid,a.status,if(e.status='0',0,if(e.selfdisable='1','0','1')) as estatus 
							   FROM ".$bid."_support_grpemp a
							   LEFT JOIN ".$bid."_support_groups g on a.gid=g.gid
							   LEFT JOIN ".$bid."_employee e on a.eid=e.eid
							   WHERE a.gid=$sgid $q 
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename']))
						?'<li><a target="_blank" href="'.site_url('sounds/'.$r['filename']).'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Recording</span></a></li>'
						:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a></li>'):"";
				$act .= '<li>'.anchor("Report/empdetail/".$rec['callid'], ' <span title="List Employees" class="glyphicon glyphicon-user"></span>',array(' class="btn-danger" data-toggle="modal" data-target="#modal-responsive"')).'</li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
