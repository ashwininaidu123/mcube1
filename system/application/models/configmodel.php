...
Class Configmodel extends Model{
	...
	function getDetail($modid='',$itemid='',$formname='',$bid=''){
		...
		switch($modid){
			case '1'://Business Detail
				//~ $sql = "SELECT b.*,bd.*,if(b.language=1,'English','English') as lang FROM business b
				              //~ LEFT JOIN businessacc_details bd ON bd.bid = b.bid
				              //~ WHERE b.bid='".$itemid."'";
				$sql = "SELECT b.*,if(b.language=1,'English','English') as lang,u.uid,u.eid FROM business b
						LEFT JOIN  user u ON b.contactemail = u.username
					    WHERE b.bid='".$itemid."'";
				$rst = (array)$DB2->query($sql)->row();
				break;
			case '2'://Employee Detail
				$sql="select a.*,e.empname as reporttoname,b.rolename  as role,if(u.recording=1,'Yes','No') as recording
			...
			case '6':
				$sql="SELECT a.*,l.number as lead,p.landingnumber as landingnumber,s.number as suptkt,
					...
					IF(a.callee_gender='f','Female',(IF(a.callee_gender='m','Male',''))) as callee_gender,
					IF(a.caller_gender='f','Female',(IF(a.caller_gender='m','Male',''))) as caller_gender,
					IF(a.emotion='p','Positive',(IF(a.emotion='n','Negative',''))) as emotion
			...
			case '22':
				...
				break;
			case 'callarchive':
				$sql="select a.*,if(a.status=1,'Active','Delete') as callstatus,
					...
					IF(rg.area!='',area,'N/A') AS region,acc.archive_path
					from ".$bid."_callarchive a
					LEFT JOIN ".$bid."_employee c on a.eid=c.eid
					LEFT JOIN ".$bid."_employee ac ON a.assignto=ac.eid
					LEFT JOIN ".$bid."_groups d on a.gid=d.gid
					LEFT JOIN account_settings acc on acc.bid=a.bid
					LEFT JOIN series_list1 rg ON 
					(rg.scode = substr(a.callfrom,1,4)
					OR rg.scode = substr(a.callfrom,1,3)
					OR rg.scode = substr(a.callfrom,1,2))
			...
			case '24':
				$sql="SELECT h.*,l.landingnumber as landingnumber,'' as asto,
					  ...
					  LEFT JOIN ".$bid."_pbx i on i.pbxid=h.pbxid
	                  LEFT JOIN prinumbers l on i.prinumber=l.number 
			...
			case '47':
					...
					break;
		    case '51':
				$sql="SELECT a.*,l.number as lead,s.number as suptkt,count(r.callid) as ratingcount,
					b.bid,b.businessname,ac.empname AS assignto,
					a.assignto AS asto,
					IF(a.pulse>0,ceil(a.pulse/60),a.pulse) AS pulse,
					a.pulse AS duration,c.empnumber,
					c.empname as eid,c.eid AS empid,c.empemail,ll.lead_status
					FROM ".$bid."_mtracker a
					LEFT JOIN ".$bid."_employee c ON a.eid=c.eid
					LEFT JOIN ".$bid."_mt_rating r on a.callid = r.callid 
					LEFT JOIN ".$bid."_employee ac ON a.assignto=ac.eid
					LEFT JOIN ".$bid."_leads l ON l.leadid=a.leadid
					LEFT JOIN ".$bid."_leads ll ON ll.number=a.callto
					LEFT JOIN ".$bid."_support_tickets s ON s.tktid=a.tktid
					LEFT JOIN business b ON a.bid=b.bid
					WHERE a.callid='".$itemid."'";
					$rst = (array)$DB2->query($sql)->row(); 
				...
				break;	
		    case '55':
					$sql="SELECT r.*,e.empname as employee FROM ".$bid."_mt_rating r
 		                  LEFT JOIN ".$bid."_employee e on e.eid = r.eid 
		                  WHERE r.ratingid='".$itemid."'"; 
					$rst = (array)$DB2->query($sql)->row(); 
					break;	
		    case '56':
					$sql="SELECT c.* FROM ".$bid."_ct_emplist c
		                  WHERE c.eid='".$itemid."'"; 
					$rst = (array)$DB2->query($sql)->row(); 
					break;	
		    case '57':
					$sql="SELECT c.* FROM ".$bid."_ct_history c
		                   WHERE c.callid='".$itemid."'"; 
					$rst = (array)$DB2->query($sql)->row(); 
            case '58':
					$sql="SELECT o.* FROM ".$bid."_outboundcalls o
		                   WHERE o.callid='".$itemid."'"; 
					$rst = (array)$DB2->query($sql)->row(); 
			...
			case 'mtarchive':
				$sql = "SELECT a.*,
					if(a.pulse>0,ceil(a.pulse/60),a.pulse) as pulse,a.pulse as duration,
					c.empname as eid,c.eid as empid,c.empemail
					FROM ".$bid."_mtarchive a
					LEFT JOIN ".$bid."_employee c on a.eid=c.eid
					LEFT JOIN ".$bid."_employee ac ON a.assignto=ac.eid
					WHERE a.callid='".$itemid."'";	
				$rst = (array)$DB2->query($sql)->row(); 
	...
	function smsBalance($bid){
		$sql = "SELECT clientsms FROM smsconfig WHERE bid='".$bid."'";
		$smschk = $this->db->query($sql)->row()->clientsms;
		if($smschk == 1){
			$balance = 1;
		}else{
	...
	public function callconvert($source){
		...
		if($this->input->post('convertlead') || $this->input->post('updatelead')){
			...
			if(isset($_POST['convertlead'])){
				...
				if($leads_use['type'] == 1 && $leads_use['used'] == 0){
					...
					if($leadsql->num_rows() == 0){
						...
						}elseif($source['type'] == 'mtracker'){
							$this->db->set('name',$this->input->post('name'));
							$this->db->set('email',$this->input->post('email'));
							$this->db->set('caller_add',$this->input->post('address'));
							$this->db->set('caller_bus',$this->input->post('business'));
						}elseif($source['type'] == 'click2connect'){
							$this->db->set('name',$this->input->post('name'));
							$this->db->set('email',$this->input->post('email'));
							$this->db->set('caller_add',$this->input->post('address'));
							$this->db->set('caller_bus',$this->input->post('business'));
					...
					if($source['type'] == 'calltrack'){
						...
						$this->db->query("UPDATE ".$bid."_mtracker SET `leadid`='".$leadid."' WHERE callto ='".$this->input->post('number')."'");
					}elseif($source['type'] == 'click2connect'){
						$this->db->query("UPDATE ".$bid."_outboundcalls SET `leadid`='".$leadid."' WHERE customer ='".$this->input->post('number')."'");
					}
					$process = "TRUE";
				}
			}
			if($this->input->post('updatelead') == 1){ 
				if($source['type'] == 'calltrack'){
					$sql = $this->db->query("SELECT c.leadid FROM ".$bid."_callhistory c WHERE c.callid='".$source['id']."'");
				}elseif($source['type'] == 'ivrs'){
					$sql = $this->db->query("SELECT c.leadid FROM ".$bid."_ivrshistory c WHERE c.hid='".$source['id']."'");
				}elseif($source['type'] == 'pbx'){
					$sql = $this->db->query("SELECT c.leadid FROM ".$bid."_pbxreport c WHERE c.callid='".$source['id']."'");
				}elseif($source['type'] == 'mtracker'){
					$sql = $this->db->query("SELECT c.leadid FROM ".$bid."_mtracker c WHERE c.callid='".$source['id']."'");
				}elseif($source['type'] == 'click2connect'){
					$sql = $this->db->query("SELECT c.leadid FROM ".$bid."_outboundcalls c WHERE c.callid='".$source['id']."'");
				}
				if($sql->num_rows() > 0){
					...
					$this->db->set('caller_add',($source['type'] == 'calltrack') ? $this->input->post('calleraddress') : $this->input->post('address'));
					$this->db->set('caller_bus',($source['type'] == 'calltrack') ? $this->input->post('callerbusiness') : $this->input->post('business'));
					$this->db->set('alert_type',$_POST['lalerttype']);
					$this->db->set('lastmodified',date("Y-m-d H:i:s"));
					$this->db->set('convertedby',$this->session->userdata('eid'));
					$this->db->where('leadid',$leadid);	
			...
			if($process == "TRUE"){
				...
				if(isset($_POST['lalerttype']) && ($this->input->post('lalerttype') =='3' || $this->input->post('lalerttype') =='1')){
					...
					$link = "https://mcube.vmctechnologies.com/site/leadlink/".$leadid;
					$message .= " <br/><br/>Please <a href='".$link."'>click here</a> to check in MCube";
	...
	function busSenderId($bid){
		$sender = $this->db->query("SELECT senderid FROM business WHERE bid='".$bid."'")->row()->senderid;
		return ($sender != '') ? $sender : 'VMCIND';
	}
    ...
    function clientdnd($bid){
		$smsconfig = $this->db->query("SELECT * FROM smsconfig WHERE bid='".$bid."'");
		$res = array();
		if($smsconfig->num_rows()>0){
			$res = $smsconfig->result_array();
			return $res[0];
		}
	}
}
