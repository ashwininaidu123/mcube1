...
Class callLogsmodel extends Model{
	...
	function getcallLogsList($bid,$ofset,$limit,$type=''){
		...
		foreach($rst as $rec){
			...
			$act = '';
			if($type == 'del'){
				$act = ($opt_add) ?'<a href="callLogs/undelete_callLog/'.$rec['call_id'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>':'';
				...
				$act .= anchor("Report/followup/".$rec['call_id']."/0/callLogs", ' <span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span>',array(' class="btn-danger" data-toggle="modal" data-target="#modal-responsive"'));
