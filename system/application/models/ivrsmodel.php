...
class Ivrsmodel extends Model {
	...
	function getOptions($args){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add && $rec['targettype']=='list') ? '<a href="AddOption/'.$r['ivrsid'].'/'.$r['optid'].'/0"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="fa fa-plus" title="Add Option"></span></a>':'<span title="Add Option" class="fa fa-plus"></span>';
	...
	function getIvrslist($bid,$ofset='0',$limit='20'){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add)? '<li><a href="ivrs/addpopup/'.$r['ivrsid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li> ':'';
				$act .= ($opt_delete)? '<li> <a href="ivrs/deleteivrs/'.$r['ivrsid'].'/'.$r['optid'].'" class="confirm deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= ($opt_add)? '<li> <a href="AddOption/'.$r['ivrsid'].'/'.$r['optid'].'/0"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-plus" title="Add Option" >&nbsp;AddOption</span></a></li>':'';
				$act .= ($opt_add)? '<li><a href="ListOption/'.$r['ivrsid'].'/'.$r['optid'].'"><span title="List Option" class="glyphicon glyphicon-list">&nbsp;ListOption</span></a></li>':'';
				$act .= ($opt_view && isset($sound))? '<li><a target="_blank" href="'.$sound.'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Greeting</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
	...
	function recent_calls($ivrsid){
		$res=array();
		$ret=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(hid), 0) AS cnt
							FROM ".$this->session->userdata('bid')."_ivrshistory h 
							WHERE (h.`datetime` >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)) AND h.ivrsid = ".$ivrsid." GROUP BY DAY(h.datetime)");					
		if($sql->num_rows()>0){
		...
		foreach($rst as $rec){
			...
			if($opt_add||$opt_view){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_add)? '<li><a href="ivrs/undelete/'.$r['ivrsid'].'" class=""><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>':'';
				$act .= ($opt_view)? '<li><a target="_blank" href="'.$sound.'"><span title="Sound" class="glyphicon glyphicon-volume-up">&nbsp;Greetings</span></a></li>':'';
				$data['action'] = $act;
			}
			$i++;
			array_push($list,$data);
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
								<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
								<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="IVRSEditReport/'.$rec['hid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>';
				...
				$act .= '<li>'.anchor("Report/clicktoconnect/".$rec['hid']."/2", ' <span title="click To Connect" class="glyphicon glyphicon-phone">&nbsp;ClicktoConnect</span>',array('class'=>'clickToConnect')).'</li>';
				$act .= ($r['email']!='')? "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('/Email/compose/".$r['hid']."/ivrs', 'Send Mail', 'toolbar=0,scrollbars=0,location=0,status=1,menubar=0,width=950,height=480,resizable=1')\"><span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a></li>":'<li><a><span title="Send Mail" class="glyphicon glyphicon-envelope">&nbsp;SendMail</span></a></li>';
				$act .= '<li>'.anchor("Report/followup/".$r['hid']."/0/ivrs", ' <span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span>','class="btn-danger" data-toggle="modal" data-target="#modal-responsive"').'</li>';                           
				$act .= '<li>'.anchor("Report/sendSms/".$rec['hid']."/ivrs", '&nbsp;<span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;ClickToSMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"').'</li>';           
				$act .= "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('Report/sendFields/".$rec['hid']."/".$bid."/16/ivrs', 'Send Fields', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=550,height=500,left=200,top=20,resizable=1')\">&nbsp;<span title='Click to Send Fields' class='glyphicon glyphicon-list'>&nbsp;SendFields</span></a></li>";
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
				//~ if($k == 'callfrom'){
				    //~ $v = anchor("Report/clicktoconnect/".$rec['hid']."/2", "$r[$k]",array('class'=>'clickToConnect','title' =>'Click to connect'));
				//~ }else
				if($k=='employee'){
	...
	function emp_check1($id){
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$emp = $this->db->query("SELECT eid FROM ".$bid."_ivrshistory WHERE hid='".$id."'");
		if($emp->num_rows()>0){
