...
Class Mtrackermodel extends Model
{
	...
	function editEmp($bid,$eid){
		$res = array();
		$res = $this->db->query("SELECT e.eid,e.empname,e.empnumber,u.recording,u.mcubecalls,u.mtdebug,me.starttime,me.endtime FROM ".$bid."_employee e 
			...
			if($_POST['eid'] == 0){
				foreach($_POST['emp_ids'] as $eids){
					if($chk < $cnt){
						...
						if(@in_array($eids,$_POST['mtdebug'])){
							$sql = $this->db->query("UPDATE user SET mtdebug = '1' WHERE bid=".$bid." AND eid = '".$eids."'");
						}
				...
				$mtd = (@in_array($eid,$_POST['mtdebug'])) ? '1' : '0';
				$sql = $this->db->query("UPDATE user SET recording = '".$rec."', mcubecalls = '".$mcc."',mtdebug = '".$mtd."'WHERE bid=".$bid." AND eid = '".$eid."'");
			}
			if($err!=0){
				return 1;
			}else{
	...
	function mtemplist($bid,$ofset,$limit){
		...
		$res['data'] = $this->db->query("SELECT SQL_CALC_FOUND_ROWS e.empname,e.empnumber,mt.eid,mt.status,mt.starttime,mt.endtime FROM ".$bid."_mt_emplist mt LEFT JOIN ".$bid."_employee e ON e.eid = mt.eid WHERE mt.status = 1 ".$con." ORDER BY e.empname ASC LIMIT $ofset,$limit")->result_array();
