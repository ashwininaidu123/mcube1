...
Class Leadsmodel extends Model
{
	...
	function list_leads($type,$ofset='0',$limit='20',$tab){
		...
		if($tab>0){
			...
			$content = isset($qs['content']) ? (array)json_decode($qs['content']) : '';
		...
		if(isset($Ads) && sizeof($Ads)>0){
			...
			if(isset($Ads['multiselect_eids']) && sizeof($Ads['multiselect_eids'])>0){
				...
				$q .= ($eids != '') ?  " and (a.assignto IN (".$eids.") OR a.enteredby IN (".$eids.") )" : '';
			}
			$custiom_ids=array();
			$cust=0;
		...
		$q.=(isset($s['leadowner']) && $s['leadowner']!='')?" and a.leadowner ='".$s['leadowner']."'":"";
		...
		foreach($fieldset as $field){
			$checked = false;
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing'] &&  !in_array($field['fieldname'],array('leadhistory'))){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add) ?'<li><a href="EditLead/'.$rec['leadid'].'/'.$lead_status.'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete) ? '<li><a href="'.base_url().'leads/delete/'.$rec['leadid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= '<li><a href="leads/active_lead/'.$rec['leadid'].'/'.$lead_status.'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-list" title="Lead Details">&nbsp;LeadDetails</span></a></li>';
				$act .= '<li><a href="Report/followup/'.$rec['leadid'].'/0/leads" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span></a></li>';		
				$act .= ($r['email']!='') ? "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('Email/compose/".$rec['leadid']."/leads', 'Sent Email', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=950,height=480,resizable=1')\">&nbsp;<span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a><li>":"<li><a><span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a></li>";
				$act .= "<li>".anchor("Report/clicktoconnect/".$rec['leadid']."/4", '<span title="click To Connect" class="glyphicon glyphicon-phone">&nbsp;ClickToConnect</span>',array('class'=>'clickToConnect'))."</li>";
				$act .= '<li><a href="Report/sendSms/'.$rec['leadid'].'/leads" class="clickToSMS" data-toggle="modal" data-target="#modal-empl"><span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;SendSMS</span></a></li>';
				$act .= "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('leads/sendFields/".$rec['leadid']."/".$bid."/26/lead', 'Send Fields', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=550,height=500,left=200,top=20,resizable=1')\"><span title='Click to Send Fields' class='glyphicon glyphicon-list-alt'>&nbsp;SendFields</span></a></li>";
				$act .= '<li><a href="leads/comments/'.$rec['leadid'].'/'.$lead_status.'" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Lead Comments" class="glyphicon glyphicon-comment">&nbsp;Comments</span></a></li>';
				$act .= '<li><a href="leads/remarks/'.$rec['leadid'].'/" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Click to Send Fields" class="glyphicon glyphicon-asterisk">&nbsp;Remarks</span></a></li>';
				$act .= '<li><a href="leads/lead_history/'.$rec['leadid'].'/'.$bid.'" class="btn-leadhis" data-toggle="modal" data-target="#modal-leadhis"><span title="Lead History" class="glyphicon glyphicon-share">&nbsp;History</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			foreach($keys as $k){
	...
	function edit_lead($id,$bid,$type){
		...
		if(isset($_POST['alert_type']) && ($this->input->post('alert_type') =='3' || $this->input->post('alert_type') =='2')){
			...
			if($smsbal > 0){
				...
				$sms = sms_send($sql[0]['empnumber'],$message,$senderid,'leadassign');
	...
	function deleted_list($ltype,$bid,$ofset='0',$limit='20'){
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS a.leadid FROM ".$bid."_leads a 
				LEFT JOIN ".$bid."_leads_groups d on a.gid=d.gid 
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= '<li><a href="'.base_url().'leads/undelete/'.$r['leadid'].'/'.$ltype.'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
	...
	function addlead($type)
	{
		...
		if($leads_use['type'] == 1 && $leads_use['used'] == 0 && $id == '') {
			...
			if($proces == "TRUE") {
				...
				for($i=0;$i<sizeof($arr);$i++) {
					if($arr[$i]!="custom" && $arr[$i] != "autoAssign" && $arr[$i] != "duplicate" && $arr[$i] != "remark" && $arr[$i] != "update_system") {
						...
						if(is_array($_POST[$arr[$i]]))
							...
							//$_POST[$arr[$i]] = str_replace("'","\\'", $_POST[$arr[$i]]);
							$val = $_POST[$arr[$i]];
						} else
							$val='';
						$this->db->set($arr[$i],$val);
				...
				///CLIENT SMS
				$cust_sms = $this->sysconfmodel->cust_sms($bid);
			    if(isset($cust_sms['lead_sms']) && ($cust_sms['lead_sms'] !='0')) {
					if($smsbal > 0) {
						$message = $cust_sms['textto_cust'];
						$this->load->helper('mcube_helper');
						$senderid = $this->configmodel->busSenderId($bid);
						$sms = sms_send($this->input->post('number'),$message,$senderid,'leadassign');
						$this->configmodel->smsDeduct($bid,'1');
					} else {
						$return = '3';
					}
	...
	function getLeadSystemfields($type, $bid) {
		...
		foreach($fieldset as $r) {
			if(!@in_array($r['fieldname'],array('bid','leadid','gid','enteredby','lastmodified','status','lead_status','dis_type','alert_type','refId','convertedby','duplicate','parentId','remark','convertedon','leadowner')) && $r['show'] == 1 && $r['type'] == 's')
	...
	function addimportLeads($data,$custdata){
		...
		if($leads_use['type'] == 1 && $leads_use['used'] == 0) {
			...
			if(array_key_exists($data['assignto'], $this->leadEmailArray)) {
                $this->leadEmailArray[$data['assignto']] = ($this->leadEmailArray[$data['assignto']]+1);
            } else {
                $this->leadEmailArray[$data['assignto']] = 1;
            }
    ...
    {
        foreach ($this->leadEmailArray as $id => $count) {

            $empData = $this->empmodel->get_empname($id);

            $message = "New Leads have been assigned to you.Please check with your login.<br/>";
            $message .= "<br/>Assigned leads Count : ". $count;
            $message .= "<br/>Assigned By : ". $this->empgetname($this->session->userdata('eid'));
            
            $body = $this->emailmodel->newEmailBody($message,$empData->empname);
            $to  = trim($empData->empemail);
            // $to = trim('siddharth.kar@vmc.in');
	...
	function empAssign_exists($empemail) {
		...
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$empassignId = $this->db->query("SELECT e.eid FROM ".$bid."_employee e WHERE (e.empemail='".$empemail."'  OR e.empname LIKE '".$empemail."') AND e.status='1'")->result_array();
	...
	function list_leads_grps($bid,$ofset,$limit,$type=''){
		...
		//~ if($opt_add || $opt_view || $opt_delete)
			//~ array_push($header,$this->lang->line('level_Action'));
		array_push($header,'Last 7 days calls');
		foreach($fieldset as $field){
			$checked = false;
			if($field['type']=='s' && !$field['is_hidden'] && $field['show'] && $field['listing']){
				foreach($roleDetail['system'] as $f){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							...
							<ul class="dropdown-menu" style="text-align:left;">';
				if($type == 'del'){
					$act .= '<li><a href="'.base_url().'leads/delete_leadsgrp/'.$rec['gid'].'/1"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li>';
				}else{
					$act .= ($opt_add) ? '<li><a href="EditLeadGroup/'.$rec['gid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
					$act .= ($opt_delete) ? '<li><a href="'.base_url().'leads/delete_leadsgrp/'.$rec['gid'].'" class="deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
					$act .= '<li><a href="AddempLeadGroup/'.$rec['gid'].'" ><span class="glyphicon glyphicon-plus" title="Add Employee">&nbsp;AddEmployee</span></a></li>';
					$act .= '<li><a href="ListempLeadGroup/'.$rec['gid'].'" ><span title="List Employees" class="glyphicon glyphicon-user">&nbsp;ListEmployee</span></a></li>';
				}
				$data['action'] = $act;
			}
			$ret['graph'] = $this->weekly_leads($rec['gid']);
	...
	function weekly_leads($gid){
		$res=array();
		$ret=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(leadid), 0) AS cnt
	...
	function leadgrpemplist($lgid,$ofset='0',$limit='20'){
		...
		$res=array();
		$res['data']=$this->db->query("SELECT SQL_CALC_FOUND_ROWS g.groupname,e.empname,a.weight as                            empweight,a.counter,a.eid,a.gid,a.status,if(e.status='0',0,if(e.selfdisable='1','0','1')) as estatus 	
							   FROM ".$bid."_leads_grpemp a
							   LEFT JOIN ".$bid."_leads_groups g on a.gid=g.gid
							   LEFT JOIN ".$bid."_employee e on a.eid=e.eid
							   WHERE a.gid=$lgid $q 
	...
	function get_grEmployees($lgid){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$res = $this->db->query("SELECT e.empname,e.selfdisable,a.eid
							   FROM ".$bid."_leads_grpemp a
							   LEFT JOIN ".$bid."_employee e on a.eid=e.eid
							   WHERE a.gid=$lgid AND e.status = 1 AND a.status = 1 AND e.selfdisable = 0
	...
	function allEmployees($gid,$eid){
		...
		$query = "SELECT e.eid,e.empname FROM ".$bid."_employee e
				 LEFT JOIN ".$bid."_leads_grpemp ge on e.eid=ge.eid
				 LEFT JOIN ".$bid."_leads_groups g on g.gid=ge.gid
				 WHERE e.status = '1' AND ge.status = '1' AND g.gid='".$gid."' AND e.selfdisable = 0 ORDER BY e.empname";
		$query=$this->db->query($query);
		if($query->num_rows()>0){
			if($eid == 0){
				$res[0] = "Select";
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act = ($opt_add) ?'<li><a href="Report/edit/'.$r['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				...
				$act .= '<li><a href="Report/activerecords/'.$r['callid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-file"  title="List Employee Group">&nbsp;GroupDetails</span></a></li>';
				$act .= ($roleDetail['role']['accessrecords']=='0') ? (($r['filename']!='' && file_exists('sounds/'.$r['filename']))
						...
						:'<a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></a>'):"";
				$act .= '<li>'.anchor("Report/empdetail/".$rec['callid'], ' <span title="List Employees" class="glyphicon glyphicon-user"></span>','class="btn-empl" data-toggle="modal" data-target="#modal-empl"').'</li>';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			$i++;
	...
	function getfieldValchk($fieldval,$fieldid,$bid){
		$fielddetails = $this->db->query("SELECT * FROM ".$bid."_customfields WHERE fieldid='".$fieldid."' AND modid='26'")->result_array();
	...
	function emp_check($id){
		$cbid = $this->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="") ? $cbid : $this->session->userdata('bid');
		$emp = $this->db->query("SELECT assignto,enteredby,convertedby FROM ".$bid."_leads WHERE leadid='".$id."'");
		if($emp->num_rows()>0){
