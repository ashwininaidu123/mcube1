...
class User extends Controller {
	...
	function otp(){
		...
		if ($this->form_validation->run() == FALSE)	{
			...
			if($this->simplelogin->login($this->input->post('login_username'), $this->input->post('login_password'))) {
				...
				if($otpType=='0'){
					$this->session->set_userdata('roleid',$this->empmodel->getRoleid($this->session->userdata('eid')));
					$refURL = (isset($_POST['referlink']) && $_POST['referlink'] != '') ? "EditLead/".$this->input->post('referlink')."/all" : 'Home';
					redirect($refURL);
				}
				...
				$message = "\nYour one time password for MCube is: ".$otp;
				$sms = sms_send($to,$message,$senderid,'otp');
				...
				$refURL = (isset($_POST['referlink']) && $_POST['referlink'] != '') ? "EditLead/".$this->input->post('referlink')."/all" : '/site/otp';
				redirect($refURL);
			}else {
				if($this->session->userdata('flash:new:msg')==""){
	...
	function actregister()	{
	    ...
	    $this->form_validation->set_rules('businessType', 'Business Type', 'required|min_length[4]|max_length[50]|alpha');
	...
	function forgetpass(){
		...
		if ($this->form_validation->run() == FALSE)	{
			...
			if($user['uid']>0 && $emp['login']=='1' && $emp['status']=='1'){
				...
				$this->load->helper('mcube_helper');
				$to = $emp['empnumber'];
				$senderid = $this->configmodel->busSenderId($user['bid']);
				$message = str_replace("\n"," ",str_replace("\t","",$message));
				$sms = sms_send($to,$message,$senderid,'password');
				//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
				//~ $message = str_replace("\n"," ",str_replace("\t","",$message));
	...
	function assistance(){
		if($this->input->post('needAssistance')){
			$this->form_validation->set_rules('firstname', 'Firstname', 'required|min_length[4]|max_length[50]');
			$this->form_validation->set_rules('lastname', 'Lastname', 'required|min_length[4]|max_length[50]');
			...
			$this->form_validation->set_rules('company', 'Company', 'required|min_length[4]|max_length[150]');
			$this->form_validation->set_rules('zipcode', 'Zipcode', 'required|min_length[4]|numeric');
			...
			if (!$this->form_validation->run() == FALSE){
				...
				$msg=$this->load->view('sendMail',$data,true);
				$this->email->message($this->outlookfilter($msg));
				if(strpos($_SERVER['HTTP_REFERER'], 'mcube.vmc.in')){
					$this->email->send();
