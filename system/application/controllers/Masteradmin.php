...
class Masteradmin extends controller
{
	...
	function AdminLog(){
		...
		$this->mastermodel->viewLayout('admin_list_view',$data);	
	}
	...
	function managePriList($page=''){
		...
		foreach ($emp_list['data'] as $item){
			...
			$edit.=(!$delete_opt)?'':'<a href="'.site_url('Masteradmin/delete_pri/'.$item['bid'].'/'.$item['number']).'" class="confirm" id="'.$item['number'].'" title="'.$cls.'">
			...
			$upload =($this->mastermodel->Module_name($this->mastermodel->landingDetails($item['landingnumber'])) =="Call Track")?
			('<a href="'.site_url('Masteradmin/upload_file/'.$item['bid'].'/'.$item['number']).'" class="callPopup">
			 <img src="'.site_url('system/application/img/icons/import.png').'" title="Upload audio file" width="16" height="16" /></a>'): '';
			 $upload = '<a href="'.site_url('Masteradmin/upload_file/'.$item['bid'].'/'.$item['number'].'/'.$item['service']).'" class="callPopup">
			 <img src="'.site_url('system/application/img/icons/import.png').'" title="Upload audio file" width="16" height="16" /></a>';
			...
			$number_status = ($item['number_status'] == 0)?'<span class="glyphicon glyphicon-eye-close" title=" Number Deactivate "></span>':'<span class="glyphicon glyphicon-eye-open" title="Number Activate "></span>';
			$chk = (!$enable_s)?'':'<a href="'.site_url('Masteradmin/number_status/'.$item['number'].'/'.$item['landingnumber']).'"  class="confirm" title="'.$title.'"/>'.$number_status.'</a>';
			...
			$rec[] = array(
				...
				,$edit.'<a href="'.(($item['type']!="Demo")?site_url('Masteradmin/Pack_history/'.$item['bid'].'/'.	$item['number']):'javascript:void(0)').'" class="'.(($item['type']!="Demo")?'btn-danger':'').'" data-toggle="modal" data-target="#modal-responsive"><i class="fa fa-list-alt" title="History"></i></a>'.$file .$chk 
		...
		$links[] = '<li><a href="Masteradmin/numberConfig"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;
		$data['form'] = array(
	...
	function delete_getunasd($sensorid){
		$r=$this->mastermodel->delete_getunasdloc($sensorid);
		$flashdata = array('msgt' => 'success', 'msg' => 'Deleted Successfully');
		$this->session->set_flashdata($flashdata);
		redirect('Masteradmin/Mageunasdlocation');
	}
	...
	function ManageunassignedNumbers($page=''){
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="Masteradmin/AssignMobileNumber/"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
		$formFields1 = array();
		$cf=array('label'=>'<label class="col-sm-4 text-right" for="groupname">'.$this->lang->line('level_groupname').' : </label>',
				  'field'=>form_input(array(
	...
	function creditlist(){
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="Masteradmin/creditConfig"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
	...
	function leadsversions(){
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="Masteradmin/leadscredit"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
	...
	function feature_access($bid){
		$show=0;
		$checklist=$this->mastermodel->checked_featuremanage($bid);
		if(in_array(17,$checklist)){
			$show=1;
		}
		return $show;
	}
	function total_trackcalls($bid){
		$this->mastermodel->data['html']['title'] .= " | Total Trackcalls";
		$data['module']['title'] ="Total Trackcalls";
		$trackcount=$this->mastermodel->total_trackcount($bid);
		$header = array('SL No'
				        ,'Business Name'
		                ,'Date'
		                ,'Total Track calls'
		...
		$data['itemlist']['header'] = $header;
		$rec = array();
		if(count($trackcount['data'])>0)
	    $sn_count = 1;
	    $data['links']='';
		foreach ($trackcount['data'] as $item){
		   $rec[] = array(
		         $sn_count++
		        ,$item['BusinessName']
				,$item['date']
		...
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $trackcount['count'];
		$data['nobulk']=true;	
		$formFields = array();
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/total_trackcalls/'.$bid,array('name'=>'total_trackcalls','class'=>'form','id'=>'total_trackcalls','method'=>'post')),
			'close'=>form_close(),
			);
	   $this->mastermodel->viewLayout('admin_list_view',$data);
	}
	function Businesslist(){
		...
		if(count($emp_list['data'])>0)
		foreach ($emp_list['data'] as $item){
		    $access1 = $this->feature_access($item['bid']);
		    ...
		    $mcheck  = $this->mastermodel->getfeatureAccess('15',$item['bid']) ;
			$mtaccess = ($mcheck == 1) ? '<a href="Masteradmin/mtrackEmp/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="MTracker License" class="glyphicon glyphicon-check"></span></a>' : '<span title="MTracker License" class="glyphicon glyphicon-check">';
			...
			$rec[] = array(
				...
				,$item['accemail']
				,$item['accphone']
				,'<a href="Masteradmin/editbusiness/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Edit" class="fa fa-edit"></span></a>
				<a href="Masteradmin/businessview/'.$item['bid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive">
				<span title="view" class="fa fa-list-alt"></span></a>'.(($item['status']=="0")?'<a href="Masteradmin/changeBusinessstatus/'.$item['bid'].'"><span class="fa fa-lock changeBusStatus" id="'.$item['bid'].'"  title="Enable">&nbsp;</a>':'<a href="Masteradmin/changeBusinessstatus/'.$item['bid'].'"><span class="fa fa-unlock changeBusStatus" id="'.$item['bid'].'"  title="Disable">&nbsp</a>')/*'<a class="callPopup" href="Masteradmin/ProductConfigure/'.$item['bid'].'"><img id="'.$item['bid'].'" src="system/application/img/icons/cog.png" title="Configure Rate" width="16" height="16" /></a>*/.'<a href="Masteradmin/ManageFeature/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Feature Manage" class="fa fa-gears"></span></a>
				...
				><span title="Send Mail to Unconfirm Employees" class="fa fa-mail-reply-all "></span> 	</a>'.' <a 
				href="Masteradmin/passwordreset/'.$item['bid'].'" 
				><span title="Reset" class="fa fa-refresh"></span></a>'/*' <a 
				...
				/></a>'*/.' <a href="Masteradmin/unconfirm_list_business/'.$item['bid'].'"><span title="List Employees" class="glyphicon glyphicon-user"></span></a>'.'<a href="Masteradmin/LandingNumberDetails/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="List Employees" class="fa fa-list-ul"></span></a>'.'<a href="Masteradmin/dailyReport/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Email Report" class="fa fa-file-text"></span></a>'.
				(($item['dnd_status']!=1)?'<a href="Masteradmin/dndfilter/'.$item['bid'].'"><span title="DND Filter" class="fa fa-check-square-o"></span></a>':'<a href="Masteradmin/dndfilter/'.$item['bid'].'"><span title="DND Filter" class="glyphicon glyphicon-ban-circle"></span></a>')
				.'<a href="Masteradmin/fsetup/'.$item['bid'].'"><span title="Folloup Setup" class="fa fa-retweet"></span></a>'.(($item['supEsc'] == 1)?'<a href="Masteradmin/supEscConfig/'.$item['bid'].'"><span class="glyphicon glyphicon-ok-circle" id="'.$item['bid'].'"  title="Support Escalation Settings"></span></a>':'<a href="Masteradmin/supEscConfig/'.$item['bid'].'"><span class="glyphicon glyphicon-remove-circle" title="Support Escalation Settings"></span></a>')
				.'<a href="Masteradmin/listlanding/'.$item['bid'].'"><span class="fa fa-list-ol" title="Business Landing NUmbers"></span></a>'
				.'<a href="Masteradmin/businessEmployee/'.$item['bid'].'"><span class="fa fa-group" title="List Business Employees"></span></a>&nbsp;'
				.$mtaccess
				.'<a href="Masteradmin/total_trackcalls/'.$item['bid'].'"><span class="fa fa-phone" title="Total Number of Track Calls"></span></a>&nbsp;'
				//.((isset($access1) && $access1 == '1')?('<a href="Masteradmin/locationlist/'.$item['bid'].'"><i class="fa fa-bullseye" title="List Sensor"></i></a>'):'')
			    // .((isset($access1) && $access1 == '1')?('<a href="Masteradmin/activateoffer/'.$item['bid'].'"><span title="Offers"  class="glyphicon glyphicon-certificate"></span></a>'):'')
			   .(($mcheck == 1) ?('<a href="Masteradmin/mtlicenceemp/'.$item['bid'].'"><span title="MTracker Employee Details"  class="fa fa-phone-square"></span></a>'):'')
                            );
		}
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
	...
	function unconfirm_list_business($bid){
		...
		foreach ($emp_list['data'] as $item){
			$rec[] = array(
				...
				,$item['empemail']
				,$item['empnumber']
				,'<a href=Masteradmin/delete_unconfirmEmployee/'.$item['eid'].'/'.$bid.'><span title="Delete" class="glyphicon glyphicon-trash"></span></a>'.'<a href="Masteradmin/confirmEmp/'.$item['eid'].'/'.$bid.'" class="confirmEmp" rel="'.$item['eid'].'"><span class="fa fa-lock changeBusStatus" id="1" title="Confirm">&nbsp;</span></a>'
			);
		}
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
 ...
 $access=$this->mastermodel->get_access_module($this->session->userdata('role_id'),'Business',($bid!='')?1:0);
		...
		$cf=array('label'=>'Action Perform on Call  ',
				  'field'=>form_input(array(
							...
							array_push($formFields,$cf);
		$cf=array('label'=>'Connect Executive by Pincode  ',
				  'field'=>form_checkbox(array(
									  'name'        => 'pincode',
										'id'          => 'pincode',
...
function group_emp_list($bid,$gid){
		...
		foreach ($emp_list['data'] as $item){
			 		...
			 		$r = array(
				(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['empname'] : '<font color=red>'.$item['empname'].'</font>')
				,(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['groupname'] : '<font color=red>'.$item['groupname'].'</font>'));
	    if($item1['primary_rule']>0)$r[]=(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['region'] : '<font color=red>'.$item['region'].'</font>');	
		if($item1['rule']=='2')	$r[]=($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['empweight'] : '<font color=red>'.$item['empweight'].'</font>';
		if($item1['rule']=='3')$r[]=($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['empPriority'] : '<font color=red>'.$item['empPriority'].'</font>';
				$r = array_merge($r,array(
							(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['starttime'] : '<font color=red>'.$item['starttime'].'</font>'),
							(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['endtime'] : '<font color=red>'.$item['endtime'].'</font>'),
							(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['failover'] : '<font color=red>'.$item['failover'].'</font>'),
							(($item['empstatus']=="1" &&  $item['grpstatus']=='1' && $item['selfdisable']=='1') ? $item['callcounter'] : '<font color=red>'.$item['callcounter'].'</font>')
							));
					$rec[] = $r;
			 }
        }
...
function listlanding($bid){
		...
		foreach ($emp_list['data'] as $item){
			...
			$rec[] = array(
		         $sn_count++
		        ,(($item['status']=="1") ? $item['LandingNumber'] : '<font color=red>'.$item['LandingNumber'].'</font>')
		        ,(($item['status']=="1") ? $item['GroupName'] : '<font color=red>'.$item['GroupName'].'</font>')
		        ,(($item['status']=="1") ? $item['Groupowner'] : '<font color=red>'.$item['Groupowner'].'</font>')
		        ,(($item['status']=="1") ? $item['keyword'] : '<font color=red>'.$item['keyword'].'</font>')
		        ,(($item['status']=="1") ? $item['Rules'] : '<font color=red>'.$item['Rules'].'</font>')
		        ,(($item['status']=="1") ? $item['Regionname'] : '<font color=red>'.$item['Regionname'].'</font>')
		        ,(($item['status']=="1") ? $v : '<font color=red>'.$v.'</font>')
		        ,(($item['status']=="1") ? $connectowner : '<font color=red>'.$connectowner.'</font>')
		        ,(($item['status']=="1") ? $item['timeout'] : '<font color=red>'.$item['timeout'].'</font>')
		        ,(($item['status']=="1") ? $act : '<font color=red>'.$act.'</font>')
			);
			}
	...
	Function DeletedList(){
		...
		foreach ($credit_info['data'] as $item){
			$rec[] = array(
					...
					$item['domain_name'],
					$item['create_date'],
					'<a href="'.base_url().'Masteradmin/UnDeletepartner/'.$item['partner_id'].'"  title="Delete"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>',
			);
			$i++;
		}
		$data['itemlist']['rec'] = $rec;
	...
	function addbusinessuser($id=''){
		...
		$cf=array('label'=>'<label class="col-sm-4 text-right">Contact Phone :</label>',
				  'field'=>form_input(array(
							...
							array_push($formFields,$cf);
	    $cf=array('label'=>'<label class="col-sm-4 text-right">Business Admin Designation :</label>',
				  'field'=>form_input(array(
									    'name'       => 'busadmdes',
										'id'          => 'busadmdes',
										'value'       => (isset($get_Fields[0]['busadmdes']))?$get_Fields[0]['busadmdes']:'',
										'class'		  =>'form-control')
									));
	    ...
	    $cf=array('label'=>'<label class="col-sm-4 text-right">Description :</label>',
				  'field'=>form_textarea(array(
							...
							($id=="")?array_push($formFields,$cf):'';					
	   $cf=array('label'=>'<label class="col-sm-4 text-right">Accountant Name :</label>',
				  'field'=>form_input(array(
									    'name'       => 'accname',
										'id'          => 'accname',
										'value'       => (isset($get_Fields[0]['accname']))?$get_Fields[0]['accname']:'',
										'class'		  =>'form-control')
									));
							($id=="")?array_push($formFields,$cf):'';				
		$cf=array('label'=>'<label class="col-sm-4 text-right">Accountant Phone :</label>',
				  'field'=>form_input(array(
									  'name'        => 'accphone',
										'id'          => 'accphone',
										'value'       =>(isset($get_Fields[0]['accphone']))?$get_Fields[0]['accphone']:'',
										'class'		=>'required form-control')
									));									
								($id=="")?array_push($formFields,$cf):'';	
	$cf=array('label'=>'<label class="col-sm-4 text-right">Accountant Email :</label>',
				  'field'=>form_input(array(
									  'name'        => 'accemail',
										'id'          => 'accemail',
										'value'       =>(isset($get_Fields[0]['accemail']))?$get_Fields[0]['accemail']:'',
										'class'		=>'required form-control')
									));
							($id=="")?array_push($formFields,$cf):'';
			$data['form'] = array(
	...
	function addAddon($id){
		...
		if($this->input->post('update_system')){
			...
			$this->form_validation->set_rules('rate1', 'Rate', 'required|numeric');
			if(!$this->form_validation->run() == FALSE){	
	...
	function listaddons(){
		...
		$data['itemlist']['count'] = $emp_list['count'];
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/listaddons/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>4					
				));
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/listaddons/',array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
	...
	function salesemp(){
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="Masteradmin/addSalesEmp"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
		$this->pagination->initialize(array(
	...
	function adminroles(){
		...
		$links[] = '<li><a href="Masteradmin/addAdminRole"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
	...
	function ListAdminusers(){
	    ...
	    $links = array();
		$links[] = '<li><a href="Masteradmin/addAdminuser"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
		$this->pagination->initialize(array(
	...
	function lbsnumbers($page=''){
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="Masteradmin/Addlbsnumber/"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
	...
	function ListFCategories(){
		...
		$links[] = '<li><a href="Masteradmin/addFCategories"><span title="Add Number" class="glyphicon glyphicon-plus-sign">&nbsp;AddNumber</span></a></li>';
		$data['links']= $links;	
		$data['itemlist']['rec'] = $rec;
	...
	function number_status($number,$landingNumber){
		$res=$this->mastermodel->number_status($number,$landingNumber);
		$this->session->set_flashdata('msgt', 'success');
		($res!=0)?$this->session->set_flashdata('msg', $landingNumber." Activated "):$this->session->set_flashdata('msg', $landingNumber." Deactivated ");
		redirect('Masteradmin/managePriList');
	}
	...
	function addbeacon(){
		if($this->input->post('update_system')){
			$res=$this->mastermodel->addnwbeacon();
			if($res == true){
				$this->session->set_flashdata('msgt', 'success');
				$this->session->set_flashdata('msg','Sensor Id added successfully');
				redirect('Masteradmin/addbeacon');
			}else{
				$this->session->set_flashdata('msgt', 'success');
				$this->session->set_flashdata('msg','Sensor Id already exists');
				redirect('Masteradmin/addbeacon');
			}
		}
		$this->mastermodel->data['html']['title'] .= " |Add Sensor";
		$data['module']['title'] = 'Add Sensor';
		$formFields = array();$formFields1 = array();
		$cf=array('label'=>'<label class="col-sm-4 text-right">Add sensor</label>',
				  'field'=>form_input(array(
									  'name'        => 'beacon_id',
										'id'          => 'beacon_id',
										'value'       => '',
						...
						array_push($formFields,$cf);
			$data['form'] = array(
		       	   'form_attr'=>array('action'=>'Masteradmin/addbeacon','id'=>'addbeacon','name'=>'addbeacon','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'fields1'=>$formFields1,
					'close'=>form_close()
				);
	...
	function locationlist($bid){
	$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;
		$limit = '30';
		$emp_list=$this->mastermodel->getlocationlist($bid,$ofset,$limit);
		$rec = array();
	    $header = array('#'
						,'Sensor ID'
						,'Business Name'
		...
		foreach ($emp_list['data'] as $rows){
			$list=array($i);
			foreach($rows as $field => $val){array_push($list,$val);}
			$i++;
			array_push($rec,$list);
		}
		...
		$data['links']= $links;	
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/locationlist/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>3					
				));		
		$data['paging'] = $this->pagination->create_links();
		$data['module']['title'] = "  Sensor List [".$data['itemlist']['count']."]";
		$data['html']['title'] = " | Sensor List [".$data['itemlist']['count']."]";
		$data['links']=$data['links']='';
		$formFields1 = array();
		...
		$formFields[] = array(
			'label'=>'<label class="col-sm-4 text-right" for="f">Sensor ID: </label>',
				'field'=>form_input(array(
						'name'      => 'beacon_id',
						'id'        => 'beacon_id',
						'value'     => '',
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/locationlist/',array('name'=>'locationlist','class'=>'form','id'=>'locationlist','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
	...
	function assigntosite(){
		if($this->input->post('update_system')){
			$this->form_validation->set_rules('bname', 'Business', 'required');
			$this->form_validation->set_rules('sensor_id', 'Sensor Id', 'required');
			if(!$this->form_validation->run() == FALSE){
				$res=$this->mastermodel->assigntosite();
				redirect('Masteradmin/assigntosite');
			}
		}
		$this->mastermodel->data['html']['title'] .= " | Add Admin user";
		$data['module']['title'] = "Add Admin user";
		$formFields = array();$formFields1 = array();
		$buss=$this->mastermodel->get_business_mconnect();
	    $cf=array('label'=>"<label class='col-sm-4 text-right'>Business: </label> ",
				  'field'=>form_dropdown("bname",$buss,''," 'id'='bname' class=form-control"));
						array_push($formFields,$cf);
		$rows=$this->mastermodel->getBeaconlist();
		$cf=array('label'=>"<label class='col-sm-4 text-right'>Sensor Id: </label> ",
				  'field'=>form_dropdown("sensor_id",$rows,''," 'id'='sensor_id' class=form-control"));
						array_push($formFields,$cf);
		$data['form'] = array(
		        'form_attr'=>array('action'=>'Masteradmin/assigntosite','name'=>'assigntosite','id'=>'assigntosite','enctype'=>"multipart/form-data"),
				'fields'=>$formFields,
				'fields1'=>$formFields1,
				'close'=>form_close()
			);
	...
	function Mageunasdlocation(){
		$ofset = ($this->uri->segment(3)!=null)?$this->uri->segment(3):0;
		$del_opt=$this->mastermodel->get_access_module($this->session->userdata('role_id'),'Number',2);
		$edit_opt=$this->mastermodel->get_access_module($this->session->userdata('role_id'),'Number',1);
		$limit = '30';
		$emp_list=$this->mastermodel->getunasdlocation($ofset,$limit);
		$rec = array();
		$header=array('#',$this->lang->line('level_Action'));
		$i=$ofset+1;$j=0;
		if($emp_list['count']>0)
		foreach ($emp_list['data'] as $rows){
			$edit=(!$del_opt)?'':'<a href="'.site_url('Masteradmin/delete_getunasd/'.$rows['SensorId']).'" class="confirm" id="'.$rows['SensorId'].'" title="">
						<span title="Delete" class="glyphicon glyphicon-trash"></span>
				  </a>';
			...
			foreach($rows as $field => $val){ array_push($header,$field);}
			foreach($rows as $field => $val){array_push($list,$val);}
		...
		$data['links']= $links;	
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/Mageunasdlocation/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>3					
				));		
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/Mageunasdlocation/',array('name'=>'Mageunasdlocation','class'=>'form','id'=>'Mageunasdlocation','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
	...
	function upload_file($bid,$number,$type){
		if($this->input->post('update_system')){
			$this->mastermodel->uploadfile($bid,$number,$type);
			redirect("Masteradmin/managePriList");
		}
		$data['module']['title'] ="Upload Audio File";
		$data['nosearch'] = true;
		$header = array('Upload file'
						);
		$data['itemlist']['header'] = $header;
		$formFields = array();
		  $formFields[] = array(
				'label'=>'<label for="f">Upload File : </label>',
				'field'=>form_input(array(
						'name'      => 'greetings',
						'id'        => 'greetings',
						'type'      => 'file'
		...
		$data['form'] = array(
							'open'=>form_open_multipart('Masteradmin/upload_file/'.$bid.'/'.$number.'/'.$type,array('name'=>'Upload_file','class'=>'form','id'=>'Upload_file','method'=>'post')),
							'fields'=>$formFields,
							'close'=>form_close(),
							);
		$this->load->view('form_view',$data);
	...
	function getDND($eid,$bid){
		$dnd = $this->mastermodel->getDND($eid,$bid);
		echo $dnd;exit;
	}
        function addoffers(){
		if($this->input->post('update_system')){
			$res=$this->mastermodel->addnewoffers();
			if($res == true){
				$this->session->set_flashdata('msgt', 'success');
				$this->session->set_flashdata('msg','Offer added successfully');
				redirect('Masteradmin/addoffers');
			}else{
				$this->session->set_flashdata('msgt', 'success');
				$this->session->set_flashdata('msg','Offer Id already exists');
				redirect('Masteradmin/addoffers');
			}
		}
		$this->mastermodel->data['html']['title'] .= " |Add Offers";
		$data['module']['title'] = 'Add Offers';
		$formFields = array();$formFields1 = array();
		$cf=array('label'=>'<label class="col-sm-4 text-right">Add Offers</label>',
				  'field'=>form_input(array(
									  'name'        => 'offers',
										'id'          => 'offers',
										'value'       => '',
						...
						array_push($formFields,$cf);
			$data['form'] = array(
		       	   'form_attr'=>array('action'=>'Masteradmin/addoffers','id'=>'addoffers','name'=>'addoffers','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'fields1'=>$formFields1,
					'close'=>form_close()
				);
	...
	foreach($fieldset as $field){
			...
			if($field['type']=='s' && $field['show']){
						...
						if($field['fieldname']=='eid'){
							...
							$v = '';
							foreach($bday as $b => $d){ $v .= (isset($d->day) && $d->day=='1')?$b.'='.$d->st.'-'.$d->et.'<br>':'';}
						}elseif($field['fieldname']=='record'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
					    }elseif($field['fieldname']=='replytocust_missed'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='replytocust_attended'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='replytocust_repcal'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    //~ }elseif($field['fieldname']=='replytocust_voice'){
							//~ $v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='supportgrp'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='replytoexecutive'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='sameexe'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='replytocustomer'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='recordnotice'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='misscall'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='allgroup'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
								    }elseif($field['fieldname']=='pincode'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
							   }elseif($field['fieldname']=='replytocust_voitext'){
							$v = isset($itemDetail[$field['fieldname']])?'Yes':'No';
		...
		array_push($formFields,$cf);
		$data['form'] = array(
		            'form_attr'=>array('action'=>'Mastermodel/activerecords/'.$gid.'/'.$bid,'name'=>'groupdetails','id'=>'groupdetails','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'submit'=>'1',
					'close'=>form_close()
				);
 ...
 function businessEmployee($bid){
        $this->mastermodel->data['html']['title'] .= " | List Employee";
		$data['module']['title'] ="Business Employee List";
		$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;
		$limit = '30';
		$emp_list=$this->mastermodel->getAllEmployee($bid,$ofset,$limit);
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		foreach ($emp_list['data'] as $item){
		   $item['selfdisable'] = ($item['selfdisable'] == 1 ) ? 'Yes': 'No' ;
		   $item['status'] = ($item['status'] == 1 ) ? 'Enable':(($item['status'] == 3 ) ? 'Unconfirm' : (($item['status'] == 2 ) ? 'Delete' : 'Disable')) ;
		   $rec[] = array(
		         $sn_count++
		        ,$item['empname']
				,$item['empnumber']
		...
		$data['itemlist']['count'] = $emp_list['count'];
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/businessEmployee/'.$bid)
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>4					
				));
		...
		$formFields = array();
		$formFields[] = array(
				'label'=>'<label class="col-sm-4 text-right" for="f">Employee Name: </label>',
				'field'=>form_input(array(
						'name'      => 'empname',
						'id'        => 'empname',
						'class'     => 'form-control',
		...
		$formFields[] = array(
				'label'=>'<label  class="col-sm-4 text-right" for="f">Employee Email: </label>',
				'field'=>form_input(array(
						'name'    => 'empemail',
						'id'        => 'empemail',
						'class'     => 'form-control',
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/businessEmployee/'.$bid,array('name'=>'businessEmployee','class'=>'form','id'=>'businessEmployee','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
	...
	function mtrackEmp($bid){	
        if($this->input->post('update_system')){
			$mtracker_licence = $_POST['mtracker_licence'];
			if($mtracker_licence != '' && is_numeric($mtracker_licence)){
				$this->mastermodel->mtrackEmp($bid);
			}else{
				$this->session->set_flashdata('msgt', 'error');
				$this->session->set_flashdata('msg', "Please enter numerical value for MTracker Employee");
			}
			redirect("Masteradmin/Businesslist");
        }
		$this->load->model('configmodel');
	    $data['module']['title'] ="MTracker License";
	    $data['nosearch'] = true;
	    $header = array('MTracker License');
	    $itemDetail = $this->mastermodel->getmtrackEmp($bid);
        $data['itemlist']['header'] = $header;
	    $formFields = array();$formFields1 = array();
        $formFields[] = array(
                'label'=>'<label class="col-sm-4 text-right" for="f">License (Numbers): </label>',
                'field'=>form_input(array(
                        'name'      => 'mtracker_licence',
                        'id'        => 'mtracker_licence',
                        'type'      => 'text',
                        'value'		=> (isset($itemDetail['mtracker_licence']) ? $itemDetail['mtracker_licence'] : ''),
                        'class'		=>'form-control'
                        ))
                        );
        $formFields[] = array(
                'label'=>'<label class="col-sm-4 text-right" for="f">Validity (Audio Files): </label>',
                'field'=>form_input(array(
                        'name'      => 'mt_record_valid',
                        'id'        => 'mt_record_valid',
                        'type'      => 'text',
                        'value'		=> (isset($itemDetail['mt_record_valid']) ? $itemDetail['mt_record_valid'] : ''),
                        'class'		=>'required number form-control'
                        ))
                        );
	$data['form'] = array(
		       	    'form_attr'=>array('action'=>'Masteradmin/mtrackEmp/'.$bid,'id'=>'mtrackEmp','name'=>'mtrackEmp','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'fields1'=>$formFields1,
					'close'=>form_close()
				);
 ...
 function Checkemail(){
	 if($this->input->post('update_system')){
			$this->form_validation->set_rules('checkemail', 'checkemail', 'required');
			if(!$this->form_validation->run() == FALSE){	
						$res=$this->mastermodel->chech_email();
						$email = $res['data']['0']['contactemail'];
						if($res!= '0'){
							$this->session->set_flashdata('msgt', 'success');
							$this->session->set_flashdata('msg', 'Email Found');
						    redirect('Masteradmin/Checkemail_list');
					    }else{
							$this->session->set_flashdata('msgt', 'error');
						    $this->session->set_flashdata('msg', 'Email Not Found');
							redirect('Masteradmin/Checkemail');
						}
		...
		$data['module']['title'] = $this->lang->line('checkemail');
		$formFields = array();$formFields1 = array();
		$cf=array('label'=>'<label class="col-sm-4 text-right">'.$this->lang->line('businessemail').' :</label>',
				  'field'=>form_input(array(
									  'name'        => 'checkemail',
										'id'          => 'checkemail',
										'value'       => '',
						...
						array_push($formFields,$cf);
			$data['form'] = array(
		       	   'form_attr'=>array('action'=>'Masteradmin/Checkemail','id'=>'Checkemail','name'=>'Checkemail','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'fields1'=>$formFields1,
					'close'=>form_close()
				);
 ...
 function Checkemail_list(){
		...
		$data['itemlist']['header'] = $header;
		$emp_list=$this->mastermodel->getBusinesslist();
		$data['module']['title'] ="Business List "."[".$emp_list['count']."]";
		$rec = array();
		if(count($emp_list['data'])>0)
		foreach ($emp_list['data'] as $item){
			...
			$rec[] = array(
				...
				,$related,$Emp
				,$item['registrationdate']
				,'<a href="Masteradmin/editbusiness_email/'.$item['bid'].'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Edit" class="fa fa-edit"></span></a>');
		}
		$data['itemlist']['rec'] = $rec;
		$data['itemlist']['count'] = $emp_list['count'];
 ...
 function editbusiness_email($bid){
		if($this->input->post('update_system')){
			$res=$this->mastermodel->update_businessemail($bid);
			redirect('Masteradmin/Checkemail_list');
		}
		$this->load->model('configmodel');
		$data['module']['title'] = "Change Email";
		$fieldset = $this->configmodel->getFields('1',$bid);
		$formFields = array();
		$itemDetail = $this->configmodel->getDetail('1',$bid,'',$bid);
		foreach($fieldset as $field){
			$checked=false;
		if($field['type']=='s' && $field['show'] && !in_array($field['fieldname'],array('webaddress','contactname','registrationdate','language','businessaddress1','businessaddress','businessphone','businessname','contactphone','businessemail','country','accname','busadmdes','accphone','accemail','locality','apisecret','zipcode','city','state','bid'))){
						$cf = array('label'=>'<label class="col-sm-4 text-right">'.(($field['customlabel']!="")
												?$field['customlabel']
												:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']
										).' : </label>',
		...
		$data['form'] = array(
		            'form_attr'=>array('action'=>'Masteradmin/editbusiness_email/'.$bid,'name'=>'businessprofile','id'=>'businessprofile','enctype'=>"multipart/form-data"),
		            'hidden'=>array('email'=>$itemDetail['contactemail'],'uid'=>$itemDetail['uid'],'eid'=>$itemDetail['eid']),
					'fields'=>$formFields,
					'close'=>form_close()
				);
		$this->load->view('popupFormView',$data);
   ...
   function mtlicenceemp($bid){
        $this->mastermodel->data['html']['title'] .= " | List of MTracker Employee";
		$data['module']['title'] ="List of MTracker Employee";
		$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;
		$limit = '30';
		$emp_list=$this->mastermodel->getAllMT_Employee($bid,$ofset,$limit);
		 	//	echo "<pre>";	print_r($emp_list); exit;
		$header = array('SL No'
				        ,'Employee Name'
		                ,'Number'
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search"class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		foreach ($emp_list['data'] as $item){
		   $item['mcubecalls'] = ($item['mcubecalls'] == 1 ) ? 'Yes': 'No' ;
		   $item['recording'] = ($item['recording'] == 1 ) ? 'Yes': 'No' ;
		   $rec[] = array(
		         $sn_count++
		        ,$item['empname']
				,$item['empnumber']
		...
		$data['itemlist']['count'] = $emp_list['count'];
		$this->pagination->initialize(array(
						 'base_url'=>site_url('Masteradmin/mtlicenceemp/'.$bid)
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit	
						,'uri_segment'=>4					
				));
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Masteradmin/mtlicenceemp/'.$bid,array('name'=>'mtlicenceemp','class'=>'form','id'=>'mtlicenceemp','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
