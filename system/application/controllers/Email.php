...
class Email extends Controller {
	...
	function sent(){
		...
		$data['form'] = array(
			'open'=>form_open_multipart('SentEmails/0',array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
	...
	function compose($item_id='',$source=''){
		...
		$data['form'] = array(
			'open'=>form_open_multipart('Email/SentMail/',array('name'=>'compose1','class'=>'form','id'=>'compose1','method'=>'post'),
	...
	function SentMail(){
		...
		for($i=0;$i<sizeof($pieces);$i++){
			...
			switch($_POST['source']){
				case 'calls':
						$itemDetail_d = $this->configmodel->getDetail('6',$pieces[$i],'',$bid);
						break;
				case 'leads':
						$itemDetail_d = $this->configmodel->getDetail('26',$pieces[$i],'',$bid);
						break;	
				case 'support':
						$itemDetail_d = $this->configmodel->getDetail('40',$pieces[$i],'',$bid);
						break;	
				case 'ivrs':
						$itemDetail_d = $this->configmodel->getDetail('16',$pieces[$i],'',$bid);
						break;	
				case 'pbx':
						$itemDetail_d = $this->configmodel->getDetail('24',$pieces[$i],'',$bid);
						break;	
			}
			$message=str_replace('&lt;','<',$this->input->post('content'));
			$message=str_replace('&gt;','>',$message);
			$email=($_POST['source']!='calls')?$itemDetail_d['email']:$itemDetail_d['caller_email'];
			$this->email->to($email); 
			$this->email->subject($this->input->post('subject'));
			$this->email->message($message); 
			$status=$this->email->send();
			$res=$this->configmodel->sentmails($bid,$itemDetail['email'],$email,$status);
		}
	        if($status == '1'){
				...
				$this->session->set_flashdata('msg', " Mail Has encountered an error while sending");
				redirect('Email/compose');
			}
	}
	function Config(){
	...
	function listemplate(){
		...
		$data['module']['title'] ="List Template [".$data['itemlist']['count']."]";
		$data['paging'] = $this->pagination->create_links();
		$links = array(); 
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$formFields = array();
	...
	function list_smsemplate(){
		...
		$data['module']['title'] ="List SMS Template [".$data['itemlist']['count']."]";
		$data['paging'] = $this->pagination->create_links();
		$formFields = array();
		$cf=array('label'=>'<label class="col-sm-4 text-right" for="groupname">Template Name : </label>',
				  'field'=>form_input(array(
		...
		$data['links'] = $links;
		$data['form'] = array(
			'open'=>form_open_multipart('ManageSMSTemplate/0',array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
			'form_field'=>$formFields,
			'close'=>form_close(),
			'title'=>$this->lang->line('level_search')
			);
	...
	function template($id=''){
		...
		if($this->input->post('update_system')){
			...
			$this->session->set_flashdata('msgt','success');
			redirect('ManageEmailTemplate/0');
		}
		$data=array();
		$bid=$this->session->userdata('bid');
		...
		$formFields[]=array('label'=>'<label class="col-sm-4 text-right">Message :</label> ',
							'field'=>'<textarea name="content" id="content" class="form-control">'.$tval.'</textarea>'
										.display_ckeditor($this->data));
		$data['form'] = array(
		    'form_attr'=>array('action'=>'AddEmailTemplate/'.$id,'name'=>'Etemp','id'=>'Etemp','enctype'=>"multipart/form-data"),
			'fields'=>$formFields,
			'close'=>form_close(),
			);
		$this->sysconfmodel->viewLayout('form_view',$data);
	...
	function Smstemplate($id=''){
		...
		$data['form'] = array(
		    'form_attr'=>array('action'=>'AddSMSTemplate/'.$id,'name'=>'Etemp','id'=>'Etemp','enctype'=>"multipart/form-data"),
			'fields'=>$formFields,
			'close'=>form_close(),
			);
		$this->sysconfmodel->viewLayout('form_view',$data);
