...
class cronjob extends Controller {
			...
			foreach($busList as $bus){
				...
				if(count($empList)>0){
					foreach($empList as $emp){
						...
						<table align='left' border='1' width='100%'>
							<tr><th colspan='2'>LEGEND</th></tr>
							<tr><th align='left'>ANSWER</th><td>Call is answered. A successful dial. The caller reached the caller.</td></tr>
							<tr><th align='left'>BUSY</th><td>Busy signal. The dial command reached its number but the number is busy.</td></tr>
							<tr><th align='left'>NOANSWER</th><td>No answer. The dial command reached its number, the number rang for too long, then the dial timed out.</td></tr>
							<tr><th align='left'>CANCEL</th><td>Call is cancelled. The dial command reached its number but the caller hung up before the callee picked up.</td></tr>
							<tr><th align='left'>CONGESTION</th><td>Congestion. This status is usually a sign that the dialled number is not recognised.</td></tr>
						...
						$output .= "</table>\n"; 
						///////////////////////////////////////////
					   // Employee wise call for today
						$list = array();
						$list = $this->CM->weeklyEmpWise($bus['bid'],$emp['eid'],'daily');
						if(count($list)>0){
							$output .= "\t<tr>\n\t\t<td>\n";
							$output .= "\t\t\t<table border='1' width='100%'>\n";
							$output .= "\t\t\t\t<tr><th colspan='3'>Employee Wise calls</th></tr>\n";
						...
						///////////////////////////////////////////
						// Group wise call for today
						$list = array();
						$list = $this->CM->weeklyGrWise($bus['bid'],$emp['eid'],'daily');
						if(count($list)>0){
							$output .= "\t<tr>\n\t\t<td>\n";
							$output .= "\t\t\t<table border='1' width='100%'>\n";
							$output .= "\t\t\t\t<tr><th colspan='2'>Group Wise calls</th></tr>\n";
						...
						$output .= "<ul style='font-weight:bold;'>
						            <li>Do not miss a single enquiry,</li>
						            <li>Now you can track all your Incoming SMS,</li>
									<li>Reach out your customers using our 'Click to call',</li>
									<li>You can also manage mobile calls using MTracker (Android App).</li>
									<li>For enquiry please call / write us.</li>
									<h2>For New release list please <a href='https://mcube.vmctechnologies.com/NewRelease'>click here</a></h2>
									</ul>";
	...
	function weekly(){
		...
		if(count($busList)>0){
			foreach($busList as $bus){
				...
				$list = array();
				$list = $this->CM->weeklyEmpWise($bus['bid'],$emp='','weekly');
				if(count($list)>0){
					$x = '1';
					$output .= "\t<tr>\n\t\t<td>\n";
					$output .= "\t\t\t<table border='1' width='100%'>\n";
				...
				$list = array();
				$list = $this->CM->weeklyGrWise($bus['bid'],$emp='','weekly');
				if(count($list)>0){
					$x = '1';
					$output .= "\t<tr>\n\t\t<td>\n";
					$output .= "\t\t\t<table border='1' width='100%'>\n";
	...
	function ReportMails(){
		...
		if(count($busList)>0){
			foreach($busList as $bus){
				$empList = $this->CM->getEmp($bus['bid']);
				$calltrack = $this->CM->getCalls($bus['bid']);
				$ivrs = $this->CM->getCallsIvrs($bus['bid']);
				$pbx = $this->CM->getCallsPbx($bus['bid']);
				$leads = $this->CM->leadEmpCalls($bus['bid']);
				$leadgroups = $this->CM->leadGrpCalls($bus['bid']);
				$trackcalls = array();$ivrscalls = array();$pbxcalls = array();
				if(count($empList)>0){
					foreach($empList as $emp){
						$output = '';
						$roleDetail = $this->CM->empRole($emp['roleid'],$bus['bid']);
						if(count($calltrack) >0){
							$fieldset = $this->Config->getFields('6',$bus['bid']);
							foreach($calltrack as $call){
								array_push($trackcalls,$this->Config->getDetail('6',$call['callid'],'',$bus['bid']));
							}
							$keys = array();
							$header = array('Sl');
							foreach($fieldset as $field){
								...
								if($field['type']=='s' && !$field['is_hidden'] && $field['show']){
									foreach($roleDetail['system'] as $f){if($f['fieldid']==$field['fieldid'])$checked = true;}
									if($checked && in_array($field['fieldname'],array('callfrom','callername','starttime','endtime','dialstatus','gid','eid','filename','remark'))){
										array_push($keys,$field['fieldname']);
										array_push($header,(($field['customlabel']!="")
															?$field['customlabel']
															:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']));
						...
						if($ivrs['count'] >0){
							...
							foreach($ivrs['data'] as $call){
								array_push($ivrscalls,$this->Config->getDetail('16',$call['hid'],'',$bus['bid']));
							}
							$keys = array();
							$header = array('Sl');
							foreach($fieldset as $field){
						...
						if($pbx['count'] >0){
							...
							foreach($pbx['data']  as $call){
								array_push($pbxcalls,$this->Config->getDetail('24',$call['callid'],'',$bus['bid']));
							}
							$keys = array();
							$header = array('Sl');
							foreach($fieldset as $field){
	...
	function mtrackerArchive(){
		$this->CM->mtArchive();
	}
	

	function deleteFiles_mtArchive(){
	    $busList = $this->CM->getAllBusiness();
	    $recordlimit = array();
		$recordlimit = $this->CM->recordlimit();
			for($i=1; $i <= $recordlimit['count'];$i++){
			   $recordvalid = $this->CM->getvalidrecord($recordlimit);
			}
		}   
