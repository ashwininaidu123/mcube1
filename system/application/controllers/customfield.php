...
class Customfield extends controller
{
	...
	function manageregion(){
		...
		foreach ($emp_list['data'] as $item){
			$rec[] = array($i
				...
				.'<a href="'.base_url().'customfield/Delete_Region/'.$item['regionid'].'" class="deleteClass" title="Are you sure to Delete the Region"><span title="Bulk Delete" class="glyphicon glyphicon-trash"></span></a>'
				.' <a href="'.base_url().'AddGrpRegion/'.$item['regionid'].'/0"><span class="fa fa-sitemap"  title="Add Group Region"></span></a>
		...
		$links = array();
		$links[]='<li><a href="AddRegion/0"><span title="Add Region" class="glyphicon glyphicon-plus-sign">&nbsp;AddRegion</span></a></li>';
	...
	function View_GroupRegion($rid){
		...
		$links = array();
		$links[]="<li><a href='".$url."'><span title='Add Region' class='glyphicon glyphicon-plus-sign'>&nbsp;AddRegion</span></a></li>";
		$formFields = array();
		$formFields[] = array(
	...
	function auditlog(){
		...
		if($this->input->post('submit')){	
			...
			if($this->input->post('modulename')!=""){
				if($this->session->userdata('modulename')!=""){
					$this->session->unset_userdata('modulename');
				}
				$this->session->set_userdata('modulename',$this->input->post('modulename'));
			}else{
				$this->session->unset_userdata('modulename');
			}
		...
		$links = array();
		$links[] ='<li><a href="customfield/auditDown/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$formFields1 = array();
		...
		$formFields[] = array(
				...
				'field'=>form_input(array(
						...
						'class'        => 'form-control',
						'id'        => 'modulename',
						'value'     => $this->session->userdata('modulename')
						)));			
		$formFields[] = array(
