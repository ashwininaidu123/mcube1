...
class Report extends controller
{
	...
	function undeleteCalls(){
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('DeleteReport/0/')
		...
		$links[] = ($opt_delete) ? '<li><a  class="blkunDeletecall" href="'.base_url().'Report/blkUnDel_calrep"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;BulkUndelete</span></a></li>':'';
		$data['links'] = $links;
		$fieldset = $this->configmodel->getFields('6',$bid);
		$formFields = array();
		foreach($fieldset as $field){
			$checked = false;
			if($field['type']=='s' && $field['show'] && !in_array($field['fieldname'],array('assignto','calleraddress','callerbusiness','region','rate','exefeedback','custfeedback','dialstatus'))){
										...
										($field['fieldname']=='gid')
											...
											:(($field['fieldname']=='eid')
												...
												:(($field['fieldname']=='pulse')
													...
													'name'      => $field['fieldname'],
													'id'        => $field['fieldname'],
													'class'		=>($field['fieldname']=="starttime" || $field['fieldname']=="endtime")?'datepicker form-control':'form-control'
													))
												)
											)
						);
		...
		$data['form'] = array(
					'open'=>form_open_multipart(site_url('DeleteReport/0/'),array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
	...
	function call(){
	   ...
	   if(!$this->feature_access())redirect('Employee/access_denied');
		...
		$url="all";
		if($this->uri->segment(2) == 'live') {
			$url = "live";
		}
		if($this->uri->segment(1)=='GrpTrackReport'){
			$_POST['submit']= 'submit';	
			$_POST['gid']=$this->uri->segment(2);
		}elseif($this->uri->segment(1)=='EmpTrackReport'){
			$_POST['submit']= 'submit';
			$_POST['empid']=$this->uri->segment(2);
			...
			$url="at";
		}elseif($this->uri->segment(1)=='PositiveTrackReport'){
			$url="p";
		}elseif($this->uri->segment(1)=='NegativeTrackReport'){
			$url="n";
		}
		if($this->input->post('graphchart')){
		    //$gettimelimit = $this->sysconfmodel->getlimit();
			$data1['module']['title'] = 'Call Analytics';
			$data1['callbyregion']=$this->reportmodel->callbyregion($url,$gettimelimit='');
			$data1['groupname'] = $this->reportmodel->recent_calls($url,$gettimelimit='');
			$data1['groups']=$this->reportmodel->groupnames($url,$gettimelimit='');
			$data1['callbyweek']=$this->reportmodel->callbyweek($url,$gettimelimit='');
			$data1['callbyemployee']=$this->reportmodel->callbyemployee($url,$gettimelimit='');
			$data1['callbytime'] = $this->reportmodel->callbytime($url,$gettimelimit='');
		    $this->sysconfmodel->viewLayout('calltrackeranalytics',$data1);
		   	return true;
	    }
		$listingarray=array(
				...
				"p"=>$this->lang->line('level_Report_positive'),
				"n"=>$this->lang->line('level_Report_negative'),
				"emp"=>$this->lang->line('level_Employeewise'),
				"group"=>$this->lang->line('level_Recent_calls')
			);
		if(!empty($listingarray[$this->uri->segment(3)])){
			 $heading=$listingarray[$url]; 
		}else{ 
			$heading=$this->lang->line('level_Report');
		}
		...
		if($this->input->post('download')){
			...
			$filename = $data['itemlist']['search_download'];
			if($filename!='0'){
			   $dlink =  "<a href='".$this->config->item('reports_path').$filename.".zip"."' target='_blank' style='color:#fff'><b>Search Download</b></a>  ";
		    }
		}
		$data['downlink'] = $dlink;
		...
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search">&nbsp;Search</span></a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-advsearch" data-toggle="modal" data-target="#modal-advsearch" ><span title="Search" class="glyphicon glyphicon-zoom-in">&nbsp;AdvanceSearch</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = ($roleDetail['modules']['6']['opt_download']!=0) ? '<li><a href="Report/Bulk_down/'.$csv_link1.'/'.$csv_link.'" class="blk_calls" data-toggle="modal" data-target="#modal-pop"><span title="Download" class="glyphicon glyphicon-arrow-down">&nbsp;DownloadSelect</span></a></li>':'';
		$links[] = ($roleDetail['modules']['6']['opt_download']!=0) ? '<li><a href="Report/incomingcalls_csv/'.$csv_link1.'/'.$csv_link.'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>':'';
		$fieldset = $this->configmodel->getFields('6',$bid);
		$formFields = array();
		$advsearch = array();
		foreach($fieldset as $field){
		...
		if(isset($_POST['search'])){
			if($_POST['search'] == 'search'){
				$data['systemfields'] = $this->configmodel->getFields('6',$bid);
				$data['roleDetail']=$this->roleDetail;
				$this->load->view('search_view',$data);
				return true;
			}
	...
	function callarchive($y='0',$m='0'){
		...
		$data['module']['title'] ='Call Archive' . "[".$data['itemlist']['count']."]";
		$this->pagination->initialize(array(
	...
	function edit($id){
		...
		if($roleDetail['role']['admin'] != 1 ){
			if($roleDetail['role']['roleid'] != 4){
				 $empcheck = $this->reportmodel->emp_check1($id);
		         if(!in_array($this->session->userdata('eid'), $empcheck))
		         redirect('Employee/access_denied');
			}
		}
		...
		$itemDetail = $this->configmodel->strip_slashes_recursive($itemDetail);
		foreach($fieldset as $field){
			$checked=false;
			if($field['type']=='s' && !$field['is_hidden'] && $field['show']){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
		...
		array_push($formFields,$cf);
		$cf = array('label'=>'<label class="col-sm-4 text-right">Follow Up : </label>',
					'field'=>anchor("Report/followup/".$itemDetail['callid']."/0/calltrack", '<span title="Followups" class="glyphicon glyphicon-book"></span>','class="btn-followup" data-toggle="modal" data-target="#modal-followup"')
					);
   ...
   function archive_records($id=''){
		...
		$fieldset = $this->configmodel->getFields('6',$bid);
		$formFields = array();
		$itemDetail = $this->configmodel->getDetail('callarchive',$id,'',$bid);
			//echo '<pre>';print_r($itemDetail); exit;
		foreach($fieldset as $field){
			$checked=false;
			if($field['type']=='s' && !$field['is_hidden'] && $field['show']){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
					if($checked   && !in_array($field['fieldname'] ,array('last_modified'))){
						$cf = array('label'=>'<label class="col-sm-4 text-right">'.(($field['customlabel']!="")
												?$field['customlabel']
												:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']
										).' : </label>',
		...
		$data['form'] = array(
		'open'=>form_open_multipart('Report/archive_records/'.$id
									,array('name'=>'callarchive','id'=>'callarchive','class'=>'form','method'=>'post')
									),
					'fields'=>$formFields,
					'close'=>form_close()
				);
	...
	function followup($id='',$dsh = '',$type=''){
		...
		if($dsh == 1){
			...
			$formFields[] =array("label"=>"<label for='alert' class='col-sm-4 text-right'>Alert :</label>"
								,"field"=>form_dropdown("alert",$array,'',"class='form-control' id='alert'")
								);
	...
	function missed(){
		...
		$this->pagination->initialize(array(
				...
				$links = array();
				$links[]='<li><a href="Report/incomingcalls_csv/m" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>';
				$fieldset = $this->configmodel->getFields('6');
				$formFields = array();
				foreach($fieldset as $field){
					$checked = false;
						...
						if($checked) $formFields[] = array(
											...
											'field'=>($field['fieldname']!='eid'&& $field['fieldname']!='gid')
													?form_input(array(
															'name'      => $field['fieldname'],
															'id'        => $field['fieldname'],
															'class'		=>($field['fieldname']=="starttime" ||$field['fieldname']=="endtime")?'datepicker':''))
													:(($field['fieldname']=='gid')?
	...
	function AttendCalls(){
		...
		$this->pagination->initialize(array(
				...
				$links = array();
				$links[]='<li><a href="Report/incomingcalls_csv/at" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>';
				$fieldset = $this->configmodel->getFields('6');
				$formFields = array();
				foreach($fieldset as $field){
					$checked = false;
	...
	function EmployeeCallReport($eid){
		...
		$links = array();
		$links[]='<li><a href="Report/incomingcalls_csv/a/'.$eid.'" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>';
		$fieldset = $this->configmodel->getFields('6');
		$formFields = array();
		foreach($fieldset as $field){
			$checked = false;
	...
	function unqualified(){
		...
		$links = array();
		$links[]='<li><a href="Report/incomingcalls_csv/u" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>';
		$fieldset = $this->configmodel->getFields('6');
		$formFields = array();
		foreach($fieldset as $field){
			$checked = false;
	...
	function qualified(){
		...
		$this->pagination->initialize(array(
				...
				$links =array();
				$links[]='<li><a href="Report/incomingcalls_csv/q" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span> </a></li>';
				$fieldset = $this->configmodel->getFields('6');
				$formFields = array();
				foreach($fieldset as $field){
					$checked = false;
	...
	function smsreport(){
		...
		$data['links']= $links;				
		$data['form'] = array(
			'open'=>form_open_multipart('SMSReport/0',array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
			'form_field'=>$formFields,
			'adv_search'=>array(),
			'save_search'=>3,
			'close'=>form_close(),
	...
	function vbroadcast(){
		...
		$links = array();
		$links[]='<li><a href="Report/voice_csv" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>';
		$formFields[] = array(
				'label'=>'<label class="col-sm-4 text-right" for="f">'.$this->lang->line('level_number').' : </label>',
				'field'=>form_input(array(
						'name'      => 'vnumber',
	...
	function listcontacts(){
	   ...
	   '<li><a href="Report/ContactsCsv/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>':'';
		$rec = array();
		if(count($emp_list['data'])>0)
		$i = $ofset+1;
		foreach ($emp_list['data'] as $item){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .=($opt_add)? '<li><a href="Report/Editcontact/'.$item['number'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .=($opt_delete) ? '<li><a href="Report/deletContact/'.$item['contid'].'"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= '<li>'.anchor("Report/clicktoconnect/".$item['number']."/5", '<span title="click To Connect" class="fa fa-phone">&nbsp;ClicktoConnect</span>',array('class'=>'clickToConnect')).'</li>';
				$act .= '<li>'.anchor("Report/sendSms/".$item['number']."/contact", ' <span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;SendSMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"').'</li>';	
				$act .= '</ul></div>';
				$arrs['action'] = $act;
			}
			foreach($keys as $k){
				...
				if($checked && !in_array($field['fieldname'] ,array('greetings','bday','hdaytext','hdayaudio','operator','prinumber','record','remark','noext'))){ 
					$formFields[] = array(
									...
									'field'=>form_input(array(
											'name'      => $field['fieldname'],
											'id'        => $field['fieldname'],
											'class'		=>($field['fieldname']=="endtime" ||$field['fieldname']=="datetime")?'datepicker':'form-control'))
											);
							$advsearch[$field['fieldname']]=(($field['customlabel']!="")
											 ?$field['customlabel']:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']);				
						}					
	...
	function click2call(){
		...
		if($this->input->post('download')){
			$filename = $this->reportmodel->click2calls_csv($bid);
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('C2GroupReport/0/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
						,'uri_segment'=>3));
	   	$links = array();
		$links[]=($roleDetail['modules']['25']['opt_download']!=0) ? '<li><a href="Report/click2calls_csv/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>':'';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
	...
	function outbound_calls(){
	    ...
	    $data['itemlist']  =$this->reportmodel->outbound_calls_list($bid,$ofset,$limit);
		$data['module']['title'] ='Click2Connect '. "[".$data['itemlist']['count']."]";
		$this->pagination->initialize(array(
		...
		$data['paging'] = $this->pagination->create_links();
		$this->sysconfmodel->data['html']['title'] .= " | Click2Connect";
		$links = array();
		$links[] = '<li><a href="Report/callMe/" class="btn-danger show-dialler" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-th" title="Click To Connect">&nbsp;Dialer</span></a></li>';
		$links[] = '<li><a href="Report/outbound_calls_csv/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
	    ...
	    foreach($fieldset as $field){
			$checked = false;
			if($field['type']=='s' && $field['show'] && !in_array($field['fieldname'],array('callid'))){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
				if($checked) { $formFields[] = array(
									'label'=>'<label class="col-sm-4 text-right" for="'.$field['fieldname'].'">'.(($field['customlabel']!="")
											 ?$field['customlabel']:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']).' : </label>',
									'field'=>
										($field['fieldname']=='status')
											?form_dropdown('stat',$status,'',"class='form-control'")
											:(($field['fieldname']=='modid')
												?form_dropdown('source',$source_arr,'',"class='form-control'")
												:(($field['fieldname']=='pulse')
													?
													form_dropdown('ptype',array(
																				'>'=>' > ',
	...
	function clk2connectedit($id){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$roleDetail = $this->roleDetail;
		if(!$roleDetail['modules']['58']['opt_add']) redirect('Employee/access_denied');
		if($this->input->post('update_system')){
			$res=$this->reportmodel->clicktoconnectedit($id,$bid);
			$this->session->set_flashdata('msgt', 'success');
			$this->session->set_flashdata('msg', "Call Updated Successfully");
			redirect('C2CReport/0');
		}
		$data['module']['title'] = 'Click2Connect';
		$fieldset = $this->configmodel->getFields('58');
		$formFields = array();
		...
		$status=array(
			...
			$checked=false;
			$srcval = isset($source_arr[$itemDetail['modid']]) ? $source_arr[$itemDetail['modid']] : '';
			$state = isset($status[$itemDetail['status']]) ? $status[$itemDetail['status']] : '';
		    if($field['type']=='s' && $field['show'] && !in_array($field['fieldname'],array('callid'))){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
						if($checked && !in_array($field['fieldname'] ,array('callstatus'))){ 
						$cf = array('label'=>'<label class="col-sm-4 text-right" for="'.$field['fieldname'].'">'.(($field['customlabel']!="")
											 ?$field['customlabel']:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']).' : </label>',
									'field'=> (($field['fieldname']=="modid")?$srcval
											 :(($field['fieldname']=="status")?$state
											 : (($field['fieldname']!="name" 
												&& $field['fieldname']!="business" 
												&& $field['fieldname']!="assignto" 
												&& $field['fieldname']!="address" 
												&& $field['fieldname']!="email" 
												&& $field['fieldname']!="remark")?(isset($itemDetail[$field['fieldname']])?$itemDetail[$field['fieldname']]:'')
											 :(($field['fieldname']=="remark")?form_textarea(array(
												  'name'      => $field['fieldname'],
												  'id'        => $field['fieldname'],
												   'class'     => 'form-control',
												   'value'     => isset($itemDetail[$field['fieldname']])?$itemDetail[$field['fieldname']]:''))
	...
	/* convert lead and support ticket start */
		$lead_access = $this->sysconfmodel->getfeatureAccess('13');
		if($lead_access == 1){
			$leadstchk = $this->reportmodel->leadstatuschk($itemDetail['customer'],$bid);
			if(empty($leadstchk) ){
				$leadtype = $this->sysconfmodel->leadtypeCheck();
				if($leadtype == 1 || $leadtype == 3){
					$cf=array('label'=>'<label class="col-sm-4 text-right">Convert As Lead :</label>	'
		...
		/*  End of Conversion */
		$data['form'] = array(
		            'form_attr'=>array('action'=>'EditC2CReport/'.$id,'name'=>'editclick2connect','id'=>'editclick2connect','enctype'=>"multipart/form-data"),
					'fields'=>$formFields,
					'hidden'=>array('bid'=>$bid,'number'=>$itemDetail['customer']),
					'close'=>form_close()
				);
		$this->sysconfmodel->viewLayout('form_view',$data);
	}
	...
	function undeleteC2Calls(){
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('DeleteC2CReport/0')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
						,'uri_segment'=>3				
				));
		...
		foreach($fieldset as $field){
			$checked = false;
			if($field['type']=='s' && $field['show'] && !in_array($field['fieldname'],array('filename','status'))){
				foreach($roleDetail['system'] as $ret){if($ret['fieldid']==$field['fieldid'])$checked = true;}
				if($checked) $formFields[] = array(
									'label'=>'<label class="col-sm-4 text-right" for="'.$field['fieldname'].'">'.(($field['customlabel']!="")
											 ?$field['customlabel']:$this->lang->line('mod_'.$field['modid'])->$field['fieldname']).' : </label>',
		...
		$data['form'] = array(
					'open'=>form_open_multipart(site_url('DeleteC2CReport/0/'),array('name'=>'manageemp','class'=>'form','id'=>'manageemp','method'=>'post')),
					'form_field'=>$formFields,
					'adv_search'=>array(),
					'parentids'=>$parentbids,
					'busid'=>$bid,
	...
	function callanalytics(){
		...
		$data['callbytime'] = $this->reportmodel->callbytime($data,$gettimelimit='');
		$data['callbyweek'] = $this->reportmodel->callbyweek($data,$gettimelimit='');
		//$data['callbyEmotion'] =$this->reportmodel->callbyEmotion();
		$data['area'] = $this->reportmodel->arealist($bid,$data);
		$this->sysconfmodel->viewLayout('callanalytics',$data);
	}
	function callbyEmotion(){
		srand((double)microtime()*1000000);
		$res=$this->reportmodel->callbyEmotion();
		$data = array();
		$region = array();
		for( $i=0; $i<5; $i++ ){
		  $data[] = rand(5,15);
		...
		$g->title( 'Calls by Emotion', '{font-size:18px; color: #d01f3c}' );
		echo $g->render();
	}
	...
	function clicktoconnect($callid,$module){
		...
		$ret = '0';
		if($module!='' && $callid != 0){
			$ret = $this->reportmodel->getDetail($bid,$callid,$module,$eid);
		}
		echo $ret;
	}
	function followupreport(){
		...
		$data['module']['title'] ='Followups['.$data['itemlist']['count'].']';
		$links = array();	
		$links[] ='<li><a href="Report/followupDownload/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$formFields = array();
		$advsearch = array();
		...
		$formFields[]=array('label'=>'<label class="col-sm-4 text-right" for="followupdate">Followup Date: </label>',
				  'field'=>form_input(array(
									  'name'        => 'followupdate3',
										'id'        => 'followupdate3',
										'class'		=> 'datetimepicker form-control',	
										'value'     => $this->session->userdata('followupdate3'))));
		...
		$data['form'] = array(
					'open'=>form_open(site_url('FollowupReport/0'),array('name'=>'followups','class'=>'form','id'=>'followups','method'=>'post')),
					'form_field'=>$formFields,
					'adv_search'=>array(),
					'save_search'=>3,
					'parentids'=>$parentbids,
	...
	function blkSms($source=''){
	    ...
	    $chk_cltdnd = $this->configmodel->clientdnd($bid);
		if($this->input->post('update_system')){
			...
			$dnds=0;
			foreach($to as $to){
			if($chk_cltdnd['clientsms'] == 1 && $chk_cltdnd['allowdnd'] == 1){
				$reply=sms_send($to,$_POST['sms_content'],$senderid,'sendsms');
	...
	function sendSms($calid,$source){
		...
		switch($source){
			...
			case 'campaign':
					...
					break;
			case 'mtracker':
					$callDetail=$this->configmodel->getDetail('51',$calid,'',$bid);
					$to=$callDetail['callto'];
					break;
		}
	...
	function Send_SMS(){
		...
		$this->load->helper('mcube_helper');
		if($chk_cltdnd['clientsms'] == 1 && $chk_cltdnd['allowdnd'] == 1){
			$reply=sms_send($_POST['to'],$_POST['sms_content'],$senderid,'sendsms');
			$fp = fopen("smslog.txt","a");fwrite($fp,"\n".'['.date('Y-m-d H:i:s').'] : '.'SMS:'.$reply);fclose($fp);
			$dnd_status=0;
		}else{
		if($this->session->userdata('dnd_status')!=0){
	...
	function outbound_calls_csv($eid=''){
	    ...
	    $data=array('systemfields'=>$this->configmodel->getFields('58'),
					'roleDetail'=>$this->roleDetail,
					'eid'=>($eid!="")?$eid:'');
	...
	function empBreakHis(){
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('EmpBreakHis/0/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
						,'uri_segment'=>3				
				));
		$data['module']['title'] =$heading . ' ['.$data['itemlist']['count'].']';
		$links = array();
		$links[] = '<li><a href="Report/breakHisDownload/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span> </a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
	...
	function blkUnDel_calrep(){
		$rst = $this->reportmodel->blk_del_callrep($_POST['callid']);
	    redirect('TrackReport/all');
	}
	...
	function callsummary(){
		...
		$links = array();
		$links[] ='<li><a href="Report/callsumDownload/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$formFields = array();
		...
		$formFields[] =array('label'=>'<label class="col-sm-4 text-right" for="groupname">Select Group : </label>',
				  'field'=>form_dropdown('gid',$this->systemmodel->CSGroups(),isset($search['gid']) ? $search['gid'] : '',"class='form-control'")
				 );
		$data['downlink'] = $dlink;
		$data['links'] = $links;
		$data['form'] = array(
	...
	function outbound(){
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('OutboundReport/0/')
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
						,'uri_segment'=>3				
				));
		...
		$link = array();	
		$links[] ='<li><a href="Report/obDownload/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		...
		$data['downlink']= $dlink;
		$data['form'] = array(
					'open'=>form_open_multipart(site_url('OutboundReport/0/'),array('name'=>'outboundcall','class'=>'form','id'=>'outboundcall','method'=>'post')),
					'form_field'=>$formFields,
					'parentids'=>$parentbids,
					'adv_search'=>array(),
					'busid'=>$bid,
	...
	function callMe(){
		...
		$formFields = array();
		$data['form'] = array(
						'open'=>form_open_multipart('Report/clicktoconnect/0/9'
		...
		//~ $this->load->view('c2cdailer',$data);
		 $this->load->view('dailer',$data);
	}

  function C2CArchive($y='0',$m='0'){
		if(!$this->feature_access())redirect('Employee/access_denied');
		$roleDetail = $this->roleDetail;
		if(!$roleDetail['modules']['6']['opt_view']) redirect('Employee/access_denied');
		if($this->input->post('submit')){	
		...
		$data['module']['title'] ='Click2Connect Archive';
		$ofset = ($this->uri->segment(4)!=null)?$this->uri->segment(4):0;
		$limit = '30';
		$data['itemlist'] = $this->reportmodel->getc2cArchivelist(
												array('bid'		=> $bid,
													  'ofset'	=> $ofset,
													  'limit'	=> $limit,
													  'year'	=> $y,
		...
		$this->pagination->initialize(array(
						 'base_url'=>site_url('C2CArchive/'.$y.'/'.$m)
						,'total_rows'=>$data['itemlist']['count']
						,'per_page'=>$limit		
						,'uri_segment'=>4				
				));
		...
		if(count($data['itemlist']['data'])>0)
		$i=$ofset+1;
		foreach ($data['itemlist']['data'] as $item){
		...
		$formFields = array();
			//		echo "<pre>"; print_r($data); exit;
		$data['form'] = array(
					'open'=>form_open(site_url('C2CArchive/'.$y.'/'.$m),array('name'=>'C2CArchive','class'=>'form','id'=>'C2CArchive','method'=>'post')),
					'form_field'=>$formFields,
					'adv_search'=>array(),
					'close'=>form_close(),
					'parentids'=>$parentbids,
	...
	function c2carchive_download($y='0',$m='0'){
		$args['year']=($y!='0')?$y:date('Y');
		$args['month']=($m!='0')?$m:date('m');
		$args['bid']=$this->session->userdata('bid');
		$list = $this->reportmodel->c2carchiveDownload($args);
		redirect(base_url()."downloads.php?file=".$list);
	}
	function showresp($id=''){
		...
		$formFields=array();
		$data['itemlist'] = $this->reportmodel->apiRes($id,$bid);
		$this->load->view('counter_view',$data);
	}
   function calltrackanalytics(){
		$cbid=$this->session->userdata('cbid');
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		if($this->input->post('submit')){
				$this->session->set_userdata('filter',$_POST);
		...
		$data['module']['title'] = "CallTracker Analytics";
		$data['callbytime'] = $this->reportmodel->callbytime($data);
		$data['callbyregion']=$this->reportmodel->callbyregion($data);
		$data['groupwise_missedcalls'] = $this->reportmodel->for_pieinfo('m');
		$data['groupname'] = $this->reportmodel->recent_calls();
		$data['groups']=$this->reportmodel->groupnames();
	    $data['callbyweek']=$this->reportmodel->callbyweek($data);
	    $data['callbyemployee']=$this->reportmodel->callbyemployee($data);
		$this->load->view('calltrackeranalytics',$data);
   }
  ...
  function CallbackUser(){ 
		...
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$eid =$this->session->userdata('eid');
		$data['empDetail'] = $this->reportmodel->empDetail($eid);
	 	if($this->input->post('update_system')){
				$res=$this->reportmodel->callbackuser();
				if($res == true){
					$this->session->set_flashdata('msgt', 'success');
		...
		$data['module']['title'] = 'Call Back';
		$this->load->view('callback',$data);
  }
   function facebook(){
	  		$data['module']['title'] = 'Support on Messanger';
			$this->sysconfmodel->viewLayout('facebook',$data);
	}
   function mcube_products(){
	  $data['roleDetail'] = $this->roleDetail;
	  $data['module']['title'] = "MCube Products List";
	  $this->sysconfmodel->data['html']['title'] .= " | MCube Products";
      $this->sysconfmodel->viewLayout('mcubeproducts',$data);
	}
}
