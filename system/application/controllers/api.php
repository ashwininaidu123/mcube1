...
class Api extends Controller {
	...
	//~ }
	  ...
	  function getSMS(){
                //print_r($_REQUEST);
           $_REQUEST['time'] = isset($_REQUEST['time']) ? $_REQUEST['time'] : date('Y-m-d H:i:s');
           $this->apimodel->smsLog($_REQUEST);
          if($_REQUEST['to'] == 8088612345){
			    $to = 'support@vmc.in';
 		        $message=$_REQUEST['text'];
				...
				$this->email->message($message);
				$this->email->send();
				$ret = array('msg'=>'VMC Landingnumber'
                                     ,'code'=>'200'
                            );
        ...
        // if(!preg_match('/^[7-9][0-9]{9}$/',substr($_REQUEST['from'],-10,10))) exit;
                $priDetail = $this->apimodel->getPRI(substr($_REQUEST['to'],-10,10));
                //print_r($priDetail);
                if(isset($priDetail['status']) && $priDetail['status']=='1' && $priDetail['type']=='0' && $priDetail['associateid']>'0'){
                        $data = $_REQUEST;
                        $data['bid'] = $priDetail['bid'];
                        $data['gid'] = $priDetail['associateid'];
                        $data['number'] = $priDetail['number'];
                        $this->apimodel->smsAssign($data);
                        $ret = array('msg'=>'Assigned'
                                     ,'code'=>'200'
                                );
                }else{
                        $ret = array('msg'=>'Landing Number not found','code'=>'201');
                }
                echo json_encode($ret);
        }
	}
	/***************************************************************************************************/
	function empstatus_api(){
		$check_authenticate=$this->user_authenticate(
												urldecode($_REQUEST['username'])
											  ,urldecode($_REQUEST['password']));
		if(!empty($check_authenticate)){
			...
			if($role_detail!=0){
				$roledetail=$this->apimodel->getRoledetail($role_detail,$check_authenticate->bid);
				if($roledetail['modules']['2']['opt_add']!=0){
					$res = $this->apimodel->ChangeEmpStatus($check_authenticate->bid);
					if($res == '1'){
						$out=array("msg"=>"Employee Disabled successfully");
					}else{
						$out=array("msg"=>"Employee Enabled successfully");
					}
				}else{
					$out=array("msg"=>"Access Denied While changing Employee status");
				}
			}else{
				$out=array("msg"=>"Access Denied While changing Employee status");
			}
