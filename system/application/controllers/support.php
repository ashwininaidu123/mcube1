...
class Support extends controller
{
	...
	function listSupportGrp($type=''){
		...
		$data['links'] = $links;
		$data['form'] = array(
					'open'=>form_open_multipart(site_url($this->uri->segment(1).'/0'),array('name'=>'listsupgrp','class'=>'form','id'=>'listsupgrp','method'=>'post')),
	...
	function listGrpEmp($sgid){
		...
		if(count($emp_list['data'])>0){
			...
			foreach ($emp_list['data'] as $item){
				...
				$rec[] = ($item['status']=="1") ? $item['groupname'] : '<font color=red>'.$item['groupname'].'</font>'; 
				if(isset($gpDetail['group_rule']) && $gpDetail['group_rule'] == 2)
					$rec[] = ($item['status']=="1" && $item['estatus']=='1') ? $item['weight'] : '<font color=red>'.$item['weight'].'</font>';	
		...
		$links = array();
		$links[] = '<li><a href="AddempSupGroup/'.$sgid.'"><span title="Add Employee to Support Group" class="glyphicon glyphicon-plus-sign">&nbsp;AddEmployee</span></a></li>';
		$links[] =	'<li><a href="support/refreshcounter/'.$sgid.'"> <span title="Reset Counter" class="fa fa-refresh">&nbsp;ResetCounter</span></a></li>';
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search" >&nbsp;Search</span></a></li>';
		$formFields = array();
	...
	function listSupportTkt($type=''){
		...
		if($this->input->post('graphchart')){
		   ...
		   $data1['sup_recent_calls'] = $this->reportmodel->support_groupwisesupports($type,$gettimelimit='');	
		   $data1['support_emp'] = $this->reportmodel->support_emp($type);
		   $data1['support_status'] = $this->reportmodel->support_status($type);
		   $data1['support_source'] = $this->reportmodel->support_source($type);
		   $data1['support_level'] = $this->reportmodel->support_level($type);
		   $this->sysconfmodel->viewLayout('supportanalytics',$data1);
		   return true;
	    }	
		...
		$links = array();
		$links[] = '<li><a href="support/addSupportTkt/'.$type.'"><span title="Add Ticket" class="glyphicon glyphicon-plus-sign">&nbsp;AddTicket</span></a></li>';
		...
		$links[] = '<li  class="divider"><a>&nbsp;</a></li>';
		$links[] = ($roleDetail['modules']['40']['opt_download']!=0)? '<li><a href="support/Bulk_down/" class="blk_calls" data-toggle="modal" data-target="#modal-pop"><span title="Download" class="glyphicon glyphicon-arrow-down">DownloadSelect</span></a></li>':'';
		$links[] = ($roleDetail['modules']['40']['opt_download']) ? '<li><a href="support/supportTktCSV/" class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span title="Download" class="glyphicon glyphicon-download-alt">DownloadAll</span></a></li>':'';
	...
	function editSupportTkt($id){
		...
		$cf = array('label'=>'<label class="col-sm-4 text-right">Remarks History : </label>',
					'field'=>anchor("support/remarksHistory/".$id, '<span title="Support Ticket Remarks" class="glyphicon glyphicon-asterisk"></span>','class="btn-followup" data-toggle="modal" data-target="#modal-followup"')
					);
		array_push($formFields,$cf);
		...
		$cf = array('label'=>'<label class="col-sm-4 text-right">Followup : </label>',
					'field'=>anchor("Report/followup/".$id."/0/support", ' <span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span>','class="btn-followup" data-toggle="modal" data-target="#modal-followup"')
					);
		array_push($formFields,$cf);
