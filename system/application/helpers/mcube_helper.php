...
if ( ! function_exists('editor'))
{
	...
	function sms_send($number,$message,$senderId='VMCIND',$type =''){
		$ci=&get_instance();
		$ci->load->database();
		$cbid = $ci->session->userdata('cbid');
		$bid = (isset($cbid) && $cbid!="")?$cbid:$ci->session->userdata('bid');
		$sql = $ci->db->query("SELECT * FROM smsconfig WHERE bid='".$bid."'");
		$row = (array)$sql->row();
		$vmcallow = array('password'=>1,'otp'=>1,'code'=>1);
		if($type != '' && (array_key_exists($type, $row) || array_key_exists($type,$vmcallow))){
			if($row[$type] == 1 ){
				//$api = ($row['clientsms'] == 1) ? $row['apiurl'] : "http://115.249.28.90/sms/sendSMS.php?from=".$senderId;
				$api = ($row['clientsms'] == 1) ? $row['apiurl'] : "http://115.249.28.90/sms/sendSMS.php?from=".$senderId."&to=@number@&text=@text@";
			}
			if($vmcallow[$type] == 1){
				//$api = "http://115.249.28.90/sms/sendSMS.php?from=".$senderId;
				$api = "http://115.249.28.90/sms/sendSMS.php?from=".$senderId."&to=@number@&text=@text@";
			}
			if($api != '' && preg_match('/^[7-9][0-9]{9}$/',substr($number,-10,10))){
				$remsg = $message;
				//$reply = $api."&to=".substr($number,-10,10)."&text=".urlencode($remsg);
				$reply = $api;
				$reply = str_replace('@number@',substr($number,-10,10),$reply);
				$reply = str_replace('@text@',urlencode($remsg),$reply);
				$ret = file($reply);
	 ...
	 /*function sms_send($number,$message,$senderId='VMCIND'){
