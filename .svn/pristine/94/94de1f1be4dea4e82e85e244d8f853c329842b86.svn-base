	...
	function getgrouplist($bid,$ofset='0',$limit='20',$type=''){
		...
		foreach($rst as $rec){
			...
			if($type == ''){
				...
				$act .= ($opt_delete) ? '&nbsp;<a href="interview/undelete/'.$r['gid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>':'';
			}
			array_push($data,$act);
			$i++;
	...
	function getqblist($bid,$ofset='0',$limit='20',$type=''){
		...
		foreach($rst as $rec){
			...
			if($type == ''){
				...
				$act = "";
				$act .= ($opt_delete) ? '&nbsp;<a href="interview/undeleteqb/'.$r['qb_id'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>':'';
			}
			array_push($data,$act);
			$i++;
			array_push($list,$data);
	...
	function listQuestions($bid,$ofset='0',$limit='20',$type=''){
		...
		foreach($rst as $rec){
			...
			if($type == ''){
				...
				$act = "";
				$act .= ($opt_delete) ? '&nbsp;<a href="interview/undelQuestion/'.$r['qid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>':'';
			}
			array_push($data,$act);
			$i++;
			array_push($list,$data);
