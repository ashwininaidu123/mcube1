...
Class Commonmodel extends Model{
	...
	function EmployeeAdd($arr,$bid){
		if(!empty($arr)){
				...
				if($dnd['dnd']==1){
					...
					$sms = sms_send($arr[0]['empnumber'],$message,$senderid,'code');
	...
	function register($partner_id='',$st=''){
		...
		$this->db->set('bid', $bid); 
		$this->db->set('businessname', $this->input->post('login_businessname')); 
		$this->db->set('businessType', $this->input->post('businessType')); 
		$this->db->set('businessemail', $this->input->post('login_username')); 
		$this->db->set('contactemail', $this->input->post('cemail')); 
		$this->db->set('contactname', $this->input->post('cname')); 
		$this->db->set('contactphone', $this->input->post('cphone')); 
		...
		$this->db->set('bid',$bid);
		$this->db->insert('businessacc_details');
		...
		$this->db->set('dailyreport', 1);
		$this->db->set('weeklyreport', 1);
		$this->db->set('login', 1);
		$this->db->set('status', 1);
...
//~ 
			...
			$this->email->bcc('tapan.chatterjee@vmc.in,padma.b@vmc.in,accounts@vmc.in'); 
	...
	<div class="pagecontent">
		...
		<div class="mcube">
			<div><a href="dashboard"><img src="'.base_url().'images/spacer.gif" width="125" height="40" border="0" /></a></div>
...
<div id="middle"><div class="pagecontent">';
  ...
  PRIMARY KEY (`search_id`)
		...
		$this->db->query('CREATE TABLE IF NOT EXISTS `' .$bid.'_callhistory` (
			...
			`landingnumber` varchar(15) NOT NULL,
			...
			`callee_gender` VARCHAR( 10 ) NOT NULL,
			`caller_gender` VARCHAR( 10 ) NOT NULL,
			`emotion` VARCHAR( 50 ) NOT NULL,
			`speech_text` TEXT NOT NULL,
			`word_cloud` VARCHAR( 250 ) NOT NULL,
			...
			`oncallResponse` TEXT NOT NULL,
			`onhangupResponse` TEXT NOT NULL,
			`apiStatus` TINYINT(2) NOT NULL,
			`hangupStatus` TINYINT(2) NOT NULL,
			PRIMARY KEY (`callid`)
	  ...
	  KEY `callid` (`callid`)
		...
		$this->db->query('CREATE TABLE IF NOT EXISTS `'.$bid.'_employee` (
			...
			`weeklyreport` TINYINT( 2 ) NOT NULL,
			`dailyreport` TINYINT( 2 ) NOT NULL,
			`dnd` TINYINT(1) NOT NULL DEFAULT "0", 
			`verify` TINYINT(1) NOT NULL DEFAULT "1",
			`gcmkey_mcube`varchar(300)NOT NULL,
			PRIMARY KEY (`eid`)
			) ENGINE=InnoDB  DEFAULT CHARSET=utf8');
		...
		$this->db->query('CREATE TABLE `' .$bid.'_activitylog` (`sno` INT NOT NULL AUTO_INCREMENT PRIMARY KEY, `bid` INT NOT NULL, `uid` INT NOT NULL, `module_name` VARCHAR(50) NOT NULL, `action` TEXT NOT NULL, `date_time` TIMESTAMP NOT NULL  DEFAULT CURRENT_TIMESTAMP) ENGINE = InnoDB DEFAULT CHARSET=utf8');	
	...
	$this->db->query("CREATE TABLE IF NOT EXISTS `".$bid."_outboundcalls` (
		...
		$this->db->query("CREATE TABLE IF NOT EXISTS `".$bid."_mtracker` (
						  `callid` varchar(25) NOT NULL,
						  `bid` int(11) NOT NULL,
						  `eid` int(11) NOT NULL,
						  `callfrom` varchar(20) NOT NULL,
						  ...
						  `remark` text NOT NULL,
						  `status` tinyint(1) NOT NULL,
						  `mcall` TINYINT( 2 ) NOT NULL,
						  `keyword` varchar(100) NOT NULL,
						  `assignto` int(11) NOT NULL,
						  `leadid` int(11) NOT NULL,
						  `tktid` int(11) NOT NULL DEFAULT '0',
						  `source` varchar(50) NOT NULL DEFAULT 'calltrack',
						  `last_modified` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
						  PRIMARY KEY (`callid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8;
						");
			$this->db->query("CREATE TABLE IF NOT EXISTS `".$bid."_mtarchive` (
							  `callid` varchar(25) NOT NULL,
							  `bid` int(11) NOT NULL,
							  `eid` int(11) NOT NULL,
							  `callfrom` varchar(20) NOT NULL,
			...
			$this->db->query("CREATE TABLE IF NOT EXISTS `".$bid."_mt_emplist` (
						  `bid` int(11) NOT NULL,
						  `eid` int(11) NOT NULL,
						  ...
						  UNIQUE KEY `bid` (`bid`,`eid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
			$this->db->query("CREATE TABLE IF NOT EXISTS `".$bid."_mt_emptimecheck` (
							  `bid` int(11) NOT NULL,
							  `eid` int(11) NOT NULL,
	...
	function userexists($str){
		$condition=($this->uri->segment('4')!="")?"username='".$str ."' and eid!='".$this->uri->segment('3')."'":"username='".$str."'";
		...
		if($res->num_rows()>0){
			...
			if($e_id->num_rows()>0){
				...
				if($row1->login!=0){
					...
					$mess="Hi".$row1->empname."\nLog on to MCube using these credentials\nUsername:".$row1->empemail."\nPassword:".$password;
					$this->load->helper('mcube_helper');
					$senderid = $this->busSenderId($bid);
					$sms = sms_send($row1->empnumber,$mess,$senderid,'password');
					//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=vmc.in";
					...
					$sms = sms_send($row1->empnumber,$mess,$senderid,'code');
					//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=vmc.in";
					//~ $reply = $api."&to=".substr($row1->empnumber,-10,10)."&text=".urlencode($mess."\nPowered by MCube");
					//~ file($reply);
					return true;
	...
	function check_Mverification(){
		...
		if($sql->num_rows()>0){
			...
			$sms = sms_send($res->number,$message,$senderid,'code');
			//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=vmc.in";
			//~ $message = "\nPlease authorize VMC to call you from MCube ".base_url().'verifyYourNumber'." Verification code: ".$res->vcode;
