...
class Pbxmodel extends Model {
	...
	function getPBXlist($bid,$ofset='0',$limit='20'){
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				...
				$act .= ($opt_add)? '<li><a href="PBXadd/'.$r['pbxid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>':'';
				$act .= ($opt_delete)? '<li><a href="pbx/delete/'.$r['pbxid'].'" class="confirm deleteClass"><span title="Delete" class="glyphicon glyphicon-trash">&nbsp;Delete</span></a></li>':'';
				$act .= ($opt_add)? '<li><a href="pbx/addext/'.$r['pbxid'].'"  class="btn-danger" data-toggle="modal" data-target="#modal-responsive"><span class="glyphicon glyphicon-plus" title="Add Extension" >&nbsp;AddExtension</span></a></li>':'';
				$act .= ($opt_add)? '<li><a href="Listextn/'.$r['pbxid'].'"><span title="List Option" class="glyphicon glyphicon-list">&nbsp;ListOptions</span></a></li>':'';
				$act .= '</ul></div>';
				$data['action'] = $act;
			}
			...
			$v = '';
			foreach($bday as $b => $d){ $v .= (isset($d->day) && $d->day=='1')?$b.'='.$d->st.'-'.$d->et.' ':'';}
		    $v = "<a href='javascript://' rel='popover' data-content='".$v."' data-placement='top' data-toggle='popover' data-container='body'>Working Hours</a>";
			$data['bday']=$v;
			$i++;
	...
	function recent_calls($pbxid){
		$res=array();
		$ret=array();
		$sql=$this->db->query("SELECT COALESCE(COUNT(callid), 0) AS cnt
							FROM ".$this->session->userdata('bid')."_pbxreport h 
							WHERE (h.`starttime` >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)) AND h.pbxid = ".$pbxid." GROUP BY DAY(h.starttime)");	
	...
	function getPBXExtlist($pbxid,$ofset='0',$limit='20'){
		...
		$opt_view 	= $roleDetail['modules']['20']['opt_view'];
		$opt_delete = $roleDetail['modules']['20']['opt_delete'];
		$header =  array($this->lang->line('label_pbxext')
	...
	function getDeletedPBXlist($bid,$ofset='0',$limit='20'){
		...
		$con .= (isset($s['title']) && $s['title']!='')?" AND i.title like '%".$s['title']."%'":"";
		$con .= (isset($s['prinumber']) && $s['prinumber']!='')?" AND p.landingnumber like '%".$s['prinumber']."%'":"";
		$con .= (isset($s['operator']) && $s['operator']!='' && $s['operator']!='0')?" AND i.operator  like '%".$s['operator']."%'":"";
		$sql = "SELECT SQL_CALC_FOUND_ROWS pbxid FROM ".$bid."_pbx i
				LEFT JOIN prinumber p on i.prinumber=p.number
	...
	//  echo $sql; exit;
		...
		foreach($rst as $rec){
			...
			$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
			$act .= '<li><a href="pbx/undelete/'.$r['pbxid'].'" class="" id="'.$r['pbxid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a></li> ';
			$act .= '</ul></div>';
			$data['action'] = $act;
			$i++;
			array_push($list,$data);
	...
	function addpbxExt(){
		...
		if($cnt>0){
			...
			if($_POST['targettype']=="employee"){
				...
				$this->db->where('eid',$tid);
				$this->db->update($bid."_employee");
				$empName=$this->groupmodel->get_empname($tid);
	...
	function getPBXReportlist($bid,$ofset='0',$limit='20',$tab){
		...
		$con.=(isset($s['endtime']) && $s['endtime']!='')?" and date(r.starttime)<= '".$s['endtime']."'":"";
		...
		foreach($rst as $rec){
			...
			if($opt_add || $opt_view || $opt_delete){
				$act = '<div class="btn-group">&nbsp;&nbsp;
							<a class="dropdown-toggle" data-toggle="dropdown" style=";font-weight:bold;"> Action <span class="caret"></span></a>
							<ul class="dropdown-menu" style="text-align:left;">';
				$act .= ($opt_add) ? '<li><a href="PBXEditReport/'.$rec['callid'].'"><span title="Edit" class="glyphicon glyphicon-edit">&nbsp;Edit</span></a></li>': '';
				$act .= 				
				...
				:'<li><a><span class="glyphicon glyphicon-volume-off">&nbsp;Recording</span></li></a> '):'';
				$act .= '<li>'.anchor("Report/clicktoconnect/".$rec['callid']."/3", '<span title="click To Connect" class="glyphicon glyphicon-phone">&nbsp;ClickToConnect</span>',array('class'=>'clickToConnect')).'</li>';
				$act .= ($r['email']!='')?"<li><a href=\"javascript:void(null)\" onClick=\"window.open('/Email/compose/".$rec['callid']."/pbx', 'Send Email', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=970,height=480,resizable=1')\"><span title='Send Mail' class='glyphicon glyphicon-envelope'>&nbsp;SendMail</span></a></li>":'<li><a href=""><span title="Send Mail" class="glyphicon glyphicon-envelope">&nbsp;SendMail</span></a></li>';
				$act .= '<li><a href="Report/followup/'.$rec['callid'].'/0/pbx" class="btn-followup" data-toggle="modal" data-target="#modal-followup"><span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span></a></li>';
				$act .= '<li>'.anchor("Report/sendSms/".$rec['callid']."/pbx", '&nbsp;<span title="Click to send SMS" class="glyphicon glyphicon-comment">&nbsp;SendSMS</span>','class="clickToSMS" data-toggle="modal" data-target="#modal-empl"').'</li>';	
				$act .= "<li><a href=\"Javascript:void(null)\" onClick=\"window.open('Report/sendFields/".$rec['callid']."/".$bid."/24/pbx', 'Send Fields', 'toolbar=0,scrollbars=1,location=0,status=1,menubar=0,width=550,height=500,left=200,top=20,resizable=1')\"><span title='Click to Send Fields' class='glyphicon glyphicon-list'>&nbsp;SendFields</span></a></li>";
				$act .= "</ul></div>";
				$data['action'] = $act;
			}
			foreach($keys as $k){
				...
				//~ }elseif($k=='callfrom'){
					//~ $v = anchor("Report/clicktoconnect/".$rec['pbxid']."/3", "$r[$k]",array('class'=>'clickToConnect','title' =>'Click to connect'));
				}elseif($k=="extensions"){
	...
	function getPrintReport($bid){
		...
		$sql .= (isset($_POST['etime']) && $_POST['etime']!='') ? " AND date(r.starttime)<='".$_POST['etime']."' " : "";
