...
class Leads extends controller {
	...
	function lead_grp_list($type=''){
		...
		$links = array();
		if($type == 'act'){
			$links[] = '<li><a href="AddLeadGroup"><span title="Add Group" class="glyphicon glyphicon-plus-sign">&nbsp;AddGroup</span></a></li>';
		...
		$data['links'] = $links;
		$data['form'] = array(
					'open'=>form_open_multipart(site_url($this->uri->segment(1).'/0'),array('name'=>'listleadgrp','class'=>'form','id'=>'manageemp','method'=>'post')),
	...
	function lead_grpemp_list($lgid){
		...
		if(count($emp_list['data'])>0){
			...
			foreach ($emp_list['data'] as $item){
				...
				$rec[] = ($item['status']=="1" && $item['estatus']=='1') ? $item['counter'] : '<font color=red>'.$item['counter'].'</font>';
				$rec['action'] = $opt;
				$i++;
				array_push($list,$rec);
			}
		...
		$links = array();
		$links[] = '<li><a href="AddempLeadGroup/'.$lgid.'"><span title="Add Employee to Lead Group" class="glyphicon glyphicon-plus-sign">&nbsp;AddEmployee</span></a></li>';
		$links[] = '<li><a href="leads/refreshcounter/'.$lgid.'"><span title="Reset Counter" class="fa fa-refresh">&nbsp;RefreshCounter</span></a></li>';
		$formFields = array();
	...
	function index($type='',$lead_num=''){
		...
		$cbid=$this->session->userdata('cbid');
		$this->session->set_userdata('lead_num', $lead_num);
		$bid=(isset($cbid) && $cbid!="")?$cbid:$this->session->userdata('bid');
		$dlink = "";
	    ...
	    if($this->input->post('graphchart')){
		   $data1['module']['title'] = 'Lead Analytics';
		   $data1['lead_recent_calls'] = $this->reportmodel->lead_groupwiseleads($type,$gettimelimit='');	
		   $data1['lead_source'] = $this->reportmodel->lead_source($type,$gettimelimit='');
		   $data1['lead_bytype'] = $this->reportmodel->lead_bytype($type,$gettimelimit='');
		   $data1['lead_employee'] = $this->reportmodel->lead_employee($type,$gettimelimit='');
		   $this->sysconfmodel->viewLayout('Leadanalytics',$data1);
		   return true;
	    }	
		$parentbids=array();
		if($this->session->userdata('eid')==1){
		...
		$links[] = ($roleDetail['modules'][$modid]['opt_delete']) ?'<li><a href="leads/bulkDel" class="blkDel"><span title="BulkDelete" class="glyphicon glyphicon-trash">&nbsp;BulkDelete</span></a></li>':'';
		$links[] = ($roleDetail['modules'][$modid]['opt_add']) ?'<li><a href="leads/bulkAssign/'.$type.'" class="blkAssign" data-toggle="modal" data-target="#modal-blkAssign"><span title="Bulk Assign" class="glyphicon glyphicon-share">&nbsp;BulkAssign</span></a></li>':'';
		$links[] = ($roleDetail['modules'][$modid]['opt_add']) ?'<li><a href="leads/leadowner/'.$type.'" class="leadowner" data-toggle="modal" data-target="#modal-leadowner"><span title="Bulk Lead Owner" class="glyphicon glyphicon-move">&nbsp;BulkLeadOwner</a></li>':'';
		$links[] = '<li><a href="leads/bulkStatChng/'.$type.'" class="blkStatus" data-toggle="modal" data-target="#modal-blkStatus"><span title="Bulk Status Change" class="glyphicon glyphicon-repeat">&nbsp;BulkStatusChange</span></a></li>';
		$links[] = '<li><a href="#" class="blkemail" rel="leads"><span title="Bulk Mail" class="glyphicon glyphicon-envelope">&nbsp;BulkEmail</span></a></li>';
		$links[] = '<li><a href="Report/blksms" class="blkSMs" data-toggle="modal" data-target="#modal-blksms" rel="leads"><span title="Bulk SMS" class="glyphicon glyphicon-comment">&nbsp;BulkSMS</span></a></li>';
		...
		$links[] = '<li class="divider"><a>&nbsp;</a></li>';
		$links[] = ($roleDetail['modules'][$modid]['opt_download']!=0)? '<li><a href="leads/Bulk_down/'.$type.'" class="blk_calls"  data-toggle="modal" data-target="#modal-pop"><span title="Download" class="glyphicon glyphicon-arrow-down">&nbsp;DownloadSelect</span></a></li>':'';
		$links[] = ($roleDetail['modules'][$modid]['opt_download']) ? '<li><a href="leads/leads_csv/'.$type.'" class="btn-csv" data-toggle="modal" data-target="#modal-csv"><span title="Download" class="glyphicon glyphicon-download-alt">&nbsp;DownloadAll</span></a></li>':'';
		$fieldset = $this->configmodel->getFields($modid,$bid);
		$formFields = array();
		$advsearch = array();
		foreach($fieldset as $field){
									...
									'field'=>
										($field['fieldname']=='gid')
											...
											:(($field['fieldname']=='assignto')
											   ...
											   :(($field['fieldname']=='leadowner')
												?form_dropdown('leadowner',$this->leadsmodel->employee_list(),'',"class='form-control'")
	...
	function edit($id,$type=''){
		...
		if($roleDetail['role']['admin'] != 1 ){
			if($roleDetail['role']['roleid'] != 4){
				$empcheck = $this->leadsmodel->emp_check($id);
		...
		$cf = array('label'=>'<label class="col-sm-4 control-label">Remarks History :</label>',
					 'field'=>anchor("leads/remarks_history/".$id."/".$type, '<span title="Lead Remarks" class="glyphicon glyphicon-asterisk"></span>' ,'class="btn-followup" data-toggle="modal" data-target="#modal-followup"')
					);
		array_push($formFields,$cf);
		...
		$cf = array('label'=>'<label class="col-sm-4 control-label">Followup : </label>',
					'field'=>anchor("Report/followup/".$id."/0/leads",'<span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span>','class="btn-followup" data-toggle="modal" data-target="#modal-followup"')
					);
		array_push($formFields,$cf);
	...
	function deleteList($type=''){
		...
		$data['module']['title'] = "Deleted List [".$data['itemlist']['count']."]";
		$fieldset = $this->configmodel->getFields($modid,$bid);
		$links = array();
		$links[] = '<li><a href="'.$_SERVER['REQUEST_URI'].'" class="btn-search" data-toggle="modal" data-target="#modal-search" ><span title="Search" class="glyphicon glyphicon-search">&nbsp;Search</span></a></li>';
		$data['links'] = $links;
		...
		if($steps == '' || $steps =='step1'){
			...
			if($err == "TRUE"){
				...
				foreach($system_leads as $k=>$v){
					if(!in_array($v,array("leadid","bid","gid","enteredby","lastmodified","status"))){
			...
			if (($handle = fopen($moved_file, "r")) !== FALSE) {
				while (($csvdata = fgetcsv($handle,'', ',',$this->input->post('enclosure')) )!== FALSE) {
					if($row != 1 && $csvdata[0]!=''){
						...
						if(!(@array_key_exists('createdon',$data) && $data['createdon'] != '')){
						...
						if(isset($data['assignto']) && $data['assignto']!= '' && $data['gid'] != '' ){
							if($dis_type == 2)
			    ...
			    // Send Email for leads Imported
				$this->leadsmodel->sendLeadEmail();
				fclose($handle);
			}
