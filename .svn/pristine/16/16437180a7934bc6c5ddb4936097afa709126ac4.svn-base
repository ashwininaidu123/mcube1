...
Class cronjobmodel extends Model
{
	...
	Function SendMailToUser($email,$message){
				...
				$mail = mail($to, $subject, $emailbody, $headers,'support@vmc.in');
...
if (mail( $recipientAddr , $subjectStr , $mailBodyEncodedText, $headers,'support@vmc.in')) {
  unlink($filePath);
}					
...
//~ }					
		...
		if($employees[0]['eid'] != ''){
			for($emp =0;$emp <count($employees);$emp++){
				$sqL=$this->db->query("
								...
								UNION
								SELECT f.eid as eid,'leads' as type,f.followupdate as followupdate,l.number AS callfrom,l.name as callername FROM ".$bid."_followup f 
								LEFT JOIN ".$bid."_leads l on f.callid = l.leadid
								WHERE CAST(f.followupdate AS DATE) = CURRENT_DATE() AND f.eid='".$employees[$emp]['eid']."' AND (f.alert=1 or f.alert=3)");
				if($sqL->num_rows()>0){
					$message = " ";
					$message.="<table cellpadding='3' cellspacing='3'  border='1' width='500'>
								 <tr>
	...
	function SMSFollowup($bid,$smsBalance){
		...
		if($employees[0]['eid'] != ''){
			for($emp =0;$emp <count($employees);$emp++){
				...
				UNION
				SELECT lf.eid as eid,lf.type,lf.followupdate as followupdate,l.number AS callfrom,l.name as callername,lf.comment as message,lf.callid as callid,lf.type as source FROM ".$bid."_followup lf 
				...
				if($sqL->num_rows()>0){
					foreach($sqL->result_array() as $rows){
						if($smsBalance>0 && (preg_match('/^[7-9][0-9]{9}$/',substr($employees[$emp]['empnumber'],-10,10)))){
							...
							$sms = sms_send($employees[$emp]['empnumber'],$rows['message'],$senderid);
	...
	function sendSMS($sms){
		$this->load->helper('mcube_helper');
		$senderid = $this->busSenderId($bid);
		$sms = sms_send($sms['number'],$sms['message'],$senderid);
		//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
	...
	function weeklyEmpWise($bid,$eid='',$day){
		$res=array();
		$q='';
		$q1='';
		if($day == 'weekly'){
	...
	function weeklyGrWise($bid,$eid='',$day){
		$res=array();
		$q='';
		$q1='';
		if($day == 'weekly'){
		...
		$sql=$this->db->query("SELECT g.gid,g.groupname,COALESCE(h.cnt,0) as count 
								...
								WHERE h.cnt>0 ORDER BY g.groupname");
									//echo $this->db->last_query();
		if($sql->num_rows()>0){
			$res=$sql->result_array();
		}
	...
	function supEscProcess($bid,$smsbal){
		...
		if(!empty($employees) && $employees[0]['eid'] != ''){
			for($emp =0;$emp <count($employees);$emp++){
					...
					foreach($sql->result_array() as $rows){
						...
						if($reportChk->num_rows()>0){
							...
							if($smsbal>0 && (preg_match('/^[7-9][0-9]{9}$/',substr($employees[$emp]['empnumber'],-10,10)))){
								...
								$this->load->helper('mcube_helper');
								$senderid = $this->busSenderId($bid);
								$sms = sms_send($employees[$emp]['empnumber'],$message,$senderid);				
								//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
							...
							if($smsbal>0 && (preg_match('/^[7-9][0-9]{9}$/',substr($reportEmpdetails->empnumber,-10,10)))){
								...
								$this->load->helper('mcube_helper');
								$senderid = $this->busSenderId($bid);
								$sms = sms_send($reportEmpdetails->empnumber,$message,$senderid);				
								//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
		...
		if($sql->num_rows() >0 ){
			...
			for($i=0;$i<=count($rst);$i++){
				...
				if($smsbal>0 && (preg_match('/^[7-9][0-9]{9}$/',substr($tktdetails['empnumber'],-10,10)))){
					...
					$this->load->helper('mcube_helper');
					$senderid = $this->busSenderId($bid);
					$sms = sms_send($tktdetails['empnumber'],$message,$senderid);			
					//~ $api = "http://115.249.28.90/sms/sendSMS.php?from=VMCIND";
   ...
   function leadEmpCalls($bid=''){
	...
	function mtArchive(){
		$busList = $this->getAllBusiness();
		foreach($busList as $bus){
			$sql = "INSERT INTO ".$bus['bid']."_mtarchive (SELECT * FROM `".$bus['bid']."_mtracker` WHERE date(`starttime`)<=DATE_SUB(CURRENT_DATE(),INTERVAL 30 DAY))";
			$this->db->query($sql);
			if($this->db->affected_rows()>0){
				$sql1 = "DELETE FROM `".$bus['bid']."_mtracker` WHERE date(`starttime`)<=DATE_SUB(CURRENT_DATE(),INTERVAL 30 DAY);";
				$this->db->query($sql1);
			}
		}
	}
	...
	function recordlimit(){
	  $res['data']=$this->db->query("SELECT mt_record_valid as mlimit,bid FROM account_settings WHERE 1")->result_array();
	  $res['count'] = $this->db->query("SELECT FOUND_ROWS() as cnt")->row()->cnt;
	  foreach($res['data'] as $r){
	     $r[$res['data'][0]['bid']]=$r['mlimit'];
	  } 
      $r['count'] = $res['count'];
      return $r; 
	}
	
	function getvalidrecord($data){
	    $sql = $this->db->query("SELECT filename  FROM `".$data['bid']."_mtracker` WHERE date(`starttime`) <= DATE_SUB(NOW(), INTERVAL ".$data['mlimit']." DAY)")->result_array();
			foreach($sql as $filename){
				 $path = './sounds/'.$filename['filename'] ;
				  if(is_file($path)){
					  unlink($path);
				  }
			}  
