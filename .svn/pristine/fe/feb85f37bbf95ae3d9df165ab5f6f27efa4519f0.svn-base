...
FROM ".$bid."_polloptions opt left join ".$bid."_pollreport p on p.polloption=opt.option_id where opt.poll_id=".$rows['poll_id']." group by p.polloption order by opt.optionkey");
	...
	function smsAssign($data){
		...
		if($eid!='0'){
			$this->load->helper('mcube_helper');
			$senderid = $this->configmodel->busSenderId($bid);
			$sms_text = "You have a SMS from ".$callfrom." as ".$data['text'];
			$sms = sms_send($eNumber,$sms_text,$senderid);
			...
			//~ file( $api);
			$fp = fopen('smslog.txt','a+');fwrite($fp,"\n[".date('Y-m-d H:i:s')."] ".$eNumber." ".$sms_text);fclose($fp);
			$to = $eEmail;
	...
	function ChangeEmpStatus($bid){
      	$email = urldecode($_REQUEST['empemail']);
      	$status = urldecode($_REQUEST['status']);
		$emp = $this->db->query("SELECT status,empname,eid FROM ".$bid."_employee WHERE empemail = '".$email."'");
		$rec = $emp->row();
		$eid = $rec->eid;
		if($status=='disable'){
