...
Class campaignmodel extends Model{
	...
	function getReport($bid,$ofset,$limit,$type=''){
		...
		foreach($rst as $rec){
			...
			$act = '';
			if($type == 'del'){
				$act = ($opt_add) ?'<a href="campaign/undeleteCampReport/'.$rec['callid'].'"><span title="Restore" class="glyphicon glyphicon-refresh">&nbsp;UnDelete</span></a>':'';
				...
				$act .= anchor("Report/followup/".$rec['callid']."/0/campaign", ' <span title="Followups" class="glyphicon glyphicon-book">&nbsp;Followups</span>',array(' class="btn-danger" data-toggle="modal" data-target="#modal-responsive"'));
