...
//~ mysql_connect("localhost","root","064818@@muk");
...
$q .= (isset($_POST['SIMInfo']) && $_POST['SIMInfo']!='') ? " AND numberlist.`SIM Info` like '%".mysql_real_escape_string($_POST['SIMInfo'])."%'" : "";
...
$q .= (isset($_POST['BusinessStatus']) && $_POST['BusinessStatus']!='') ? " AND numberlist.`Business Status` = '".$_POST['BusinessStatus']."'" : "";
...
CASE
  WHEN p.payment_term = 1 THEN 'Monthly'
  WHEN p.payment_term = 2 THEN 'By Monthly'
  WHEN p.payment_term = 3 THEN 'quarterly'
  WHEN p.payment_term = 6 THEN 'Half Yearly'
  WHEN p.payment_term = 12 THEN 'Yearly'
END as `Payment Term`,
...
if(p.number_status=1,'Active','Inactive') as `Status`
FROM
(SELECT 
p.number,                       b.businessname,                 p.landingnumber,  p.number_status,
